...
test('「次の行動」発言は仮選択時ではなく最終確定カードで判定する', () => {
  const room = setup(); clear(room);
  room.phase = 'chat';
  const [speaker,subject] = room.players;
  subject.draft = draft({ normal:'attack', normalTargetId:speaker.playerId });
  const claim = recordStructuredStatement(room, speaker, {
    subjectId:subject.playerId, kind:'nextAction', normalType:'attack'
  });
  assert.equal(claim.ok, true);
  assert.equal(claim.deferred, true);
  assert.equal(speaker.secretState.infoByTurn[1], undefined);
  // 会話後に対象が行動を変更
  subject.draft = draft({ normal:'defense' });
  room.phase = 'action';
  resolveTurn(room);
...
test('カード指定を受けた対象が必須対象を選ばずタイムアウトしても指定カードを温存できない', () => {
  const room = setup(); clear(room);
  const [target,other] = room.players;
  room.turn = 2;
  ...
  assert.equal(other.hp, 5, '対象未選択なので攻撃効果は不発');
});
...
test('カード指定の強制通常カードでタイムアウトしても、未完成の特殊カードは誤消費しない', () => {
  const room = setup(); clear(room);
  ...
  // 攻撃対象も無効対象も未選択。強制された攻撃だけは消費、未完成の無効カードは残るべき。
...
test('ターン解決エラー時の保留nextActionは残留せず正直者/詐欺師のどちらにも加点材料にならない', () => {
  ...
  const [speaker,subject] = room.players;
  room.phase = 'chat';
  const r = recordStructuredStatement(room, speaker, { kind:'nextAction', subjectId:subject.playerId, normalType:'attack' });
  assert.equal(r.ok, true);
