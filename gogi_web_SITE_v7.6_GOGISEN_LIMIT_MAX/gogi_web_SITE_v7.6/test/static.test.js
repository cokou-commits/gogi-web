...
test('他プレイヤー個人の準備OK/行動確定状態は公開せず集計だけ返す', () => {
  ...
  assert.match(app, /state\.phaseProgress\?\.locked/);
});
...
test('結果表示後、全員脱落ならターン番号を増やさず終了する', () => {
  const block = server.match(/room\.timer = setTimeout\(\(\) => \{[\s\S]*?RESULT_SECONDS \* 1000 \+ 40\);/)?.[0] || '';
  assert.match(block, /getAlive\(room\)\.length === 0/);
...
test('高速な複数選択でdraftを上書きしないよう更新を直列化し、確定前に待つ', () => {
  ...
  assert.match(app, /lockAction[\s\S]*?await waitForDraftUpdates\(\)/);
});
...
test('準備OK/行動確定は待機開始後に追加されたdraft更新まで完全にflushする', () => {
  const app = fs.readFileSync(path.join(root, 'public', 'app.js'), 'utf8');
...
test('Socketイベントはnull/primitive payloadでもデストラクチャ例外でプロセスを落とさない', () => {
  ...
  for (const name of ['resume','publicMatch','createPrivate','joinPrivate','setReady','setDraft','lockAction','chat','infoStatement','buy','transferCard','transferPoints']) {
...
test('再接続でSocketレート制限をリセットしてもプレイヤー単位制限が残る', () => {
  ...
  for (const key of ['resume','ready','draft','lock','chat','info','buy','transferCard','transferPoints']) {
...
test('モバイルではフェーズ切替時に10秒の行動画面を自動で表示位置へ戻す', () => {
  assert.match(app, /function navigateToActivePhaseOnMobile\(\)/);
  assert.match(app, /state\.phase === 'action' \? \$\('#actionPanel'\)/);
...
test('10秒の行動開始時はルールモーダルと入力フォーカスを閉じて操作を塞がない', () => {
  assert.match(app, /state\?\.phase === 'action'[\s\S]*closeRules\(\)/);
  assert.match(app, /document\.activeElement[\s\S]*active\.blur\(\)/);
});
...
test('最後の準備OK/行動確定は冗長な中間stateを配信せず次フェーズへ直接進む', () => {
  const readyBlock = server.match(/socket\.on\('setReady'[\s\S]*?\n  \}\);/)?.[0] || '';
  const actionBlock = server.match(/socket\.on\('lockAction'[\s\S]*?\n  \}\);/)?.[0] || '';
  assert.match(readyBlock, /if \(allAliveReady\(room\)\) beginAction\(room\);\s*else emitState\(room\)/);
  assert.match(actionBlock, /if \(allAliveLocked\(room\)\) resolveAndShowResult\(room\);\s*else emitState\(room\)/);
});
...
test('モバイルの主要小型操作は44px以上のタップ領域を確保する', () => {
  ...
  assert.match(css, /\.quickOps button,\.chatTop select,\.shopRow button\{min-height:44px\}/);
  assert.match(css, /\.shopRow button\{min-width:44px\}/);
});

test('モバイルの10秒行動確定ボタンは先頭へ寄せスクロール中も操作できる', () => {
  assert.match(css, /#actionPanel \.actionTitle\{order:0\}/);
  assert.match(css, /#lockAction\{[\s\S]*?order:1;[\s\S]*?position:sticky;[\s\S]*?top:calc\(120px \+ env\(safe-area-inset-top\)\);[\s\S]*?z-index:6;/);
  assert.match(css, /#actionComposer\{order:2\}/);
});
...
test('対戦フェーズ名は日本語でチャット中・バトル中・結果発表を明示する', () => {
  assert.match(app, /chat:'チャット中'/);
  assert.match(app, /action:'バトル中'/);
  assert.match(app, /result:'結果発表'/);
  assert.match(html, /id="phaseLabel">チャット中</);
});
...
test('PCでも10秒の行動確定ボタンをカードより先に表示し固定する', () => {
  assert.match(css, /#actionPanel \.actionTitle\{order:0\}/);
  assert.match(css, /#lockAction\{[\s\S]*?order:1;[\s\S]*?position:sticky;[\s\S]*?top:12px;[\s\S]*?z-index:8;/);
  assert.match(css, /#actionComposer\{order:2\}/);
});
...
test('会話クイック操作から偵察結果へ直接移動できる', () => {
  assert.match(html, /data-scroll-target="scoutReports">偵察結果<\/button>/);
  assert.match(css, /\.quickOps\{grid-template-columns:repeat\(4,minmax\(0,1fr\)\)\}/);
});
...
test('カード画像はリビジョン付きURLとimmutable長期キャッシュを使う', () => {
  assert.match(app, /const ASSET_REV = '20260911b'/);
