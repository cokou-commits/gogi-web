...
function cpuAcceptedContract(room, p) {
  return (room.publicContracts || []).find(c => c.status === 'accepted' && Array.isArray(c.acceptorIds) && c.acceptorIds.includes(p.playerId) && c.dueTurn === room.turn) || null;
}
...
function tryAcceptPublicContractAsCpu(room, contract) {
  ...
  const candidates = room.players.filter(p => isTestCpu(p) && p.alive && p.playerId !== contract.issuerId && !contract.acceptorIds.includes(p.playerId) && !(contract.conditionType === 'attackTarget' && contract.subjectId === p.playerId));
  const cpu = cpuRandom(candidates);
  if (!cpu) return false;
  contract.acceptorIds.push(cpu.playerId);
  cpu.turnHadManualInput = true;
  const issuer = getPlayer(room, contract.issuerId);
  log(room, `公開契約参加：${cpu.color.label}（CP）が${issuer?.color?.label || '契約主'}の契約に参加しました。`);
  emitGameNotice(room, room.players.map(x => x.playerId), { kind:'contract', toolTarget:'contractBox', text:`${cpu.color.label}（CP）が公開契約に参加しました。` });
  return true;
}
...
function expireOpenPublicContracts(room) {
  ...
  for (const contract of room.publicContracts) {
    ...
    if (acceptorIds.length > 0) {
      ...
      log(room, `${issuer?.color?.label || '契約主'}の公開契約は${acceptorIds.length}人参加で受付終了。第${contract.dueTurn}ターンに判定します。`);
    } else {
      contract.status = 'expired';
      refundPublicContract(room, contract);
      if (issuer) log(room, `${issuer.color.label}の公開契約は参加者なしで終了し、${contract.reward}Pが返却されました。`);
    }
  }
}
  ...
  for (const contract of room.publicContracts) {
    ...
    for (const playerId of acceptorIds) {
      ...
      if (success && acceptor) {
        ...
        emitGameNotice(room, [playerId], { kind:'contract', toolTarget:'contractBox', text:`公開契約達成：+${perPlayer}P` });
      }
    }
    ...
    const text = successCount > 0
      ? `公開契約結果：${successColors.join('・')}が達成。1人${perPlayer}P、合計${payoutTotal}Pを配布し、余り${refund}Pを${issuer?.color?.label || '契約主'}へ返却しました。`
      : `公開契約結果：達成者なし。預けた${contract.reward}Pを${issuer?.color?.label || '契約主'}へ返却しました。`;
    result.publicEvents.push({ type:'contract', text });
    if (issuer) emitGameNotice(room, [issuer.playerId], { kind:'contract', toolTarget:'contractBox', text:`公開契約終了：${successCount}人達成 / ${refund}P返却` });
  }
}
...
io.on('connection', socket => {
  ...
  socket.on('postPublicContract', (rawPayload = {}, cb) => {
    ...
    emitGameNotice(room, room.players.filter(x => x.playerId !== p.playerId).map(x => x.playerId), { kind:'contract', toolTarget:'contractBox', text:`新しい公開契約：達成者1人${n/5}P` });
  ...
  socket.on('acceptPublicContract', (rawPayload = {}, cb) => {
    ...
    markTurnActivity(room, p); commitMutation(p, mutation.key);
    const issuer = getPlayer(room, contract.issuerId);
    log(room, `公開契約参加：${p.color.label}が${issuer?.color?.label || '契約主'}の契約に参加しました。`);
