...
const {
  ...
  settleWinnerBets, settleChipWager, buildRanking
} = require('../src/engine');
...
const seedOffset = Number.parseInt(process.env.GOGI_STRESS_SEED_OFFSET || '0', 10);
if (!Number.isInteger(games) || games < 1 || games > 100000) {
  throw new Error('GOGI_STRESS_GAMES は1〜100000の整数にしてください。');
}
if (!Number.isSafeInteger(seedOffset) || seedOffset < 0 || seedOffset > 0xFFFFFFFF) {
  throw new Error('GOGI_STRESS_SEED_OFFSET は0〜4294967295の整数にしてください。');
}
...
function randomDraft(room, p, rng) {
  ...
  if (['cancel','specify'].includes(d.special) && others.length) d.specialTargetId = pick(rng, others).playerId;
  if (d.special === 'specify') d.specifiedType = pick(rng, Object.keys(NORMAL_CARDS));
  if (d.special === 'steal') {
    d.stealTargets = {};
    let remaining = 50;
    const shuffled = [...allOthers];
    for (let i = shuffled.length - 1; i > 0; i--) {
      const j = Math.floor(rng() * (i + 1));
      [shuffled[i], shuffled[j]] = [shuffled[j], shuffled[i]];
    }
    for (const target of shuffled) {
      if (remaining <= 0 || rng() < 0.35) continue;
      const amount = Math.max(1, Math.min(20, remaining, 1 + Math.floor(rng() * 20)));
      d.stealTargets[target.playerId] = amount;
      remaining -= amount;
    }
    if (!Object.keys(d.stealTargets).length && shuffled.length) d.stealTargets[shuffled[0].playerId] = 1;
  }
...
for (let game = 1; game <= games; game++) {
  const rng = rngFactory((0x9E3779B9 + seedOffset + game) >>> 0);
  const room = createRoom({ chipStake: pick(rng, [0,100,500,1000,5000]) });
  for (let i = 0; i < MAX_PLAYERS; i++) { const p = createPlayer(); p.chips = 100000; room.players.push(p); }
  startGame(room, rng);
  check(room);
  ...
  room.winnerBetResults = settleWinnerBets(room, preBetRanking);
  const finalRanking = buildRanking(room);
  room.finishedRanking = finalRanking;
  const chipSettlement = settleChipWager(room);
  assert.equal(chipSettlement.pot, room.chipStake * MAX_PLAYERS);
  assert.ok(chipSettlement.totalAwarded <= chipSettlement.pot + 0.001);
  assert.ok(chipSettlement.unawarded >= 0);
...
console.log(`${games} games x ${MAX_TURNS} turns + bonuses/bets/chips/final ranking: OK (seedOffset=${seedOffset}, ${Date.now() - started} ms)`);
