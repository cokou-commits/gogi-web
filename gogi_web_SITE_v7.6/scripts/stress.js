...
function check(room) {
  ...
  assert.equal(new Set(objectiveKeys).size, MAX_PLAYERS);
...
function randomDraft(room, p, rng) {
  ...
  if (['attack','scout','accusation'].includes(d.normal) && others.length) d.normalTargetId = pick(rng, others).playerId;
  ...
  if (d.special === 'double' && d.normal === 'scout' && others.length > 1 && rng() < 0.7) {
    d.secondNormalTargetId = pick(rng, others.filter(t => t.playerId !== d.normalTargetId)).playerId;
  }
  if (d.special === 'double' && d.normal === 'accusation' && others.length && rng() < 0.7) {
    d.secondNormalTargetId = pick(rng, others).playerId;
