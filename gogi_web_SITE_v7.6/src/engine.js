...
function resolveTurn(room) {
  ...
  // 同一対象へ2回攻撃しても攻撃成功+5Pはその対象につき1回分。別々の対象へ通れば各対象で成功判定する。
...
function recordStructuredStatement(room, p, payload) {
  const selfOnlyKinds = new Set(['points', 'hasSpecial', 'lastAction', 'nextAction']);
  const subject = getPlayer(room, payload.subjectId || p.playerId);
  if (!subject) return { ok: false, message: '対象が不正です。' };
  const kind = String(payload.kind || '');
  if (selfOnlyKinds.has(kind) && subject.playerId !== p.playerId) return { ok: false, message: 'その情報発言は自分についてのみ宣言できます。' };
  let truth = false;
  let deferred = false;
  let text = '';
