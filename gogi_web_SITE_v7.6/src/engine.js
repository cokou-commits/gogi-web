...
// 有効な対象が存在するのに対象/告発内容を選ばなかった場合、カードは消費し効果なし。
function draftForResolution(room, p) {
  ...
  // 対象や告発内容が欠けていても通常カード自体は消費される。
  // resolveTurn側は不正対象を無視するため、効果だけ不発となる。
  if (action.normalTargetId && !targetIsValid(room, p, action.normalTargetId)) action.normalTargetId = null;
  if (forced === 'accusation' && !OBJECTIVES.some(x => x.key === action.accusationGuess)) action.accusationGuess = null;
  return action;
}
...
module.exports = {
  ...
  buildRanking, recordStructuredStatement, canTransferPoints, awardPoints, randomRoomCode,
