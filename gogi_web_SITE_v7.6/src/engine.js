...
const {
  ...
  SECRET_REWARD, SCORING, STEAL_AMOUNTS, TURN_START_BONUSES, WINNER_BET, GOGI_CHIPS, ROOM_CODE_LENGTH, ROOM_CODE_ALPHABET,
...
// カード指定で固定された「通常カード＋対象」が成功した場合、成功得点は指定した側へ入る。
// 指定された本人の失敗減点（告発失敗など）は移さない。
// 2倍カード等で本人が追加した別対象分は、指定された対象とは別なので本人の得点とする。
function awardNormalSuccess(room, actor, normalType, targetId, base, reason, scoreEvents) {
  const forced = actor?.forcedNormalType;
  const isForcedTarget = !!(
    forced &&
    forced.turn === room.turn &&
    !forced.conflict &&
    forced.type === normalType &&
    forced.targetId === targetId &&
    Array.isArray(forced.rewardPlayerIds) &&
    forced.rewardPlayerIds.length
  );
  if (!isForcedTarget) return awardPoints(room, actor, base, reason, scoreEvents);

  let firstActual = 0;
  const seen = new Set();
  for (const playerId of forced.rewardPlayerIds) {
    if (seen.has(playerId)) continue;
    seen.add(playerId);
    const recipient = getPlayer(room, playerId);
    if (!recipient) continue;
    const actual = awardPoints(room, recipient, base, reason, scoreEvents);
    const event = scoreEvents?.[scoreEvents.length - 1];
    if (event) {
      event.sourcePlayerId = actor.playerId;
      event.viaCardSpecify = true;
      event.normalType = normalType;
      event.actionTargetId = targetId;
    }
    if (!firstActual) firstActual = actual;
  }
  return firstActual;
}
  ...
  if (p.forcedNormalType?.turn === room.turn) {
    if (p.forcedNormalType.conflict) {
      ...
      if ((p.hand[forced] || 0) <= 0) {
        ...
        d.normal = forced;
        if (p.forcedNormalType.targetId) d.normalTargetId = p.forcedNormalType.targetId;
      }
    }
  }
...
function resolveTurn(room) {
  ...
  for (const a of actions.values()) {
    ...
    if (a.special === 'specify') {
      ...
      if (target && target.alive && a.specifiedType && hasOwn(NORMAL_CARDS, a.specifiedType) && useTarget) {
        if ((target.hand[a.specifiedType] || 0) <= 0) {
          a.p.specials.specify = (a.p.specials.specify || 0) + 1;
          privateEvents.push({ to: a.p.playerId, type: 'specialResult', special: 'specify', success: false, text: `カード指定失敗：${target.color.label}は「${NORMAL_CARDS[a.specifiedType].label}」を持っていないため、カード指定が返却されました。` });
        } else {
  ...
  for (const [targetId, claims] of specifyClaims) {
    ...
    if (uniqueSpecs.length === 1) {
      ...
      for (const c of claims) privateEvents.push({ to: c.actor.playerId, type: 'specialResult', special: 'specify', success: true, text: `カード指定成功：${target.color.label}に「${NORMAL_CARDS[claim.type].label}」を${useTarget?.color?.label || '指定対象'}へ使用するよう指定しました。成功得点は指定者に入ります。` });
      privateEvents.push({ to: target.playerId, type: 'notice', text: `次ターンは「${NORMAL_CARDS[claim.type].label}」を${useTarget?.color?.label || '指定対象'}へ使用するよう指定されました。` });
    } else {
      target.forcedNormalType = { turn: room.turn + 1, type: null, targetId: null, conflict: true };
  ...
  for (const [targetId, claims] of stealClaims) {
    ...
    for (const claim of claims) {
      ...
      stealNotices.push(actual > 0
        ? { to: claim.actor.playerId, type: 'specialResult', special: 'steal', success: true, text: `ポイント泥棒成功：${target.color.label}から${actual}P奪いました。` }
        : { to: claim.actor.playerId, type: 'specialResult', special: 'steal', success: false, text: 'ポイント泥棒失敗：相手に奪えるポイントがありませんでした。' });
    }
    if (totalStolen > 0) {
  ...
  // 回復は攻撃より先に解決する。ターン開始時に生存している使用者は、自分または生存中の他プレイヤー1人を回復できる。
  // 先に回復してから同ターンの攻撃ダメージを受ける。2倍カード併用時は同じ対象を+4回復する。
  for (const a of actions.values()) {
    ...
    if (target.playerId !== a.p.playerId) {
      if (healed > 0) awardNormalSuccess(room, a.p, 'heal', target.playerId, SCORING.healOtherSuccess, '他人回復成功', scoreEvents);
  ...
  for (const [targetId, hits] of incoming.entries()) {
    ...
    if (!fullDefense.has(targetId) && blockedPower > 0 && claims.length) {
      ...
      for (const claim of orderedClaims) {
        ...
        awardNormalSuccess(room, claim.defender, 'defense', targetId, defenseScore, '防御成功', scoreEvents);
        remainingBlocked -= used;
      }
    }
    if (fullDefense.has(targetId)) {
      fullDefenseSucceeded.add(targetId);
    ...
    if (damage > 0) {
      ...
      for (const attacker of [...new Map(hits.map(h => [h.attacker.playerId, h.attacker])).values()]) {
        ...
        awardNormalSuccess(room, attacker, 'attack', targetId, SCORING.attackHit, '攻撃成功', scoreEvents);
      }
  ...
  for (const a of actions.values()) {
    ...
    const success = !canceled.has(a.p.playerId) && fullDefenseSucceeded.has(a.p.playerId);
    privateEvents.push({ to: a.p.playerId, type: 'specialResult', special: 'fullDefense', success, text: `完全防御${success ? '成功' : '失敗'}` });
  }
  for (const a of actions.values()) {
    if (a.special === 'specify' && canceled.has(a.p.playerId)) privateEvents.push({ to: a.p.playerId, type: 'specialResult', special: 'specify', success: false, text: 'カード指定失敗：無効カードで打ち消されました。' });
    if (a.special === 'steal' && canceled.has(a.p.playerId)) privateEvents.push({ to: a.p.playerId, type: 'specialResult', special: 'steal', success: false, text: 'ポイント泥棒失敗：無効カードで打ち消されました。' });
  }
  ...
  for (const attempt of accusationAttempts) {
    ...
    awardNormalSuccess(room, attempt.actor, 'accusation', attempt.target.playerId, SCORING.accusationSuccess, '告発成功', scoreEvents);
...
function settleChipWager(room) {
  ...
  const gamePool = Number((stake * multiplier).toFixed(2));
  const predictionPool = Number((stake * multiplier).toFixed(2));
  const finalRanking = Array.isArray(room.finishedRanking) ? room.finishedRanking : buildRanking(room);
  const gameWinnerIds = finalRanking.filter(row => row.rank === 1).map(row => row.playerId);
  const gameAwards = stake > 0 ? splitChipPool(gamePool, gameWinnerIds) : [];

  const hitResults = (room.winnerBetResults || []).filter(result => result?.hit && Number(result.payout || 0) > 0);
  const bestPredictionPayout = hitResults.length ? Math.max(...hitResults.map(result => Number(result.payout || 0))) : 0;
  const predictionWinnerIdsByBet = bestPredictionPayout > 0
    ? hitResults.filter(result => Number(result.payout || 0) === bestPredictionPayout).map(result => result.playerId)
    : [];
  // 1位予想の的中者がいない／誰も予想していない場合も賞金枠を余らせず、
  // 「第2順位発表（払戻し後の最終順位）」の1位へ予想賞金枠を回す。同率1位なら均等分配。
  const predictionFallbackToFinalWinner = predictionWinnerIdsByBet.length === 0;
  const predictionWinnerIds = predictionFallbackToFinalWinner ? [...gameWinnerIds] : predictionWinnerIdsByBet;
  const predictionAwards = stake > 0 ? splitChipPool(predictionPool, predictionWinnerIds) : [];
  ...
  for (const award of gameAwards) {
    ...
    row.gameAward = Number(award.amount.toFixed(2));
  }
  for (const award of predictionAwards) {
    const p = getPlayer(room, award.playerId);
    if (!p) continue;
    p.chips = Number((Number(p.chips || 0) + award.amount).toFixed(2));
    const row = byPlayer.get(p.playerId);
    row.predictionAward = Number(award.amount.toFixed(2));
  }
  for (const p of room.players) {
    const row = byPlayer.get(p.playerId);
    row.totalAward = Number((row.gameAward + row.predictionAward).toFixed(2));
    p.chipPayout = { ...row };
  }
  ...
  return {
    ...
    gamePool,
    predictionPool,
    bestPredictionPayout,
    gameWinnerIds,
    predictionWinnerIds,
    predictionFallbackToFinalWinner,
    totalAwarded,
