...
function resolveTurn(room) {
  ...
  // 同じ相手へ2人以上の有効なカード指定宣言が重なった場合は、指定内容が同じでも異なっていても全員失敗。
  // 競合時は指定対象本人が元々選んでいた通常行動をそのまま実行する。
  const specifyDeclarations = new Map();
  for (const a of actions.values()) {
    if (canceledSpecial.has(a.p.playerId) || a.special !== 'specify') continue;
    const target = getPlayer(room, a.specialTargetId);
    const useTarget = specifiedActionTargetIsValid(room, a.specialTargetId, a.specifiedType, a.specifiedTargetId);
    if (!target || !target.alive || !a.specifiedType || !hasOwn(NORMAL_CARDS, a.specifiedType) || !useTarget) continue;
    if (!specifyDeclarations.has(target.playerId)) specifyDeclarations.set(target.playerId, []);
    specifyDeclarations.get(target.playerId).push(a);
  }

  const specifyClaims = new Map();
  const specifyCollisionTargets = new Set();
  const stealClaims = [];
  for (const a of actions.values()) {
    ...
    if (a.special === 'specify') {
      ...
      if (target && target.alive && a.specifiedType && hasOwn(NORMAL_CARDS, a.specifiedType) && useTarget) {
        const declarations = specifyDeclarations.get(target.playerId) || [];
        if (declarations.length >= 2) {
          specifyCollisionTargets.add(target.playerId);
          privateEvents.push({
            to: a.p.playerId,
            type: 'specialResult',
            special: 'specify',
            success: false,
            text: `カード指定失敗：${target.color.label}へのカード指定が複数人で重なりました。`
          });
        } else {
  ...
  for (const targetId of specifyCollisionTargets) {
    const target = getPlayer(room, targetId);
    if (target) privateEvents.push({ to: target.playerId, type: 'notice', text: '複数人のカード指定が重なったため、カード指定はすべて失敗しました。元々選んだ通常行動を実行します。' });
  }

  for (const [targetId, claims] of specifyClaims) {
    const target = getPlayer(room, targetId);
    const targetAction = actions.get(targetId);
    if (!target || !targetAction || claims.length !== 1) continue;
    ...
    target.forcedNormalType = {
      ...
      conflict: false,
      rewardPlayerIds: [claim.actor.playerId]
    };
    ...
    const specifyBonus = awardPoints(room, claim.actor, SCORING.specifySuccess, 'カード指定成功', scoreEvents, { multiplyFinal:false });
    privateEvents.push({
      to: claim.actor.playerId,
      type: 'specialResult',
      special: 'specify',
      success: true,
