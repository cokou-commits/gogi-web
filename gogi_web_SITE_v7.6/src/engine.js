...
function validateDraft(room, p, draft, { strict = false } = {}) {
  ...
  if (!['attack', 'scout', 'accusation', 'heal'].includes(d.normal)) d.normalTargetId = null;
...
function draftForResolution(room, p) {
  ...
  const needsTarget = ['attack', 'scout', 'accusation', 'heal'].includes(forced);
  const validOthers = room.players.filter(x => x.alive && x.playerId !== p.playerId);
  if (['attack', 'scout', 'accusation'].includes(forced) && validOthers.length === 0) return emptyDraft();
...
function resolveTurn(room) {
  // v7.6は会話と行動選択を同一フェーズへ統合しているため、通常の解決元はchat。
  // 旧テスト/内部検証との後方互換のためactionも許可するが、result等からの二重解決は拒否する。
  if (room.status !== 'playing' || !['chat', 'action'].includes(room.phase)) throw new Error('ターン解決可能なフェーズではありません。');
  ...
  for (const [targetId, hits] of incoming.entries()) {
    ...
    if (blockedPower > 0 && defensePower.has(targetId)) {
      target.stats.defenseSuccessTurns++;
      awardPoints(room, target, SCORING.defenseSuccess, '防御成功', scoreEvents);
    }
