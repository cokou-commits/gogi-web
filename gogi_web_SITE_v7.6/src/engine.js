...
function validateDraft(room, p, draft, { strict = false } = {}) {
  ...
  // 防御は従来どおり自分が既定対象。必要なら他の生存者へ変更できる。
  if (d.normal === 'defense' && !d.normalTargetId) d.normalTargetId = p.playerId;
  ...
  const normalNeedsOtherTarget = ['attack', 'scout', 'accusation'].includes(d.normal);
  if (normalNeedsOtherTarget && !targetIsValid(room, p, d.normalTargetId)) {
