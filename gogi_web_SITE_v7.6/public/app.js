...
const CLIENT_BUILD_ID = '20260912-waitleaveracefix1';
...
async function emitAck(event, payload = {}, timeoutMs = 7000) {
  ...
  if (event !== 'resume' && event !== 'leaveRoom') {
...
async function leaveCurrentRoom() {
  ...
  // 明示退出を最優先にする。退出開始後に遅れて届くロビーstate/同期結果を受理すると
  // 復帰トークンと待機画面が復活するため、ACK送信前に復帰経路を遮断する。
  leavingLobby = true;
  ...
  // UIはタップ直後にホームへ戻す。サーバー退出はこの後に確実に完了させる。
  clearSessionToken(leavingToken);
  ...
  if (!leavingToken && !socket.connected) {
    // 待機中の切断はサーバー側で即時退出扱い。接続情報がない場合は再接続させない。
    disconnectIdleSocket();
    return;
  }

  // leaveRoomだけは既存のresume完了待ちに巻き込ませず、最優先で送る。
  let res = { ok:false, transient:true };
  try {
    res = await emitAck('leaveRoom', {
  ...
  // ACKが落ちても待機中Socketの切断はサーバー側で即時退出になるため、最後に必ず切断する。
  if (socket.connected) socket.disconnect();
  if (!res?.ok && !res?.alreadyLeft && res?.message) toast(res.message);
}
