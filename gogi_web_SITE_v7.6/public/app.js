...
async function sharePrivateRoom() {
  const code = state?.code || '';
  if (!code || state?.isPublic) return toast('プライベートルームの招待情報を取得できません。');
  const url = new URL(location.origin + location.pathname);
  url.hash = `room=${code}`;
  const inviteText = `五疑戦に参加：部屋コード ${code}
${url}`;
  // Web Share APIは必ずユーザーのタップ処理内で最初に呼ぶ。iOS Safariで共有シートが開かない
  // ケースを避けるためcanShare判定は挟まず、失敗時だけコピーへフォールバックする。
  if (typeof navigator.share === 'function') {
    try {
      await navigator.share({ title:'五疑戦', text:`部屋コード ${code}`, url:url.toString() });
      return;
    } catch (error) {
      if (error?.name === 'AbortError') return;
    }
  }
  if (await copyTextRobust(inviteText)) return toast('招待リンクをコピーしました。');
  toast(`部屋コード：${code}`);
}
...
function leaveCurrentRoom() {
  ...
  // 退出はネットワーク応答に依存させない。押した瞬間にローカル状態を破棄してホームへ戻す。
  // その後SocketとHTTPの2経路で退出を通知するため、片方が落ちても待機室に残りにくい。
  leavingLobby = true;
  ...
  setTimeout(() => {
    if (!state && !joining && socket.connected) socket.disconnect();
    leavingLobby = false;
  }, 300);
}
...
document.addEventListener('click', event => {
  ...
  const share = event.target?.closest?.('#shareRoom');
  if (share) {
    event.preventDefault();
    event.stopPropagation();
    sharePrivateRoom();
