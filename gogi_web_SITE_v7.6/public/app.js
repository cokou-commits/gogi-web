...
const CLIENT_BUILD_ID = '20260912-leavefix4';
...
async function prepareManualJoin() {
  if (lobbyLeavePromise) { try { await lobbyLeavePromise; } catch {} }
  ignoredLobbyRoomKey = '';
  leavingLobby = false;
...
function leaveCurrentRoom() {
  if (leavingLobby) return lobbyLeavePromise || Promise.resolve();
  const leavingToken = tabSessionToken || state?.resumeToken || storageGet(localStore, SESSION_KEY) || null;
  const roomKey = state?.roomId || state?.code || '';
  const shouldNotifyServer = state?.status === 'lobby';
  const leavePayload = {
    sessionToken: leavingToken || '',
    browserId,
    clientInstanceId,
    pageInstanceId,
    roomId: state?.roomId || '',
  ...
  // UIは即ホームへ戻す。ただしSocketはすぐ切らない。
  // 先にサーバーへ「退出完了」を確定させてから切断することで、
  // lobbyの参加記録/browserIdが残って次回「別の対戦に参加中」になるレースを防ぐ。
  leavingLobby = true;
  sessionWritesBlocked = true;
  ignoredLobbyRoomKey = roomKey;
  clearSessionToken(leavingToken);
  if (leavingToken && storageGet(localStore, SESSION_KEY) === leavingToken) storageRemove(localStore, SESSION_KEY);
  storageRemove(sessionStore, TAB_SESSION_KEY);
  tabSessionToken = null;
  ...
  lobbyLeavePromise = (async () => {
    if (!shouldNotifyServer) return { ok:true, skipped:true };

    // Socket ACKとHTTP keepaliveを並行送信。どちらか一方が落ちても、もう一方で退出を確定する。
    const socketLeave = socket.connected
      ? new Promise(resolve => {
          try {
            socket.timeout(1800).emit('leaveRoom', {}, (err, res) => {
              resolve(!err && res?.ok ? { ok:true, via:'socket' } : { ok:false, via:'socket' });
            });
          } catch { resolve({ ok:false, via:'socket' }); }
        })
      : Promise.resolve({ ok:false, via:'socket' });

    const httpLeave = fetch('/api/leave-lobby', {
      method:'POST',
      ...
      return response.ok && body?.ok ? { ok:true, via:'http' } : { ok:false, via:'http' };
    }).catch(() => ({ ok:false, via:'http' }));

    const results = await Promise.all([socketLeave, httpLeave]);
...
const leaveLobbyButton = $('#leaveLobby');
const activateLeaveLobby = event => {
  event?.preventDefault?.();
  event?.stopPropagation?.();
  leaveCurrentRoom();
};
// pointerupでclickより先に退出を確定。キーボード操作はclickでフォールバックする。
leaveLobbyButton?.addEventListener('pointerup', activateLeaveLobby, { passive:false });
leaveLobbyButton?.addEventListener('click', activateLeaveLobby);
// 待機室の主要ボタンはcapture段階でも拾う。モバイルSafariで別要素やアニメーションが
// 通常のbubble clickを妨げても、退出・招待だけは必ず実行される。
document.addEventListener('click', event => {
  const leave = event.target?.closest?.('#leaveLobby');
  if (leave) {
    event.preventDefault();
    event.stopPropagation();
    leaveCurrentRoom();
  ...
  if (me.forcedNormalType?.turn === state.turn) {
    ...
    banner.textContent = me.forcedConflict
      ...
      : me.forcedUnavailable
        ...
        : `カード指定中：通常カードは「${cardLabel(me.forcedNormalType.type)}」しか使えません。特殊カードは併用できます。`;
