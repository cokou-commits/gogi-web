...
function startBgm() {
  if (bgmTimer || !audioPrefs.bgm || !audioUnlocked || state?.status !== 'playing') return;
  bgmStep = 0;
  bgmTick();
  bgmTimer = setInterval(bgmTick, 690);
}
function stopBgm() {
...
function renderChatNotice() {
  ...
  else if (privateCount) el.textContent = `個別チャットに新着 ${privateCount}件`;
