...
function rechargeChipWallet() {
  if (Number(chipWallet.balance || 0) !== 0) {
    toast('五戯チップは所持0のときだけチャージできます。');
    return false;
  }
  chipWallet.balance = CHIP_RECHARGE_AMOUNT;
  saveChipWallet();
  renderChipWallet();
...
const CLIENT_BUILD_ID = '20260913-healfirst-afk-chipfallback1-cptest';
...
const ASSET_REV = '20260912decisive1';
const CARD_VISUALS = {
...
async function startPublic() {
  ...
  if (chipStake == null) return;
  await prepareManualJoin();
  joining = true;
  manualJoinInFlight = true;
  let res;
  try { res = await emitAck('publicMatch', { clientInstanceId, browserId, pageInstanceId, chipStake, chipBalance:chipWallet.balance }); }
...
async function createPrivate() {
  ...
  manualJoinInFlight = true;
  let res;
  try { res = await emitAck('createPrivate', { clientInstanceId, browserId, pageInstanceId, chipStake, chipBalance:chipWallet.balance }); }
  finally { joining = false; manualJoinInFlight = false; }
  if (!res.ok) { toast(res.message); disconnectIdleSocket(); return; }
  saveJoinResult(res);
  if (state) render(); else show('#lobby');
...
async function joinPrivate() {
  ...
  manualJoinInFlight = true;
  let res;
  try { res = await emitAck('joinPrivate', { code, clientInstanceId, browserId, pageInstanceId, chipBalance:chipWallet.balance }); }
  finally { joining = false; manualJoinInFlight = false; }
  if (!res.ok) { toast(res.message); disconnectIdleSocket(); return; }
  saveJoinResult(res);
  clearInviteQuery();
...
function renderTrade() {
  ...
  function makeCardBundleControls(prefix, ownedOnly) {
    ...
    const kind = makeSelect([['none','0枚（カードなし）'],['normal','通常カード1枚'],['special','特殊カード1枚']], ['none','normal','special'].includes(tradeUi[kindKey]) ? tradeUi[kindKey] : 'none', value => { tradeUi[kindKey]=value; rebuild(); }, disabled);
    ...
    function rebuild() {
      ...
      tradeUi[typeKey]=type.value; type.disabled = disabled;
    }
  ...
  const exchangeNote = document.createElement('div'); exchangeNote.className='microNote'; exchangeNote.textContent='交換提案の件数制限なし。同じ相手へ複数件、複数の相手へ同時に提案できます。双方ともポイント0P・カード0枚を選択可能。片側だけ資産を出す提案もでき、相手が承認すれば無償提供として成立します。両側とも0P・0枚だけの空交換は不可。提案に出した資産は成立・拒否・取消・期限切れまで保留されます。ターン消費なし。'; exchangeBlock.appendChild(exchangeNote);
...
function renderPublicContracts() {
  ...
  const type=makeSelect([
    ...
    ['dontAttackIssuer',`${contractTiming}、${myColor}を攻撃しなければ報酬`],
    ['defendIssuer',`${contractTiming}、${myColor}を防御したら報酬`],
  ...
  const contractTargets = type.value === 'accuseTarget' ? accusationPlayerOptions(false) : otherAlivePlayers().map(p=>[p.playerId,colorLabel(p)]);
  ...
  post.addEventListener('click', async()=>{
    ...
    if(!res.ok){toast(res.message);post.disabled=false;} else { const perPlayer=(n/4)*(state.turn>=state.maxTurns?2:1); toast(`公開契約を提示しました。報酬1人${perPlayer}P`); }
  });
  creator.append(type, subject, reward, rewardPreview, post);
  const note=document.createElement('div'); note.className='microNote'; note.textContent='公開契約は提示と同時に、条件上参加可能な全員が自動参加します。提示者と参加者の色は公開されません。件数制限なし。同じターンに複数件、同時に複数件提示できます。使用Pは20P刻み。通常は20P使用→1人5P、40P使用→1人10P。第15ターンは報酬2倍。未達成分は提示者へ返却。ターン消費なし。'; creator.appendChild(note);
