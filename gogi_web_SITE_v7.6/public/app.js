...
function requestLobbySync() {
  if (leavingLobby || state?.status !== 'lobby' || !socket.connected) return;
  socket.emit('requestState', {});
}
function ensureLobbySync() {
  if (lobbySyncHandle) return;
  lobbySyncHandle = setInterval(requestLobbySync, 2500);
}
...
async function sharePrivateRoom() {
  if (!state?.code || state.isPublic) return toast('プライベートルームの招待情報を取得できません。');
  const url = new URL(location.origin + location.pathname);
  // ルームコードはURLフラグメントへ入れ、HTTP Refererやサーバーログへ送らない。
  url.hash = `room=${state.code}`;
  const inviteText = `五疑戦に参加：部屋コード ${state.code}\n${url}`;
  const shareData = { title:'五疑戦', text:inviteText };
  if (navigator.share) {
    try {
  ...
  // 共有API・Clipboard APIの両方が使えないブラウザでもコード自体は必ず見せる。
...
async function leaveCurrentRoom() {
  const leavingToken = tabSessionToken || state?.resumeToken || null;
  const shouldNotifyServer = state?.status !== 'finished';

  // iPhone/Safariでもボタンを押した瞬間に必ずホームへ戻す。
  // ACK待ちで画面を拘束すると、通信揺らぎ時に「待機をやめる」が反応しないように見えるため、
  // ローカルUIを先に確定し、サーバー退出は短い猶予で後処理する。
  leavingLobby = true;
  clearSessionToken(leavingToken);
  stopTimer();
  stopLobbySync();
  ...
  if (shouldNotifyServer && socket.connected) {
    // UIはすでにホーム。resume待ちを挟まず、現在のSocketから退出だけを直接送る。
    // ACKが欠落しても最大1.5秒で切断し、サーバー側のロビー切断清掃へ移行する。
    await new Promise(resolve => {
      try { socket.timeout(1500).emit('leaveRoom', {}, () => resolve()); }
      catch { resolve(); }
    });
  }
  disconnectIdleSocket();
...
socket.on('state', next => {
  ...
  if (leavingLobby && next.status === 'lobby') return;
