...
function accusationPlayerOptions(includeBlank = true) {
  const arr = includeBlank ? [['', '告発対象を選択']] : [];
  for (const p of state?.players || []) {
...
function renderActionComposer(container) {
  ...
  if (['attack','defense','scout','accusation','heal'].includes(draft.normal)) {
    ...
    const targetOptions = draft.normal === 'heal' ? healPlayerOptions() : draft.normal === 'defense' ? defensePlayerOptions() : draft.normal === 'accusation' ? accusationPlayerOptions() : playerOptions();
  ...
  if (draft.special === 'double' && ['attack','scout','accusation'].includes(draft.normal)) {
    ...
    target.appendChild(makeSelect(draft.normal === 'accusation' ? accusationPlayerOptions() : playerOptions(), draft.secondNormalTargetId, value => updateDraft({ secondNormalTargetId: value || null }), disabled));
