...
const contractUi = { conditionType:'attackTarget', subjectId:'', reward:'5' };
const winnerBetUi = { targetId:'', amount:'5' };
let draftUpdateChain = Promise.resolve();
let operationCounter = 0;
let sessionReadyPromise = Promise.resolve();
let chatSending = false;
let chatNoticeState = { global:0, private:0, latestKind:'', latestFrom:'' };
const AUDIO_BGM_KEY = 'gogi.audio.bgm.v1';
const AUDIO_SFX_KEY = 'gogi.audio.sfx.v1';
const audioPrefs = {
  bgm: storageGet(localStore, AUDIO_BGM_KEY) !== 'off',
...
function primeBgmMediaFromGesture() {
  const media = bgmMedia();
  if (!media) return Promise.resolve(false);
  media.volume = 0.34;
  // iOS/Safariは、同じHTMLAudioElementをユーザー操作中に一度playしておくと
  // 以後のターン遷移でも再生を継続しやすい。play()はawaitより先に呼ぶ。
  const shouldKeepPlaying = audioPrefs.bgm && state?.status === 'playing';
  let playPromise;
  try { playPromise = media.play(); } catch { return Promise.resolve(false); }
  return Promise.resolve(playPromise).then(() => {
    if (!shouldKeepPlaying) { media.pause(); try { media.currentTime = 0; } catch {} }
    return true;
  }).catch(() => false);
}
async function unlockAudio() {
  try { if (navigator?.audioSession && 'type' in navigator.audioSession) navigator.audioSession.type = 'playback'; } catch {}
  const mediaPromise = primeBgmMediaFromGesture();
  const ctx = createAudioContext();
  if (!ctx) {
    audioUnlocked = await mediaPromise;
    syncBgmState();
    return audioUnlocked;
  }
  try {
    // Safariではresumeだけでは無音のままのケースがあるため、短い無音バッファを同じ操作内で開始する。
  ...
  const mediaOk = await mediaPromise;
  audioUnlocked = audioUnlocked || mediaOk;
  syncBgmState();
  return audioUnlocked;
}
function setAudioPref(kind, enabled) {
  audioPrefs[kind] = !!enabled;
  storageSet(localStore, kind === 'bgm' ? AUDIO_BGM_KEY : AUDIO_SFX_KEY, enabled ? 'on' : 'off');
  renderAudioControls();
  if (kind === 'bgm') syncBgmState();
  else if (enabled) playSfx('toggle');
}
function renderAudioControls() {
  const bgm = $('#bgmToggle');
  const sfx = $('#sfxToggle');
  if (bgm) {
    bgm.textContent = audioPrefs.bgm ? 'BGM ON' : 'BGM OFF';
    bgm.setAttribute('aria-pressed', String(audioPrefs.bgm));
  }
  if (sfx) {
...
function tone(freq, duration = 0.12, { gain = 0.08, type = 'sine', when = 0, target = 'sfx', detune = 0 } = {}) {
  if (!audioUnlocked || !audioContext) return;
  const bus = target === 'bgm' ? bgmGain : sfxGain;
  if (!bus) return;
...
function playSfx(kind) {
  ...
  switch (kind) {
    ...
    case 'chatGlobal': tone(740,.09,{gain:.045,type:'sine'}); tone(930,.10,{gain:.038,when:.07}); break;
    case 'chatPrivate': tone(880,.10,{gain:.05,type:'triangle'}); tone(1180,.13,{gain:.045,when:.07}); break;
    case 'card': tone(360,.06,{gain:.035,type:'triangle'}); tone(520,.07,{gain:.03,when:.04}); break;
    case 'pointsUp': tone(660,.08,{gain:.04}); tone(990,.13,{gain:.045,when:.06}); break;
    case 'pointsDown': tone(420,.09,{gain:.04,type:'triangle'}); tone(260,.15,{gain:.04,when:.06}); break;
    case 'heal': tone(520,.12,{gain:.04}); tone(780,.16,{gain:.045,when:.08}); break;
    case 'damage': noiseBurst(.075,.025); tone(110,.16,{gain:.07,type:'sawtooth'}); break;
    case 'success': tone(523.25,.10,{gain:.045}); tone(659.25,.10,{gain:.042,when:.07}); tone(783.99,.16,{gain:.04,when:.14}); break;
    case 'fail': tone(330,.11,{gain:.045,type:'triangle'}); tone(220,.18,{gain:.055,when:.08,type:'sawtooth'}); break;
    case 'turn': tone(392,.10,{gain:.035}); tone(523.25,.15,{gain:.04,when:.08}); break;
    case 'result': tone(196,.13,{gain:.045,type:'triangle'}); tone(293.66,.18,{gain:.04,when:.08}); break;
    case 'eliminate': noiseBurst(.12,.03); tone(82.4,.34,{gain:.075,type:'sawtooth'}); break;
    case 'objective': tone(523.25,.08,{gain:.04}); tone(659.25,.08,{gain:.04,when:.06}); tone(880,.22,{gain:.055,when:.13}); break;
    case 'finish': tone(261.63,.16,{gain:.045}); tone(329.63,.16,{gain:.04,when:.12}); tone(392,.16,{gain:.04,when:.24}); tone(523.25,.34,{gain:.05,when:.36}); break;
  }
}
function bgmVoice(freq, duration = 0.3, { gain = 0.05, type = 'sawtooth', when = 0, cutoff = 900, resonance = 1.2, detune = 0, pan = 0 } = {}) {
  if (!audioPrefs.bgm || !audioUnlocked || !audioContext || !bgmGain) return;
  const t = audioContext.currentTime + Math.max(0, when);
  const osc = audioContext.createOscillator();
  ...
  osc.detune.setValueAtTime(detune, t);
  filter.type = 'lowpass';
  filter.frequency.setValueAtTime(Math.max(80, cutoff), t);
  filter.Q.setValueAtTime(Math.max(0.1, resonance), t);
  env.gain.setValueAtTime(0.0001, t);
  env.gain.exponentialRampToValueAtTime(Math.max(0.0002, gain), t + 0.018);
  env.gain.exponentialRampToValueAtTime(0.0001, t + Math.max(0.05, duration));
  osc.connect(filter); filter.connect(env);
  if (panner) { panner.pan.setValueAtTime(Math.max(-1, Math.min(1, pan)), t); env.connect(panner); panner.connect(bgmGain); }
  else env.connect(bgmGain);
  osc.start(t); osc.stop(t + Math.max(0.06, duration) + 0.05);
}
function bgmAir(when = 0, gain = 0.012, pan = 0) {
  if (!audioPrefs.bgm || !audioUnlocked || !audioContext || !bgmGain) return;
  ...
  const panner = typeof audioContext.createStereoPanner === 'function' ? audioContext.createStereoPanner() : null;
  source.buffer = buffer;
  filter.type = 'highpass'; filter.frequency.setValueAtTime(3600, t); filter.Q.setValueAtTime(0.8, t);
  env.gain.setValueAtTime(Math.max(0.0001, gain), t);
...
function bgmTick() {
  if (!audioPrefs.bgm || !audioUnlocked || !audioContext || state?.status !== 'playing') return;
  // 五疑戦専用：暗いネオン感のあるオリジナル16ステップ。
  // 外部曲の旋律・サンプルは使用せず、低音ドローンと脈動だけで心理戦の緊張感を作る。
  const sub = [55.00,55.00,65.41,49.00,82.41,73.42,65.41,49.00,55.00,82.41,87.31,49.00,73.42,65.41,82.41,49.00];
  const pulse = [220.00,261.63,329.63,293.66,246.94,329.63,293.66,196.00,220.00,329.63,349.23,293.66,246.94,261.63,329.63,196.00];
  const i = bgmStep++ % sub.length;
  const turn = Math.max(1, Number(state?.turn || 1));
  const intensity = 1 + Math.min(0.30, (turn - 1) * 0.022);

  // 常に薄く鳴るサブベース。偶数ステップを少し長くして「脈」を作る。
  bgmVoice(sub[i], i % 2 === 0 ? 0.62 : 0.40, { gain:0.105 * intensity, type:'sine', cutoff:170, resonance:0.7 });

  // 中域パルス。左右をわずかに振り、チャットを邪魔しない短い音価に限定。
  if (i % 2 === 0) {
    bgmVoice(pulse[i], 0.25, { gain:0.036 * intensity, type:'sawtooth', cutoff:620 + (i % 4) * 110, resonance:2.4, pan:i % 4 === 0 ? -0.18 : 0.18 });
    bgmVoice(pulse[i] / 2, 0.34, { gain:0.020 * intensity, type:'triangle', cutoff:430, resonance:1.4, detune:-7 });
  }

  // 4拍ごとの薄い高音。旋律ではなく空気感だけを付加。
  if ([3,7,11,15].includes(i)) {
    const high = pulse[(i + 5) % pulse.length] * 2;
    bgmVoice(high, 0.18, { gain:0.010 * intensity, type:'sine', cutoff:1800, resonance:1.1, pan:i === 7 || i === 15 ? 0.35 : -0.35 });
    bgmAir(0.03, 0.0065 * intensity, i === 7 || i === 15 ? 0.28 : -0.28);
  }

  // 終盤は控えめな裏拍を追加して焦燥感だけ上げる。
  if (turn >= 11 && i % 4 === 2) bgmAir(0.18, 0.0045 * intensity, i % 8 === 2 ? -0.2 : 0.2);
}
function startBgm() {
  if (!audioPrefs.bgm || !audioUnlocked || state?.status !== 'playing') return;
  const media = bgmMedia();
  if (media) {
    media.volume = 0.34;
    const p = media.play();
  ...
  // HTMLAudioが使えない環境だけWeb Audio生成BGMへフォールバック。
  if (bgmTimer) return;
  bgmStep = 0; bgmTick(); bgmTimer = setInterval(bgmTick, 690);
}
function stopBgm() {
...
function syncBgmState() {
  if (audioPrefs.bgm && audioUnlocked && state?.status === 'playing') startBgm();
  else stopBgm();
}
...
function handleStateAudio(previousSnapshot, next) {
  ...
  if (!current) { lastAudioSnapshot = current; syncBgmState(); return; }
...
function resetChatNotice() {
  chatNoticeState = { global:0, private:0, latestKind:'', latestFrom:'' };
  renderChatNotice();
}
function registerIncomingChatNotice(message) {
  if (!message || isOwnMessage(message)) return;
  const kind = message.toId ? 'private' : 'global';
  chatNoticeState[kind] = Number(chatNoticeState[kind] || 0) + 1;
  chatNoticeState.latestKind = kind;
...
function renderChatNotice() {
  ...
  const privateCount = Number(chatNoticeState.private || 0);
  ...
  if (globalCount && privateCount) el.textContent = `新着：全体${globalCount}件 / 個別${privateCount}件`;
  else if (privateCount) {
    const latest = chatNoticeState.latestFrom ? `${chatNoticeState.latestFrom}個別チャット` : '個別チャット';
    el.textContent = `${latest}に新着 ${privateCount}件`;
  }
  else el.textContent = `全体チャットに新着 ${globalCount}件`;
}
function clearChatNoticeIfAtBottom() {
  const box = $('#messages');
  if (!box) return;
  const distance = box.scrollHeight - box.scrollTop - box.clientHeight;
  if (distance < 24) resetChatNotice();
}
...
$('#showBetSettlement')?.addEventListener('click', () => { finalRevealAdvanced = true; $('#preBetStage')?.classList.add('finalStageDone'); $('#showBetSettlement')?.classList.add('hidden'); $('#postBetStage')?.classList.remove('hidden'); });
...
$('#bgmToggle')?.addEventListener('click', async e => {
  e.stopPropagation();
  await unlockAudio();
  setAudioPref('bgm', !audioPrefs.bgm);
});
$('#sfxToggle')?.addEventListener('click', async e => {
  e.stopPropagation();
  await unlockAudio();
  setAudioPref('sfx', !audioPrefs.sfx);
});
...
document.addEventListener('visibilitychange', () => {
  ...
  else { if (audioContext?.state === 'suspended') audioContext.resume().catch(() => {}); syncBgmState(); }
});
window.addEventListener('pageshow', () => { if (audioContext?.state === 'suspended') audioContext.resume().catch(() => {}); syncBgmState(); });
document.addEventListener('click', e => {
  if (!audioPrefs.sfx || !audioUnlocked) return;
  const target = e.target?.closest?.('button,.gameCard,.cardChoice,.cardTile');
  if (!target || target.id === 'bgmToggle' || target.id === 'sfxToggle') return;
...
function appendMessage(message, autoScroll = true) {
  ...
  const prefix = message.toId ? `【${privateChatLabel(message)}】 ` : '';
...
function renderPublicContracts() {
  ...
  if (!contracts.length) {
    ...
    for (const c of contracts) {
      ...
      const condition = c.conditionType === 'attackTarget' ? `第${c.dueTurn}Tに${c.subjectColor}を攻撃` : `第${c.dueTurn}Tに${c.issuerColor}を攻撃しない`;
      const text=document.createElement('span'); text.textContent=`${c.issuerColor}：${condition} / 報酬${c.reward}P${c.acceptorColor ? ` / 受諾：${c.acceptorColor}` : ''}`;
      row.appendChild(text);
      if (c.status === 'open' && c.issuerId !== me.playerId) {
        const accept=document.createElement('button'); accept.type='button'; accept.className='ghost'; accept.textContent='受ける'; accept.disabled=state.phase!=='chat' || me.ready || !me.alive || c.createdTurn!==state.turn;
  ...
  const type=makeSelect([['attackTarget','次ターン、指定相手を攻撃したら報酬'],['dontAttackIssuer','次ターン、契約主を攻撃しなければ報酬']], contractUi.conditionType, value=>{contractUi.conditionType=value; renderPublicContracts();}, !canPost);
  const targetOptions=[['','攻撃対象を選択'],...otherAlivePlayers().map(p=>[p.playerId,colorLabel(p)])];
  const subject=makeSelect(targetOptions, targetOptions.some(([v])=>v===contractUi.subjectId)?contractUi.subjectId:'', value=>{contractUi.subjectId=value;}, !canPost || type.value!=='attackTarget');
  if (type.value !== 'attackTarget') subject.classList.add('hidden');
  const reward=document.createElement('input'); reward.type='number'; reward.inputMode='numeric'; reward.min='5'; reward.step='5'; reward.max=String(Math.max(5,me.points)); reward.value=(Number(contractUi.reward)>=5 && Number(contractUi.reward)%5===0)?String(contractUi.reward):(me.points>=5?'5':''); reward.placeholder='報酬 5P刻み'; reward.disabled=!canPost || me.points<5; reward.addEventListener('input',()=>{contractUi.reward=reward.value;});
  const post=document.createElement('button'); post.type='button'; post.className='ghost'; post.textContent='公開契約を出す'; post.disabled=!canPost || me.points<5;
  post.addEventListener('click', async()=>{
    const n=Number(reward.value); if(!Number.isInteger(n)||n<5||n%5!==0||n>state.me.points) return toast('報酬は所持P以内の5P刻みで指定してください。');
    if(type.value==='attackTarget' && !subject.value) return toast('攻撃対象を選択してください。');
  ...
  const note=document.createElement('div'); note.className='microNote'; note.textContent='公開契約は全員に条件・報酬・受諾者が見えます。報酬は提示時に預け、次ターン終了時に自動判定。未成立・未達なら契約主へ返却。ターン消費なし。'; creator.appendChild(note);
...
function renderRanking() {
  stopTimer();
  appendRankingRows($('#preBetRanking'), state.preBetRanking || state.finishedRanking || []);
  appendRankingRows($('#ranking'), state.finishedRanking || []);
  $('#showBetSettlement')?.classList.toggle('hidden', finalRevealAdvanced);
  $('#postBetStage')?.classList.toggle('hidden', !finalRevealAdvanced);
