...
function healPlayerOptions(includeBlank = true) {
  const arr = includeBlank ? [['', '回復対象を選択']] : [];
...
function renderActionComposer(container, finalMode) {
  ...
  if (['attack','scout','accusation','heal'].includes(draft.normal)) {
    const targetBlock = document.createElement('div');
    targetBlock.className = 'fieldGrid';
    const label = document.createElement('label');
    label.innerHTML = `<span>${draft.normal === 'heal' ? '回復する対象' : '通常カードの対象'}</span>`;
    const targetOptions = draft.normal === 'heal' ? healPlayerOptions() : playerOptions();
...
function updateDraft(patch) {
  ...
  draftUpdateChain = draftUpdateChain.catch(() => {}).then(async () => {
    ...
    // カード変更時に不要な付随選択を掃除する。
    if ('normal' in patch) {
      if (!['attack','scout','accusation','heal'].includes(next.normal)) next.normalTargetId = null;
...
function renderPublicContracts() {
  ...
  const note=document.createElement('div'); note.className='microNote'; note.textContent='25P刻みで預けます。25Pなら達成者1人5P、50Pなら1人10P。複数人参加可。未参加・未達成分など余ったポイントは契約主へ返却。ターン消費なし。'; creator.appendChild(note);
