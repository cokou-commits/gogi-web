...
const CHIP_WALLET_KEY = 'gogi.chipWallet.v2';
const INITIAL_CHIP_BALANCE = 100000;
const CHIP_RECHARGE_AMOUNT = 100000;
const CHIP_STAKE_STEP = 100;
...
function loadChipWallet() {
  try {
    const raw = storageGet(localStore, CHIP_WALLET_KEY);
    if (!raw) return { balance:INITIAL_CHIP_BALANCE };
    const parsed = JSON.parse(raw);
    const balance = Number(parsed?.balance);
    return { balance:Number.isFinite(balance) && balance >= 0 ? Math.round(balance * 100) / 100 : INITIAL_CHIP_BALANCE };
  } catch {
    return { balance:INITIAL_CHIP_BALANCE };
  }
}
let chipWallet = loadChipWallet();
function saveChipWallet() {
  chipWallet.balance = Math.max(0, Math.round(Number(chipWallet.balance || 0) * 100) / 100);
  storageSet(localStore, CHIP_WALLET_KEY, JSON.stringify(chipWallet));
}
function formatChips(value) {
  const n = Number(value || 0);
  return Number.isInteger(n)
    ? n.toLocaleString('ja-JP')
    : n.toLocaleString('ja-JP', { minimumFractionDigits:0, maximumFractionDigits:2 });
}
async function refreshChipWallet() {
  const res = await emitAck('getChipWallet', { browserId });
  if (!res?.ok) return res || { ok:false, message:'五戯チップ情報を取得できません。' };
  syncChipWalletBalance(res.chipBalance);
  return res;
}
async function rechargeChipWallet() {
  // ローカル値は表示キャッシュのみ。残高変更は必ずサーバー側で判定する。
  const res = await emitAck('rechargeChipWallet', { browserId });
  if (!res?.ok) {
    if (Number.isFinite(Number(res?.chipBalance))) syncChipWalletBalance(res.chipBalance);
    toast(res?.message || '五戯チップをチャージできませんでした。');
    return false;
  }
  syncChipWalletBalance(res.chipBalance);
  toast(`五戯チップ +${formatChips(CHIP_RECHARGE_AMOUNT)}`);
  disconnectIdleSocket();
  return true;
}
function syncChipWalletBalance(value) {
  const n = Number(value);
  if (!Number.isFinite(n) || n < 0) return;
  chipWallet.balance = Math.round(n * 100) / 100;
  saveChipWallet();
  renderChipWallet();
}
function renderChipWallet() {
  const balance = $('#chipBalance');
  if (balance) balance.textContent = formatChips(chipWallet.balance);
  const recharge = $('#chipRecharge');
  if (recharge) {
    const canRecharge = Number(chipWallet.balance || 0) === 0;
    recharge.classList.toggle('hidden', !canRecharge);
    recharge.disabled = !canRecharge;
  }
  const input = $('#chipStakeInput');
  if (input) {
    input.max = String(Math.floor(Number(chipWallet.balance || 0) / CHIP_STAKE_STEP) * CHIP_STAKE_STEP);
    const n = Number(input.value || 0);
    if (!Number.isFinite(n) || n < 0) input.value = '0';
  }
}
function selectedChipStake() {
  const input = $('#chipStakeInput');
  const value = Number(input?.value || 0);
  if (!Number.isInteger(value) || value < 0 || value % CHIP_STAKE_STEP !== 0) {
    toast('五戯チップは0または100刻みで指定してください。');
    return null;
  }
  if (value > Number(chipWallet.balance || 0)) {
    toast(`五戯チップが不足しています。所持${formatChips(chipWallet.balance)}`);
    return null;
  }
...
let clockOffsetMs = 0;
const tradeUi = { exchangeTarget:'', offerPoints:'0', offerKind:'none', offerType:'', offerCount:'1', requestPoints:'0', requestKind:'none', requestType:'', requestCount:'1' };
...
let dismissedAfkWarningKey = '';
let lobbySyncHandle = null;
let roomCodeComposing = false;
let lobbyCountOverride = null;
let lobbySyncInFlight = false;
let ignoredLobbyRoomKey = '';
let sessionWritesBlocked = false;
let lobbyLeavePromise = null;
const CLIENT_BUILD_ID = '20260914-complete-final33-game-card-art-current-rules';
...
function showGameNotice(event) {
  ...
  // 公開契約・交換は準備OK中やツール画面表示中でも見落とさないよう、固定通知に加えてトーストも出す。
  if (event.kind === 'contract' || event.kind === 'exchange') toast(event.text);
...
function stopLobbySync() {
  if (lobbySyncHandle) clearInterval(lobbySyncHandle);
  lobbySyncHandle = null;
}
async function requestLobbySync() {
  if (leavingLobby || state?.status !== 'lobby' || lobbySyncInFlight) return;
  const roomKey = state.roomId || state.code || '';
  const code = state.code || '';
  if (!roomKey || roomKey === ignoredLobbyRoomKey) return;
  lobbySyncInFlight = true;
  try {
    if (socket.connected) {
      socket.timeout(1800).emit('requestState', {}, (err, res) => {
        if (err || !res?.ok) {
          // Socket自体は接続中でもサーバー上のプレイヤー紐付けが外れている場合がある。
          // 復帰トークンが残っていれば再紐付けを試す。
          if (resumeTokenCandidate() && !joining && !leavingLobby) sessionReadyPromise = tryResume();
        }
      });
    }
    if (code) {
      const response = await fetch(`/api/lobby/${encodeURIComponent(code)}?build=${encodeURIComponent(CLIENT_BUILD_ID)}`, {
        cache:'no-store', headers:{ 'Accept':'application/json' }
      });
      if (response.ok) {
        const info = await response.json();
        if (state?.status === 'lobby' && (state.roomId || state.code) === roomKey && roomKey !== ignoredLobbyRoomKey && info?.ok) {
          lobbyCountOverride = Number.isInteger(info.count) ? info.count : lobbyCountOverride;
          if (state && Number.isInteger(info.cpuReady) && Number.isInteger(info.humanCount)) state.lobbyCpuConsent = { ready:info.cpuReady, humans:info.humanCount };
          if (info.status === 'lobby') renderLobby();
          else if (info.status === 'playing' && resumeTokenCandidate() && !joining) {
            sessionReadyPromise = tryResume();
            socket.emit('requestState', {});
          }
        }
      }
    }
  } catch {} finally { lobbySyncInFlight = false; }
}
function ensureLobbySync() {
  if (lobbySyncHandle) return;
  requestLobbySync();
  lobbySyncHandle = setInterval(requestLobbySync, 2000);
}
...
function setSession(token, { forceShared = false } = {}) {
  // 明示退出後は、遅れて完了したresume/stateが復帰トークンを書き戻すのを禁止する。
  if (token && sessionWritesBlocked) return;
...
function resumeTokenCandidate() {
  if (sessionWritesBlocked || leavingLobby) return null;
...
async function emitAck(event, payload = {}, timeoutMs = 7000) {
  ...
  // 待機退出は再接続時のresume完了待ちに依存させない。
  // 旧・正常動作版と同じく、leaveRoomはSocket接続後すぐ送信する。
  if (!['resume','leaveRoom'].includes(event)) {
...
const ASSET_REV = '20260914-complete-final33-game-card-art-current-rules';
const CARD_VISUALS = {
  attack: { image: `/cards/attack.webp?v=${ASSET_REV}`, effect: '相手1人に1ダメージ' },
  defense: { image: `/cards/defense.webp?v=${ASSET_REV}`, effect: '対象1人への攻撃を1ダメージ防ぐ / 成功+10P' },
  scout: { image: `/cards/scout.webp?v=${ASSET_REV}`, effect: '相手1人のHP・キル数・通常カードを確認 / 成功+10P' },
  accusation: { image: `/cards/accusation.webp?v=${ASSET_REV}`, effect: '秘密目標を告発。成功+30P / 失敗-15P' },
  heal: { image: `/cards/heal.webp?v=${ASSET_REV}`, effect: '対象1人のHPを2回復 / 自分成功+10P / 他人成功+20P' },
  fullDefense: { image: `/cards/fullDefense.webp?v=${ASSET_REV}`, effect: '対象1人への攻撃をすべて防ぐ / 成功+20P' },
  cancel: { image: `/cards/cancel.webp?v=${ASSET_REV}`, effect: '相手1人の通常カードまたは特殊カードを選んで無効化。止めたカードの購入価格相当Pを獲得' },
  specify: { image: `/cards/specify.webp?v=${ASSET_REV}`, effect: '相手のこのターンの通常カード・対象を指定 / 成功+20P。指定行動の成功P・公開契約Pは相手本人' },
  steal: { image: `/cards/steal.webp?v=${ASSET_REV}`, effect: '合計最大40P・1人最大20Pを自由配分して奪う' },
  double: { image: `/cards/double.webp?v=${ASSET_REV}`, effect: '通常カードを2回使う。成功Pが2倍になるのは2回目だけ' }
};
...
function privateInviteInfo() {
  const buttonCode = $('#shareRoom')?.dataset?.roomCode || '';
  const code = normalizeRoomCodeValue(buttonCode || state?.code || '');
  if (!code || state?.isPublic) return null;
  const url = new URL(location.origin + location.pathname);
  // 招待コードはフラグメントに置き、リンク先サーバーや外部サイトへのRefererに含めない。
  url.hash = `room=${code}`;
  return {
    code,
    url: url.toString(),
    text: `五疑戦に参加：部屋コード ${code}\n${url}`
  };
}
function openInviteFallback(info = privateInviteInfo()) {
  if (!info) return toast('プライベートルームの招待情報を取得できません。');
  const dialog = $('#inviteDialog');
  const code = $('#inviteRoomCode');
  const link = $('#inviteRoomLink');
  if (code) code.textContent = info.code;
  if (link) link.value = info.url;
  if (!dialog) return toast(`招待リンク：${info.url}`);
  if (typeof dialog.showModal === 'function') {
    if (!dialog.open) dialog.showModal();
  } else dialog.setAttribute('open', '');
}
function closeInviteFallback() {
  const dialog = $('#inviteDialog');
  if (!dialog) return;
  if (dialog.open && typeof dialog.close === 'function') dialog.close();
  else dialog.removeAttribute('open');
}
async function sharePrivateRoom(event) {
  // Web Shareは「友達を招待」ボタン自身のclickから直接呼ぶ。
  // documentのcapture経由にしないことでiOS Safariのuser activationを確実に保持する。
  const info = privateInviteInfo();
  if (!info) return toast('プライベートルームの招待情報を取得できません。');
  if (typeof navigator.share === 'function') {
    try {
      // URLフィールド非対応のWebViewもあるため、URLはtextへ含める。iOS/Android双方で共有可能。
      await navigator.share({ title:'五疑戦', text:info.text });
      return;
    } catch (error) {
      if (error?.name === 'AbortError') return;
    }
  }
  // 共有APIがない/失敗したPC・WebViewでは、必ず目に見える招待画面を出す。
  openInviteFallback(info);
}
async function sharePrivateRoomFromFallback() {
  const info = privateInviteInfo();
  if (!info) return toast('プライベートルームの招待情報を取得できません。');
  if (typeof navigator.share === 'function') {
    try {
  ...
  if (await copyTextRobust(info.text)) return toast('招待リンクをコピーしました。');
  toast(`部屋コード：${info.code}`);
}
...
async function cleanupOwnStaleLobby() {
  // 直前の退出通知が通信競合で欠落していても、新規参加前に同一タブの古い待機枠を必ず掃除する。
  try {
    const response = await fetch('/api/leave-lobby', {
      method:'POST',
      headers:{ 'Content-Type':'application/json', 'Accept':'application/json' },
      cache:'no-store',
      keepalive:true,
      body:JSON.stringify({ browserId, clientInstanceId, pageInstanceId })
    });
    return response.ok;
  } catch { return false; }
}
async function prepareManualJoin() {
  if (lobbyLeavePromise) {
    try { await lobbyLeavePromise; } catch {}
    lobbyLeavePromise = null;
  }
  // 「待機をやめる」直後の即再参加でもサーバー側の古い待機記録を残さない。
  await cleanupOwnStaleLobby();
  ignoredLobbyRoomKey = '';
  leavingLobby = false;
  sessionWritesBlocked = false;
  lobbyCountOverride = null;
  stopLobbySync();
}

async function startPublic() {
  if (joining) return;
  await prepareManualJoin();
  const walletRes = await refreshChipWallet();
  if (!walletRes.ok) { toast(walletRes.message || '五戯チップ情報を取得できません。'); disconnectIdleSocket(); return; }
  const chipStake = selectedChipStake();
  if (chipStake == null) { disconnectIdleSocket(); return; }
  joining = true;
  manualJoinInFlight = true;
  let res;
  try { res = await emitAck('publicMatch', { clientInstanceId, browserId, pageInstanceId, chipStake }); }
  finally { joining = false; manualJoinInFlight = false; }
  if (!res.ok) { if (Number.isFinite(Number(res.chipBalance))) syncChipWalletBalance(res.chipBalance); toast(res.message); disconnectIdleSocket(); return; }
  if (Number.isFinite(Number(res.chipBalance))) syncChipWalletBalance(res.chipBalance);
  saveJoinResult(res);
...
async function createPrivate() {
  ...
  manualJoinInFlight = true;
  let res;
  try { res = await emitAck('createPrivate', { clientInstanceId, browserId, pageInstanceId, chipStake }); }
  finally { joining = false; manualJoinInFlight = false; }
  if (!res.ok) { if (Number.isFinite(Number(res.chipBalance))) syncChipWalletBalance(res.chipBalance); toast(res.message); disconnectIdleSocket(); return; }
  if (Number.isFinite(Number(res.chipBalance))) syncChipWalletBalance(res.chipBalance);
  saveJoinResult(res);
...
async function joinPrivate() {
  const input = $('#codeInput');
  const code = normalizeRoomCodeValue(input?.value || '');
  if (input) input.value = code;
...
async function fillWithCpu() {
  ...
  const button = $('#fillCpu');
  if (button) button.disabled = true;
  const res = await emitAck('fillWithCpu', {});
  if (!res.ok) { if (button) button.disabled = false; return toast(res.message || 'CP補充の開始同意に失敗しました。'); }
  if (res.waiting) return toast(`CP補充でプレイ：${res.readyCount}/${res.humanCount}人が同意済み`);
  if (res.added > 0) toast(`全員同意。CPを${res.added}人補充して開始します。`);
}

function returnHomeFromFinish() {
  const leavingToken = tabSessionToken || state?.resumeToken || null;
  // 終了画面では通信処理を一切待たない。iPhone/Safariでもタップ直後に必ずホームへ戻す。
  try {
...
async function leaveCurrentRoom() {
  ...
  const leavingToken = tabSessionToken || state?.resumeToken || null;
  const leaveButton = $('#leaveLobby');
  const originalLabel = leaveButton?.textContent || '待機をやめる';
  leavingRoom = true;
  if (leaveButton) { leaveButton.disabled = true; leaveButton.textContent = '退出中…'; }
  try {
    // 終了画面では既にSocketを解放しているため、再接続してleaveRoomする必要はない。
    // 待機室ではsessionTokenも渡し、Socketの紐付けが一瞬外れていても
    // サーバー側で本人の待機レコードを確実に削除できるようにする。
    if (state?.status !== 'finished') {
      const res = await emitAck('leaveRoom', { sessionToken: leavingToken }, 3500);
      if (!res.ok) {
        if (res.transient) {
          // 通信断でもUIは待機室へ閉じ込めない。Socket切断によりサーバー側の
          // ロビー待機レコードは再接続猶予後に自動削除される。
          clearSessionToken(leavingToken);
          stopTimer();
          state = null;
          lastMessageIds.clear();
    ...
    leavingRoom = false;
    if (leaveButton) { leaveButton.disabled = false; leaveButton.textContent = originalLabel; }
  }
}
...
const shareRoomButton = $('#shareRoom');
// 招待はdocumentのcaptureではなくボタン自身のclickへ直結する。
shareRoomButton?.addEventListener('click', sharePrivateRoom);
$('#closeInvite')?.addEventListener('click', closeInviteFallback);
$('#inviteDialog')?.addEventListener('click', event => { if (event.target === event.currentTarget) closeInviteFallback(); });
$('#inviteNativeShare')?.addEventListener('click', sharePrivateRoomFromFallback);
...
function normalizeRoomCodeValue(value) {
  return String(value || '').normalize('NFKC').toUpperCase().replace(/[^ABCDEFGHJKLMNPQRSTUVWXYZ23456789]/g, '').slice(0, 8);
}
const codeInput = $('#codeInput');
codeInput?.addEventListener('compositionstart', () => { roomCodeComposing = true; });
codeInput?.addEventListener('compositionend', e => {
  roomCodeComposing = false;
  e.target.value = normalizeRoomCodeValue(e.target.value);
});
codeInput?.addEventListener('input', e => {
  // 日本語12キー/ローマ字IMEの変換途中でvalueを書き換えるとiOSの入力状態が壊れる。
  if (roomCodeComposing || e.isComposing || e.inputType === 'insertCompositionText') return;
  e.target.value = normalizeRoomCodeValue(e.target.value);
});
codeInput?.addEventListener('paste', () => setTimeout(() => {
  if (codeInput && !roomCodeComposing) codeInput.value = normalizeRoomCodeValue(codeInput.value);
}, 0));
codeInput?.addEventListener('keydown', e => { if (e.key === 'Enter' && !roomCodeComposing && !e.isComposing) joinPrivate(); });
$('#backHome')?.addEventListener('click', returnHomeFromFinish);
...
document.addEventListener('visibilitychange', () => {
  ...
  if (!document.hidden) requestLobbySync();
});
window.addEventListener('pageshow', () => { if (audioContext?.state === 'suspended') audioContext.resume().catch(() => {}); requestLobbySync(); });
...
socket.on('state', next => {
  ...
  // 退出ボタン直後に遅れて届いたロビーstateで画面が待機室へ戻るレースを防ぐ。
  const nextRoomKey = next.roomId || next.code || '';
  if (next.status === 'lobby' && (leavingLobby || (ignoredLobbyRoomKey && nextRoomKey === ignoredLobbyRoomKey))) return;
  ...
  if (!sameRoom) { renderedChatKey = ''; renderedScoutKey = ''; lastMessageIds.clear(); lastPhaseNavigationSeq = null; finalRevealAdvanced = false; resetChatNotice(); }
  ...
  if (next?.me && Number.isFinite(Number(next.me.chips))) syncChipWalletBalance(next.me.chips);
  state = next;
  if (next.status === 'lobby') lobbyCountOverride = Array.isArray(next.players) ? next.players.length : lobbyCountOverride;
  else lobbyCountOverride = null;
  handleStateAudio(previousAudioSnapshot, next);
  renderAudioControls();
  render();
});
socket.on('lobbyStatus', info => {
  if (!info || state?.status !== 'lobby') return;
  const roomKey = state.roomId || state.code || '';
  const infoKey = info.roomId || info.code || '';
  if (!roomKey || roomKey === ignoredLobbyRoomKey || (infoKey && infoKey !== roomKey && info.code !== state.code)) return;
  if (Number.isInteger(info.count)) lobbyCountOverride = info.count;
  if (state && Number.isInteger(info.cpuReady) && Number.isInteger(info.humanCount)) state.lobbyCpuConsent = { ready:info.cpuReady, humans:info.humanCount };
  if (info.status === 'lobby') renderLobby();
  else if (info.status === 'playing') requestLobbySync();
});
...
function renderLobby() {
  ...
  preloadCardArt();
  const count = Number.isInteger(lobbyCountOverride) ? lobbyCountOverride : state.players.length;
  $('#lobbyTitle').textContent = state.isPublic ? '対戦相手を探しています' : 'プライベートルームで待機中';
  const consent = state.lobbyCpuConsent || { ready:0, humans:state.players.filter(p => !p.isCpu).length };
  const humanCount = Math.max(1, Number(consent.humans || 0));
  const consentReady = Math.max(0, Number(consent.ready || 0));
  $('#lobbyNote').textContent = count >= 5 ? `${count}/5人参加中。開始します。` : `${count}/5人参加中。CP補充は参加中の人間全員の同意で開始します。`;
  const fillCpuBtn = $('#fillCpu');
  if (fillCpuBtn) {
    const mineReady = !!state.me?.cpuFillReady;
    fillCpuBtn.disabled = count >= 5 || mineReady;
    fillCpuBtn.textContent = count >= 5 ? '5人揃いました' : mineReady ? `CP補充でプレイ 同意済み ${consentReady}/${humanCount}` : `CP補充でプレイ ${consentReady}/${humanCount}`;
  }
  const stake = Number(state.chipStake || 0);
  const stakeEl = $('#lobbyStake');
  if (stakeEl) stakeEl.textContent = stake > 0 ? `賭け額：${formatChips(stake)} 五戯チップ` : '賭けなし（0 五戯チップ）';
  const copy = $('#copyCode');
  const share = $('#shareRoom');
  if (state.isPublic) {
    copy.classList.add('hidden');
    share?.classList.add('hidden');
    if (share) delete share.dataset.roomCode;
  } else {
    copy.classList.remove('hidden');
    share?.classList.remove('hidden');
    if (share) share.dataset.roomCode = state.code || '';
...
function renderMe() {
  ...
  $('#myChips').textContent = formatChips(me.chips);
  ...
  $('#mobileChips').textContent = formatChips(me.chips);
  ...
  if (afkBanner) {
    const warningActive = me.alive && Number(me.afkStreak || 0) >= 2;
    const warningKey = warningActive ? `${state.roomId || ''}:${state.turn || 0}:${Number(me.afkStreak || 0)}` : '';
    afkBanner.dataset.warningKey = warningKey;
    afkBanner.classList.toggle('hidden', !warningActive || dismissedAfkWarningKey === warningKey);
  }
...
function renderPhase() {
  ...
  if (isChat) {
    ...
    renderActionComposer($('#draftComposer'));
  }
  if (isResult) {
...
function scoutPlayerOptions(includeBlank = true) {
  const arr = includeBlank ? [['', '対象を選択']] : [];
  for (const p of state?.players || []) {
    if (!p?.playerId || p.playerId === state?.me?.playerId) continue;
    arr.push([p.playerId, `${colorLabel(p)}${p.alive ? '' : '（脱落）'}`]);
  }
  return arr;
}
function accusationPlayerOptions(includeBlank = true) {
  const arr = includeBlank ? [['', '対象を選択']] : [];
  for (const p of state?.players || []) {
    if (!p?.playerId || p.playerId === state?.me?.playerId || p.accusationResolved) continue;
    arr.push([p.playerId, `${colorLabel(p)}${p.alive ? '' : '（脱落）'}`]);
  }
  return arr;
}
function publicContractTargetOptions(conditionType) {
  const accusation = conditionType === 'accuseTarget';
  const arr = [];
  for (const p of state?.players || []) {
    if (!p?.playerId) continue;
    if (accusation) {
      if (p.accusationResolved) continue;
    } else if (!p.alive) {
      continue;
    }
    const own = p.playerId === state?.me?.playerId ? '（自分）' : '';
    const eliminated = !p.alive ? '（脱落）' : '';
    arr.push([p.playerId, `${colorLabel(p)}${own}${eliminated}`]);
  }
  return arr;
}
...
function healPlayerOptions(includeBlank = true) { return supportPlayerOptions(includeBlank, '対象を選択'); }
function defensePlayerOptions(includeBlank = true) { return supportPlayerOptions(includeBlank, '対象を選択'); }
...
function specifiedActionTargetOptions(forcedPlayerId, type, includeBlank = true) {
  const arr = includeBlank ? [['', '使用対象を選択']] : [];
  for (const p of state?.players || []) {
    if (!p?.playerId) continue;
    if (['attack','scout','accusation'].includes(type) && p.playerId === forcedPlayerId) continue;
    if (['attack','defense','heal'].includes(type) && !p.alive) continue;
    arr.push([p.playerId, `${colorLabel(p)}${p.alive ? '' : '（脱落）'}`]);
  }
  return arr;
}

function renderActionComposer(container) {
  if (!container || !state.me) return;
  container.replaceChildren();
  const me = state.me;
  const draft = { ...me.draft };
  const disabled = !me.alive || me.ready;

  if (me.forcedNormalType?.turn === state.turn) {
    const banner = document.createElement('div');
    banner.className = `forcedBanner${me.forcedUnavailable || me.forcedTargetInvalid || me.forcedConflict ? ' danger' : ''}`;
    banner.textContent = me.forcedConflict
      ...
      : me.forcedUnavailable
        ...
        : me.forcedTargetInvalid
          ? `カード指定：指定された対象が無効になったため、このターンは行動なし（指定カードは消費しません）`
          : me.forcedNormalType.targetId
          ? `カード指定中：「${cardLabel(me.forcedNormalType.type)}」を${colorLabel(state.players?.find(p => p.playerId === me.forcedNormalType.targetId))}へ使用します。特殊カードは併用できます。`
          : `カード指定中：「${cardLabel(me.forcedNormalType.type)}」を使用します。特殊カードは併用できます。`;
  ...
  for (const [key, def] of Object.entries(state.normalCards || {})) {
    ...
    const button = createVisualCardButton({
      ...
      disabled: disabled || me.forcedUnavailable || me.forcedTargetInvalid || me.forcedConflict || count <= 0 || (!!forcedKey && forcedKey !== key)
    });
  ...
  clearNormal.disabled = disabled || !!forcedKey || me.forcedTargetInvalid || me.forcedConflict || !draft.normal;
  ...
  if (['attack','defense','scout','accusation','heal'].includes(draft.normal)) {
    ...
    const targetLabel = '対象を選択';
    label.innerHTML = `<span>${targetLabel}</span>`;
    const targetOptions = draft.normal === 'heal' ? healPlayerOptions() : draft.normal === 'defense' ? defensePlayerOptions() : draft.normal === 'scout' ? scoutPlayerOptions() : draft.normal === 'accusation' ? accusationPlayerOptions() : playerOptions();
  ...
  for (const [key, def] of Object.entries(state.specialCards || {})) {
    ...
    const button = createVisualCardButton({
      ...
      disabled: disabled || me.forcedUnavailable || me.forcedTargetInvalid || me.forcedConflict || count <= 0 || (key === 'double' && !draft.normal)
    });
  ...
  clearSpecial.disabled = disabled || me.forcedUnavailable || me.forcedTargetInvalid || me.forcedConflict || !draft.special;
  clearSpecial.addEventListener('click', () => updateDraft({ special: null, specialTargetId: null, cancelKind: null, specifiedType: null, specifiedTargetId: null }));
  section2.appendChild(clearSpecial);
  container.appendChild(section2);

  if (['cancel','specify','fullDefense'].includes(draft.special)) {
    const specialFields = document.createElement('div');
    specialFields.className = 'fieldGrid';
    const target = document.createElement('label');
    const targetLabel = draft.special === 'fullDefense' ? '守る対象を選択' : '対象を選択';
    target.innerHTML = `<span>${targetLabel}</span>`;
    const targetOptions = draft.special === 'fullDefense' ? defensePlayerOptions() : playerOptions();
    target.appendChild(makeSelect(targetOptions, draft.specialTargetId, value => updateDraft({ specialTargetId: value || null, ...(draft.special === 'specify' ? { specifiedTargetId: null } : {}) }), disabled));
    specialFields.appendChild(target);
    if (draft.special === 'cancel') {
      const kind = document.createElement('label');
      kind.innerHTML = '<span>無効化する種類</span>';
      kind.appendChild(makeSelect([['','種類を選択'], ['normal','通常カード無効'], ['special','特殊カード無効']], draft.cancelKind, value => updateDraft({ cancelKind: value || null }), disabled));
      specialFields.appendChild(kind);
    }
    if (draft.special === 'specify') {
      const type = document.createElement('label');
      type.innerHTML = '<span>このターンに指定するカード</span>';
      type.appendChild(makeSelect(normalOptions(), draft.specifiedType, value => updateDraft({ specifiedType: value || null, specifiedTargetId: null }), disabled));
      specialFields.appendChild(type);
      const useTarget = document.createElement('label');
      useTarget.innerHTML = '<span>そのカードを使う相手</span>';
      useTarget.appendChild(makeSelect(specifiedActionTargetOptions(draft.specialTargetId, draft.specifiedType), draft.specifiedTargetId, value => updateDraft({ specifiedTargetId: value || null }), disabled || !draft.specialTargetId || !draft.specifiedType));
      specialFields.appendChild(useTarget);
    }
    container.appendChild(specialFields);
  }

  if (draft.special === 'steal') {
    const rules = state.rules?.stealRules || { maxTotal:50, maxPerTarget:20 };
    const specialFields = document.createElement('div');
    specialFields.className = 'fieldGrid stealAllocationGrid';
    const currentTargets = { ...(draft.stealTargets || {}) };
    const opponents = (state.players || []).filter(p => p.playerId !== state.me.playerId);
    const totalNow = () => Object.values(currentTargets).reduce((sum,n) => sum + (Number(n) || 0), 0);
    for (const player of opponents) {
      const label = document.createElement('label');
      label.innerHTML = `<span>${colorLabel(player)}から奪うP${player.alive ? '' : '（脱落）'}</span>`;
      const input = document.createElement('input');
      input.type = 'number'; input.min = '0'; input.max = String(rules.maxPerTarget || 20); input.step = '1'; input.inputMode = 'numeric';
      input.value = String(Number(currentTargets[player.playerId] || 0));
      input.disabled = disabled;
      input.addEventListener('change', () => {
        const n = Math.max(0, Math.min(Number(rules.maxPerTarget || 20), Math.trunc(Number(input.value) || 0)));
        const next = { ...currentTargets };
        if (n > 0) next[player.playerId] = n; else delete next[player.playerId];
        const total = Object.values(next).reduce((sum,v) => sum + Number(v || 0), 0);
        if (total > Number(rules.maxTotal || 50)) {
          input.value = String(Number(currentTargets[player.playerId] || 0));
          return toast(`ポイント泥棒は合計${rules.maxTotal || 50}Pまでです。`);
        }
        updateDraft({ stealTargets: next, specialTargetId:null });
      });
      label.appendChild(input);
      specialFields.appendChild(label);
    }
    const total = document.createElement('div'); total.className = 'draftSummary';
    total.textContent = `指定合計 ${totalNow()} / ${rules.maxTotal || 50}P（1人最大${rules.maxPerTarget || 20}P）`;
    container.appendChild(specialFields); container.appendChild(total);
  }

  if (draft.special === 'double' && ['attack','defense','scout','accusation','heal'].includes(draft.normal)) {
    const second = document.createElement('div');
    second.className = 'fieldGrid doubleFields';
    const target = document.createElement('label');
    target.innerHTML = '<span>対象を選択（2回目・任意）</span>';
    target.appendChild(makeSelect(draft.normal === 'accusation' ? accusationPlayerOptions() : draft.normal === 'heal' ? healPlayerOptions() : draft.normal === 'defense' ? defensePlayerOptions() : draft.normal === 'scout' ? scoutPlayerOptions() : playerOptions(), draft.secondNormalTargetId, value => updateDraft({ secondNormalTargetId: value || null }), disabled));
...
function updateDraft(patch) {
  ...
  draftUpdateChain = draftUpdateChain.catch(() => {}).then(async () => {
    ...
    if ('normal' in patch) {
      ...
      if (!['attack','defense','scout','accusation','heal'].includes(next.normal)) {
    ...
    if ('special' in patch) {
      if (!['cancel','specify','steal','fullDefense'].includes(next.special)) next.specialTargetId = null;
      if (next.special !== 'cancel') next.cancelKind = null;
      else if (!['normal','special'].includes(next.cancelKind)) next.cancelKind = 'normal';
      if (next.special !== 'specify') { next.specifiedType = null; next.specifiedTargetId = null; }
      if (next.special !== 'steal') next.stealTargets = {};
      if (next.special === 'fullDefense' && !next.specialTargetId) next.specialTargetId = state.me.playerId;
    ...
    if (res.draft && state?.me && state.phaseSeq === requestedPhaseSeq) {
      ...
      if (state.phase === 'chat') renderActionComposer($('#draftComposer'));
    }
  });
...
function renderInventory() {
  ...
  for (const [key, def] of Object.entries(state.normalCards || {})) {
    ...
    buy.disabled = state.phase !== 'chat' || !me.alive || me.ready || normalInsufficient;
  ...
  const specifiedSpecialPrice = Number(state.rules?.specialSpecifiedPurchasePrice ?? 50);
  for (const [key, def] of Object.entries(state.specialCards || {})) {
    const row = document.createElement('div'); row.className = 'shopRow specialSpecifiedRow';
    const text = document.createElement('span'); text.textContent = def.label;
    const exactBuy = document.createElement('button'); exactBuy.type = 'button'; exactBuy.textContent = `${specifiedSpecialPrice}P 指定`;
    const exactInsufficient = me.points < specifiedSpecialPrice;
    exactBuy.disabled = state.phase !== 'chat' || !me.alive || me.ready || exactInsufficient;
    if (exactInsufficient) exactBuy.textContent = 'ポイント不足';
    exactBuy.addEventListener('click', async () => {
      exactBuy.disabled = true;
      const res = await emitMutation('buy', { type: 'special', specialType: key, phaseSeq: state.phaseSeq });
      if (!res.ok) { toast(res.message); exactBuy.disabled = false; }
    });
    row.append(text, exactBuy); box.appendChild(row);
  }
  ...
  const specialPrice = Number(state.rules?.specialPurchasePrice ?? 40);
  ...
  buy.disabled = state.phase !== 'chat' || !me.alive || me.ready || specialInsufficient;
  ...
  note.textContent = `特殊カード購入：ランダム${specialPrice}P / 指定${specifiedSpecialPrice}P。所持ポイントの範囲で回数制限なし`;
...
function renderTrade() {
  ...
  const bundleText = bundle => {
    ...
    if (bundle?.card) parts.push(`「${bundle.card.cardLabel || (bundle.card.special ? specialLabel(bundle.card.type) : cardLabel(bundle.card.type))}」${Math.max(1, Number(bundle.card.quantity || 1))}枚`);
  ...
  const exchangeTarget = makeSelect(targetOptions, exchangeTargetInitial, value => { tradeUi.exchangeTarget = value; }, disabled);

  const protectedForcedType = me.forcedNormalType?.turn === state.turn && !me.forcedNormalType?.conflict && !me.forcedTargetInvalid ? me.forcedNormalType.type : null;
  ...
  function makeCardBundleControls(prefix, ownedOnly) {
    ...
    points.placeholder='ポイント 0P〜（5P刻み）'; points.disabled = disabled;
    points.addEventListener('input', () => { tradeUi[`${prefix}Points`] = points.value; });
    const kindKey = `${prefix}Kind`, typeKey = `${prefix}Type`;
    const kind = makeSelect([['none','0枚（カードなし）'],['normal','通常カード'],['special','特殊カード']], ['none','normal','special'].includes(tradeUi[kindKey]) ? tradeUi[kindKey] : 'none', value => { tradeUi[kindKey]=value; rebuild(); }, disabled);
    const type = document.createElement('select'); type.disabled = disabled;
    const countKey = `${prefix}Count`;
    const count = document.createElement('input'); count.type='number'; count.inputMode='numeric'; count.min='1'; count.step='1'; count.max='99'; count.placeholder='カード枚数';
    const offerAvailable = () => {
      if (!ownedOnly || !type.value) return 99;
      if (kind.value === 'normal') {
        const held = Number(me.hand[type.value] || 0);
        return Math.max(0, held - (type.value === protectedForcedType ? 1 : 0));
      }
      if (kind.value === 'special') return Math.max(0, Number(me.specials[type.value] || 0));
      return 0;
    };
    function syncCount() {
      if (kind.value === 'none' || !type.value) {
        count.value='0'; count.disabled=true; tradeUi[countKey]='1'; return;
      }
      const max = ownedOnly ? offerAvailable() : 99;
      count.max = String(Math.max(1, max));
      let current = Number(tradeUi[countKey] || 1);
      if (!Number.isInteger(current) || current < 1) current = 1;
      if (ownedOnly) current = Math.min(current, Math.max(1, max));
      count.value=String(current);
      count.disabled = disabled || max <= 0;
      tradeUi[countKey]=count.value;
    }
    function rebuild() {
      type.replaceChildren();
      const source = kind.value === 'normal' ? (ownedOnly ? ownedNormal : allNormal) : kind.value === 'special' ? (ownedOnly ? ownedSpecial : allSpecial) : [];
      if (kind.value === 'none') {
        const o=document.createElement('option'); o.value=''; o.textContent='0枚（カードなし）'; type.appendChild(o); type.disabled=true; tradeUi[typeKey]=''; syncCount(); return;
      }
      if (!source.length) {
        const o=document.createElement('option'); o.value=''; o.textContent=ownedOnly ? '出せるカードなし' : '0枚（カードなし）'; type.appendChild(o); type.disabled=true; tradeUi[typeKey]=''; syncCount(); return;
      }
      ...
      tradeUi[typeKey]=type.value; type.disabled = disabled; syncCount();
    }
    type.addEventListener('change', () => { tradeUi[typeKey]=type.value; syncCount(); });
    count.addEventListener('input', () => { tradeUi[countKey]=count.value; });
    rebuild(); wrap.append(points, kind, type, count); return { wrap, points, kind, type, count };
  }
  const offerControls = makeCardBundleControls('offer', true);
  const requestControls = makeCardBundleControls('request', false);
  const sendExchange = document.createElement('button'); sendExchange.type='button'; sendExchange.className='ghost'; sendExchange.textContent = '交換を提案'; sendExchange.disabled = disabled;
  sendExchange.addEventListener('click', async () => {
    ...
    const offerCardCount = offerCardType ? Number(offerControls.count.value || 0) : 0;
    const requestCardCount = requestCardType ? Number(requestControls.count.value || 0) : 0;
    ...
    if (![offerCardCount, requestCardCount].every(n => Number.isInteger(n) && n >= 0 && n <= 99)) return toast('カード枚数は1〜99枚で指定してください。');
    if (offerCardType && offerCardCount < 1) return toast('交換に出すカード枚数を指定してください。');
    if (requestCardType && requestCardCount < 1) return toast('相手に求めるカード枚数を指定してください。');
    ...
    tradeUi.exchangeTarget=exchangeTarget.value; tradeUi.offerPoints=offerControls.points.value; tradeUi.requestPoints=requestControls.points.value; tradeUi.offerCount=offerControls.count.value; tradeUi.requestCount=requestControls.count.value;
    ...
    else if (res.accepted) toast('CPが交換を承認しました。');
    else if (res.rejected) toast('CPが交換を拒否しました。');
    else toast('交換を提案しました。');
  });
  exchangeBlock.append(exchangeTarget, offerControls.wrap, requestControls.wrap, sendExchange);
  const exchangeNote = document.createElement('div'); exchangeNote.className='microNote'; exchangeNote.textContent='交換提案の件数制限なし。同じ相手へ複数件、複数の相手へ同時に提案できます。双方ともポイント0P・カード0枚を選択可能。カードを選ぶ場合は1〜99枚で枚数指定可能。片側だけ資産を出す提案もでき、相手が承認すれば無償提供として成立します。両側とも0P・0枚だけの空交換は不可。提案に出した資産は成立・拒否・取消・期限切れまで保留されます。ターン消費なし。'; exchangeBlock.appendChild(exchangeNote);
...
function renderPublicContracts() {
  ...
  if (!contracts.length) {
    ...
    for (const c of contracts) {
      ...
      const condition = c.conditionType === 'attackTarget'
        ? `このターン、${c.targetColor}を攻撃したら報酬`
        : c.conditionType === 'dontAttackTarget'
          ? `このターン、${c.targetColor}を攻撃しなければ報酬`
          : c.conditionType === 'defendTarget'
            ? `このターン、${c.targetColor}を防御したら報酬`
            : c.conditionType === 'healTarget'
              ? `このターン、${c.targetColor}に回復カードを使ったら報酬`
              : `このターン、${c.targetColor}を告発したら報酬（成否不問）`;
      const rewardLabel = `報酬：1人${c.rewardPerPlayer}P${Number(c.rewardMultiplier)===2?'（最終ターン2倍）':''}`;
      const text=document.createElement('span'); text.textContent=`【${rewardLabel}】 ${condition}`;
  ...
  const canPost = state.status==='playing' && state.phase==='chat' && me.alive && !me.ready && state.turn <= state.maxTurns;
  const creator=document.createElement('div'); creator.className='contractCreator';
  const contractTiming = 'このターン';
  const type=makeSelect([
    ['attackTarget',`${contractTiming}、指定した色を攻撃したら報酬`],
    ['dontAttackTarget',`${contractTiming}、指定した色を攻撃しなければ報酬`],
    ['defendTarget',`${contractTiming}、指定した色を防御したら報酬`],
    ['healTarget',`${contractTiming}、指定した色に回復カードを使ったら報酬`],
    ['accuseTarget',`${contractTiming}、指定した色を告発したら報酬（成功・失敗どちらでも可）`]
  ], contractUi.conditionType, value=>{contractUi.conditionType=value; renderPublicContracts();}, !canPost);
  const needsSubject = true;
  const contractTargets = publicContractTargetOptions(type.value);
  const targetPlaceholder = type.value === 'accuseTarget' ? '告発する色を選択'
    : type.value === 'attackTarget' ? '攻撃する色を選択'
    : type.value === 'dontAttackTarget' ? '攻撃しない色を選択'
    : type.value === 'defendTarget' ? '防御する色を選択'
    : '回復する色を選択';
  const targetOptions=[['',targetPlaceholder],...contractTargets];
  ...
  const reward=document.createElement('input'); reward.type='number'; reward.inputMode='numeric'; reward.min='20'; reward.step='20'; reward.max=String(Math.max(20,me.points)); reward.value=(Number(contractUi.reward)>=20 && Number(contractUi.reward)%20===0)?String(contractUi.reward):(me.points>=20?'20':''); reward.placeholder='使用P（20P刻み）'; reward.disabled=!canPost || me.points<20;
  const rewardPreview=document.createElement('div'); rewardPreview.className='microNote';
  const refreshRewardPreview=()=>{
    const n=Number(reward.value);
    const perPlayer=Number.isInteger(n)&&n>=20&&n%20===0 ? (n/4)*(state.turn>=state.maxTurns?2:1) : 0;
    rewardPreview.textContent=perPlayer>0 ? `この契約の達成報酬：1人${perPlayer}P${state.turn>=state.maxTurns?'（第15ターン2倍）':''}` : '使用Pを入力すると1人あたりの報酬を表示します。';
  };
  reward.addEventListener('input',()=>{contractUi.reward=reward.value;refreshRewardPreview();});
  ...
  post.addEventListener('click', async()=>{
    const n=Number(reward.value); if(!Number.isInteger(n)||n<20||n%20!==0||n>state.me.points) return toast('使用Pは所持P以内の20P刻みで指定してください。');
    if(!subject.value) return toast('契約対象の色を選択してください。');
    ...
    if(!res.ok){toast(res.message);post.disabled=false;}
    else if(res.expired){toast('参加対象者がいないため契約は成立せず、使用Pを返却しました。');}
    else { const perPlayer=(n/4)*(state.turn>=state.maxTurns?2:1); toast(`公開契約を提示しました。報酬1人${perPlayer}P`); }
  });
  creator.append(type, subject, reward, rewardPreview, post);
  const note=document.createElement('div'); note.className='microNote'; note.textContent='公開契約は自分自身の色も条件対象に指定できます。提示と同時に、条件上参加可能な全員が自動参加します。誰が提示したか・誰が参加しているかは公開されません。件数制限なし。同じターンに複数件、同時に複数件提示できます。使用Pは20P刻み。通常は20P使用→1人5P、40P使用→1人10P。第15ターンは報酬2倍。未達成分は提示者へ返却。ターン消費なし。'; creator.appendChild(note);
...
function renderTurnHistory() {
  ...
  if (!history.length) {
    const empty = document.createElement('div');
    empty.className = 'microNote';
    empty.textContent = 'ターン履歴はありません。';
    box.appendChild(empty);
    return;
  }
  for (const turn of history) {
    const details = document.createElement('details');
    details.className = 'turnHistoryTurn';
    if (Number(turn.turn) === Number(history.at(-1)?.turn)) details.open = true;
    const summary = document.createElement('summary');
    const title = document.createElement('b'); title.textContent = `第${turn.turn}ターン`;
    const count = document.createElement('span'); count.textContent = `${(turn.players || []).length}人`;
    summary.append(title, count);
    details.appendChild(summary);

    const body = document.createElement('div'); body.className = 'turnHistoryBody';
    for (const rowData of turn.players || []) {
      const row = document.createElement('div'); row.className = 'turnHistoryRow';
      const who = document.createElement('b'); who.className = 'turnHistoryWho'; who.textContent = rowData.color || '—';
      const action = document.createElement('div'); action.className = 'turnHistoryAction'; action.textContent = rowData.action || '行動なし';
      const result = document.createElement('div'); result.className = 'turnHistoryResult';
      result.textContent = (rowData.results || []).filter(Boolean).join(' / ') || '結果なし';
      row.append(who, action, result); body.appendChild(row);
    }
    if ((turn.events || []).length) {
      const events = document.createElement('div'); events.className = 'turnHistoryEvents';
      const eventTitle = document.createElement('b'); eventTitle.textContent = '全体結果';
      events.appendChild(eventTitle);
      for (const text of turn.events) {
        const line = document.createElement('div'); line.textContent = text; events.appendChild(line);
      }
...
function renderRanking() {
  ...
  const chipBox = $('#chipSettlement');
  chipBox?.replaceChildren();
  const settlement = state.chipSettlement;
  if (chipBox) {
    if (!settlement || Number(settlement.stake || 0) <= 0) {
      const empty = document.createElement('div');
      empty.className = 'microNote';
      empty.textContent = 'この試合は五戯チップを賭けていません。';
      chipBox.appendChild(empty);
    } else {
      const summary = document.createElement('div');
      summary.className = 'chipSettlementSummary';
      const firstPool = settlement.firstRankingPool ?? settlement.gamePool;
      const secondPool = settlement.secondRankingPool ?? settlement.predictionPool;
      summary.textContent = `賭け額 ${formatChips(settlement.stake)} / 第1順位1位賞金枠 ${formatChips(firstPool)} / 第2順位1位賞金枠 ${formatChips(secondPool)}`;
      chipBox.appendChild(summary);
      for (const result of settlement.results || []) {
        const row = document.createElement('div');
        row.className = `chipResultRow${result.playerId === state.me?.playerId ? ' me' : ''}`;
        const who = document.createElement('b');
        who.textContent = result.color || '—';
        const detail = document.createElement('span');
        const parts = [];
        const firstAward = result.firstRankingAward ?? result.gameAward ?? 0;
        const secondAward = result.secondRankingAward ?? result.predictionAward ?? 0;
        if (Number(firstAward) > 0) parts.push(`第1順位1位 +${formatChips(firstAward)}`);
        if (Number(secondAward) > 0) parts.push(`第2順位1位 +${formatChips(secondAward)}`);
        detail.textContent = parts.length ? parts.join(' / ') : '賞金なし';
        const total = document.createElement('strong');
        total.textContent = `+${formatChips(result.totalAward || 0)}`;
        row.append(who, detail, total);
...
saveChipWallet();
renderChipWallet();
$('#chipStakeInput')?.addEventListener('change', () => {
  const input = $('#chipStakeInput');
  let n = Number(input?.value || 0);
  if (!Number.isFinite(n) || n < 0) n = 0;
  n = Math.floor(n / CHIP_STAKE_STEP) * CHIP_STAKE_STEP;
  const max = Math.floor(Number(chipWallet.balance || 0) / CHIP_STAKE_STEP) * CHIP_STAKE_STEP;
  if (n > max) n = max;
  if (input) input.value = String(n);
});
$('#chipRecharge')?.addEventListener('click', rechargeChipWallet);
$('#dismissAfkWarning')?.addEventListener('click', () => {
  const banner = $('#afkWarningBanner');
  dismissedAfkWarningKey = banner?.dataset?.warningKey || `${state?.roomId || ''}:${state?.turn || 0}:${Number(state?.me?.afkStreak || 0)}`;
...
// iOS Safari / standalone PWA: prevent pinch gesture zoom. Scrolling remains enabled.
