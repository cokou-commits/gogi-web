...
const CARD_VISUALS = {
  ...
  double: { image: `/cards/double.webp?v=${ASSET_REV}`, effect: '通常カードの効果を2倍にする' }
};
...
function phaseName(phase) { return ({chat:'チャット中', action:'バトル中', result:'結果発表', finished:'終了'})[phase] || phase || '—'; }
...
$('#startWithCpu')?.addEventListener('click', async () => {
  const button = $('#startWithCpu');
  if (!state?.canStartWithCpu || button?.disabled) return;
  if (button) button.disabled = true;
  const res = await emitAck('startWithCpu', {});
  if (!res.ok) {
    if (button) button.disabled = false;
    return toast(res.message);
  }
  toast(`${res.added || 0}人をCPで補充しました。`);
});
...
function renderLobby() {
  ...
  const cpuStart = $('#startWithCpu');
  cpuStart?.classList.toggle('hidden', !state.canStartWithCpu);
  if (cpuStart) {
    cpuStart.disabled = false;
    cpuStart.textContent = `不足${Math.max(0, 5 - count)}人をCPで補充して開始`;
  }
  const copy = $('#copyCode');
...
function renderMe() {
  ...
  standingStat?.classList.toggle('hidden', !standing);
...
function renderPlayers() {
  ...
  for (const p of state.players) {
    ...
    if (!p.isCpu && !p.connected) flags.push('切断');
...
function renderActionComposer(container, finalMode) {
  ...
  noneSpecial.className = `cardBtn noSpecial${!draft.special ? ' selected' : ''}`;
...
function renderRanking() {
  ...
  for (const row of state.finishedRanking || []) {
    ...
    const title = document.createElement('b'); title.textContent = `${row.color}${row.isCpu ? '（CP）' : ''}`;
