...
const CLIENT_BUILD_ID = '20260913-complete-final17-pinch-cardfixed';
...
const ASSET_REV = '20260913-complete-final17-pinch-cardfixed';
const CARD_VISUALS = {
...
function renderRanking() {
  ...
  if (chipBox) {
    if (!settlement || Number(settlement.stake || 0) <= 0) {
      ...
      summary.textContent = `賭け額 ${formatChips(settlement.stake)} / 最終1位賞金枠 ${formatChips(settlement.gamePool)} / 予想1位賞金枠 ${formatChips(settlement.predictionPool)}`;
      ...
      for (const result of settlement.results || []) {
        ...
        const parts = [];
        if (Number(result.gameAward || 0) > 0) parts.push(`最終1位 +${formatChips(result.gameAward)}`);
        if (Number(result.predictionAward || 0) > 0) parts.push(`${settlement.predictionFallbackToFinalWinner ? '予想枠繰越' : '予想1位'} +${formatChips(result.predictionAward)}`);
      ...
      if (settlement.predictionFallbackToFinalWinner) {
        const note = document.createElement('div');
        note.className = 'microNote';
        note.textContent = '1位予想の的中者がいない／参加者がいないため、予想賞金枠は第2順位発表の1位へ配分しました。';
...
// iOS Safari / standalone PWA: pinch zoom is allowed. Input auto-zoom is prevented by 16px form controls.
