...
const CLIENT_BUILD_ID = '20260912-waitleavefinal1';
...
async function leaveCurrentRoom() {
  ...
  // 待機退出は、以前正常に動作していた正式なleaveRoom ACK方式に戻す。
  // Socket切断を退出の代用にせず、サーバーがプレイヤー削除を完了してからホームへ戻る。
  if (state?.status !== 'finished') {
    ...
    if (!res.ok) return toast(res.message || '退出できませんでした。もう一度押してください。');
  }
  clearSessionToken(leavingToken);
