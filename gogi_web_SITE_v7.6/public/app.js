...
const CLIENT_BUILD_ID = '20260913-complete-final16-nozoom';
...
const ASSET_REV = '20260913-complete-final16-nozoom';
const CARD_VISUALS = {
  ...
  fullDefense: { image: `/cards/fullDefense.webp?v=${ASSET_REV}`, effect: '自分への攻撃をすべて防ぐ' },
...
function renderActionComposer(container) {
  ...
  clearSpecial.addEventListener('click', () => updateDraft({ special: null, specialTargetId: null, specifiedType: null, specifiedTargetId: null }));
  ...
  if (['cancel','specify','steal'].includes(draft.special)) {
    ...
    target.innerHTML = '<span>対象を選択</span>';
    target.appendChild(makeSelect(playerOptions(), draft.specialTargetId, value => updateDraft({ specialTargetId: value || null, ...(draft.special === 'specify' ? { specifiedTargetId: null } : {}) }), disabled));
  ...
  if (draft.special === 'double' && ['attack','scout','accusation'].includes(draft.normal)) {
    ...
    target.appendChild(makeSelect(draft.normal === 'accusation' ? accusationPlayerOptions() : draft.normal === 'scout' ? scoutPlayerOptions() : playerOptions(), draft.secondNormalTargetId, value => updateDraft({ secondNormalTargetId: value || null }), disabled));
...
function renderInventory() {
  ...
  for (const [key, def] of Object.entries(state.specialCards || {})) {
    ...
    const text = document.createElement('span'); text.textContent = def.label;
    row.appendChild(text); box.appendChild(row);
  }
  ...
  note.textContent = `特殊カード購入：${specialPrice}P・ランダム。所持ポイントの範囲で回数制限なし`;
