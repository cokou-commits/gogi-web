...
const CLIENT_BUILD_ID = '20260912-waitleave-final2';
...
async function leaveCurrentRoom() {
  // ここは、実機で待機退出できていた旧WAIT_CANCEL_FIXの方式を
  // 現行版へ移植したもの。ゲーム本体・招待・人数同期など他機能は触らない。
  if (leavingLobby && lobbyLeavePromise) return lobbyLeavePromise;

  const leavingToken = tabSessionToken || state?.resumeToken || null;
  const leavingRoomKey = state?.roomId || state?.code || '';
  const leaveButton = $('#leaveLobby');
  const originalLabel = leaveButton?.textContent || '待機をやめる';

  // 退出中に遅延stateや人数同期が旧ロビーへ戻すことだけは防ぐ。
  leavingLobby = true;
  sessionWritesBlocked = true;
  ignoredLobbyRoomKey = leavingRoomKey;
  stopLobbySync();
  lobbyCountOverride = null;
  if (leaveButton) { leaveButton.disabled = true; leaveButton.textContent = '退出中…'; }

  lobbyLeavePromise = (async () => {
    // 旧正常版と同じ本線：resume完了を待たず、sessionToken付きleaveRoomを送る。
    const res = await emitAck('leaveRoom', { sessionToken: leavingToken }, 3500);

    if (!res.ok && !res.transient) {
      // サーバーが明示的に拒否した時だけ待機室へ残す。
      leavingLobby = false;
      sessionWritesBlocked = false;
      ignoredLobbyRoomKey = '';
      if (state?.status === 'lobby') startLobbySync();
      toast(res.message || '退出できませんでした。');
      return false;
    }

    // ACK欠落・通信断でもUIを待機室へ閉じ込めない。
    // 現行サーバーはロビーSocket切断時に待機レコードを即削除する。
    clearSessionToken(leavingToken);
    stopTimer();
    state = null;
    lastMessageIds.clear();
    ...
    if (!res.ok && res.transient) toast('待機を終了しました。');
    return true;
  })();

  try {
    return await lobbyLeavePromise;
  } finally {
    lobbyLeavePromise = null;
