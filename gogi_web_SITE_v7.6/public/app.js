...
function renderPublicContracts() {
  ...
  if (!contracts.length) {
    ...
    for (const c of contracts) {
      ...
      const rewardLabel = `報酬：1人${c.rewardPerPlayer || Math.floor(c.reward/4)}P${Number(c.rewardMultiplier)===2?'（最終ターン2倍）':''}`;
      const text=document.createElement('span'); text.textContent=`【${rewardLabel}】 ${c.issuerColor}：${condition}${participants}`;
  ...
  const note=document.createElement('div'); note.className='microNote'; note.textContent='公開契約の件数制限なし。同じターンに複数件、同時に複数件提示できます。使用Pは20P刻み。通常は20P使用→1人5P、40P使用→1人10P。第15ターンに判定される契約は報酬2倍で20P使用→1人10P、40P使用→1人20P。契約一覧・通知には使用Pを表示せず、1人あたりの報酬額だけを全員へ表示します。提示者本人は参加不可で最大4人。未参加・未達成分は提示者へ返却。ターン消費なし。'; creator.appendChild(note);
