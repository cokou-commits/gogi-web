...
const CLIENT_BUILD_ID = '20260913-gogi-zero-recharge1-cptest';
  ...
  if (chipBox) {
    if (!settlement || Number(settlement.stake || 0) <= 0) {
      ...
      for (const result of settlement.results || []) {
        ...
        if (Number(result.predictionAward || 0) > 0) parts.push(`予想1位 +${formatChips(result.predictionAward)}`);
      ...
      if (Number(settlement.unawarded || 0) > 0) {
        const note = document.createElement('div');
        note.className = 'microNote';
        note.textContent = `1位予想の的中者がいない等で未配布：${formatChips(settlement.unawarded)} 五戯チップ`;
