...
const CLIENT_BUILD_ID = '20260912-waitcancel-homefix1';
...
function leaveCurrentRoom() {
  ...
  // 現在表示しているroomのresumeTokenを最優先にする。古いタブ用tokenを先に使うと、
  // 本当に退出したい待機室のtokenがlocalStorageへ残り、再接続で待機室へ戻る原因になる。
  const leavingToken = state?.resumeToken || tabSessionToken || storageGet(localStore, SESSION_KEY) || null;
  const roomKey = state?.roomId || state?.code || '';
  const shouldNotifyServer = state?.status === 'lobby';

  // 「待機をやめる」はページ再読み込みを行わず、その場でホームへ確定遷移する。
  // leavingLobby/sessionWritesBlocked により、遅れて届くresume/state/人数同期がホームをロビーへ戻すのを遮断する。
  leavingLobby = true;
  ...
  // サーバー側の待機枠削除はバックグラウンドで行う。画面遷移は通信完了を一切待たない。
