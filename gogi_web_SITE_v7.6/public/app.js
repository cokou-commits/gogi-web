...
const CLIENT_BUILD_ID = '20260912-lobbyfix3';
...
function leaveCurrentRoom() {
  ...
  // 「待機をやめる」は旧ページの非同期処理と切り離す。
  // 1) 再接続/トークン再保存を永久停止 2) ローカル状態を即破棄 3) Beacon/keepaliveで退出通知
  // 4) clean URLへreplaceして旧ページ上のpending Promise/Socketイベント自体を終了する。
  leavingLobby = true;
  sessionWritesBlocked = true;
  ignoredLobbyRoomKey = roomKey;
  clearSessionToken(leavingToken);
  // 共有側に別経路で同じ退出トークンが残っていても、このタブの明示退出では削除する。
  ...
  if (shouldNotifyServer && leavingToken) {
    let beaconQueued = false;
    try {
      if (typeof navigator.sendBeacon === 'function') {
        const body = new Blob([JSON.stringify({ sessionToken: leavingToken })], { type:'application/json' });
        beaconQueued = navigator.sendBeacon('/api/leave-lobby', body);
      }
    } catch {}
    if (!beaconQueued) {
      fetch('/api/leave-lobby', {
        ...
        body:JSON.stringify({ sessionToken: leavingToken }),
        cache:'no-store',
        keepalive:true
      }).catch(() => {});
    }
  ...
  // 同じページ内でホーム表示だけにすると、既に走っているresume Promiseが後からロビーを復元し得る。
  // replaceでページ実行コンテキストごと破棄し、復帰トークンのない完全なホームとして再起動する。
  const cleanHomeUrl = `${location.origin}${location.pathname}`;
  try { location.replace(cleanHomeUrl); }
  catch { location.href = cleanHomeUrl; }
}
