...
- Start: `npm start`
- Health: `GET /health`
- 試合状態と五戯チップ台帳はサーバーメモリ保持のため、**1インスタンス運用**を前提とする
...
- `MAX_CHIP_WALLETS` — サーバー内五戯チップ台帳の最大browserId件数（既定10,000、超過時は最終利用が最も古いものを退避）
...
- 公開契約5条件（攻撃 / 攻撃しない / 防御 / 回復 / 告発）/ 自動参加 / 匿名提示 / 同ターン判定 / 複数同時契約 / 余剰返却 / 精算通知
...
- CP補充の全員同意 / CPの購入・交換・公開契約・1位予想・秘密目標対応
- 五戯チップlocalStorage改変がサーバー残高へ反映されないこと / 0残高のみチャージ可 / 対戦中チャージ拒否
- 15ターン完走を複数回
...
- 五戯チップ残高は実行中サーバーのbrowserId別台帳を正とし、localStorageの残高改変は参加残高へ反映しない。ただし恒久アカウント/DBがないため、browserId削除による新規扱い・別ブラウザ/別端末・サーバー再起動をまたぐ恒久残高までは保証しない。実価値を持たせる場合はアカウント認証＋サーバー永続DBが必須。
- サーバー再起動で進行中試合と五戯チップ台帳は失われ、チップは初期残高から再開する
