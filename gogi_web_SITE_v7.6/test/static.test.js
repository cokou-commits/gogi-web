...
test('待機をやめるは再接続競合を遮断し、再読み込みせず完全なホームへ遷移する', () => {
  const block = app.match(/function leaveCurrentRoom\(\) \{[\s\S]*?\n\}/)?.[0] || '';
  assert.match(block, /const leavingToken = state\?\.resumeToken \|\| tabSessionToken/);
  assert.match(block, /leavingLobby = true[\s\S]*sessionWritesBlocked = true[\s\S]*show\('#home'\)/);
  assert.match(block, /navigator\.sendBeacon/);
  assert.match(block, /fetch\('\/api\/leave-lobby'/);
  assert.match(block, /socket\.emit\('leaveRoom'/);
  assert.match(block, /socket\.disconnect\(\)/);
  assert.doesNotMatch(block, /location\.(?:replace|href)/);
...
test('待機室修正版はJS/CSSを新しいリビジョンで読み込み、HTML/JSの旧キャッシュを残さない', () => {
  assert.match(html, /app\.js\?v=20260912waitcancelhome1/);
  assert.match(html, /styles\.css\?v=20260912waitcancelhome1/);
