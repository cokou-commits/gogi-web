...
test('ゲーム終了画面ではWebSocketを解放し、トップ復帰で不要な再接続をしない', () => {
  ...
  assert.doesNotMatch(leaveBlock, /socket\.disconnect\(\)/);
});
...
test('待機をやめるは正式なleaveRoom ACK完了後にホームへ戻る', () => {
  const block = app.match(/async function leaveCurrentRoom\(\) \{[\s\S]*?\n\}(?=\s*\$\('#showJoin')/);
  const leaveBlock = block?.[0] || '';
  assert.match(leaveBlock, /await emitAck\('leaveRoom'/);
  assert.match(leaveBlock, /if \(!res\.ok\) return toast/);
  assert.match(leaveBlock, /clearSessionToken\(leavingToken\)/);
...
test('待機室修正版はJS/CSSを新しいリビジョンで読み込み、HTML/JSの旧キャッシュを残さない', () => {
  assert.match(html, /app\.js\?v=20260912leaveackrestore1/);
