...
test('確認テスト版はCP補充・自動行動・チップ対応を持ち、外部AI接続は持たない', () => {
  ...
  assert.match(server, /公開契約は選択参加ではなく自動参加/);
...
test('公開契約は参加操作不要で自動参加し、誰が出したか・誰が参加したかをクライアントへ出さない', () => {
  ...
  assert.match(app, /提示者と参加者の色は公開されません/);
...
test('待機室修正版はJS/CSSを新しいリビジョンで読み込み、HTML/JSの旧キャッシュを残さない', () => {
  assert.match(html, /app\.js\?v=20260913cpfull1/);
...
test('CP確認版はチャット以外の主要ゲーム機能を自動利用できる', () => {
  const server = fs.readFileSync(path.join(root, 'server.js'), 'utf8');
...
test('CPはチャット自動送信ロジックを持たない', () => {
  const server = fs.readFileSync(path.join(root, 'server.js'), 'utf8');
  const cpArea = server.slice(server.indexOf('// 確認テスト版専用CP。'), server.indexOf('function uniqueRoomCode'));
...
test('CP策士版は状況評価・偵察推論・勝率判断・取引評価を持ち、相手の秘密を直接参照しない', () => {
  ...
  const cpBlock = server.slice(server.indexOf('// 確認テスト版専用CP。'), server.indexOf('function autoEnrollPublicContract'));
...
test('CP最高戦略版は目標期限・終盤順位・期待値・再計画・二重告発推理まで使う', () => {
  ...
  assert.doesNotMatch(cpBlock, /target\.points/);
  assert.doesNotMatch(cpBlock, /target\.hand/);
  assert.match(server, /20260913-cp-strategist-max/);
});
