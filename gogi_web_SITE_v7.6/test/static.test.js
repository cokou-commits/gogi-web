...
test('ゲーム終了画面ではWebSocketを解放し、待機退出は初期版と同じ正式leaveRoom順序を使う', () => {
  ...
  const leaveAt = leaveBlock.indexOf("await emitAck('leaveRoom', {})");
  const clearAt = leaveBlock.indexOf('clearSessionToken');
  const stateNullAt = leaveBlock.indexOf('state = null');
  assert.ok(leaveAt >= 0 && clearAt > leaveAt && stateNullAt > leaveAt);
...
test('ホーム閲覧だけではWebSocketを自動接続せず、待機退出後もSocketを残さない', () => {
  ...
  const leaveBlock = app.match(/async function leaveCurrentRoom\(\) \{[\s\S]*?\n\}/)?.[0] || '';
  assert.match(leaveBlock, /await emitAck\('leaveRoom', \{\}\)/);
  assert.match(leaveBlock, /state = null[\s\S]*disconnectIdleSocket\(\)/);
  assert.doesNotMatch(leaveBlock, /fetch\('\/api\/leave-lobby'/);
});
...
test('待機をやめるは初期版と同じく退出ACK前にsessionとstateを破棄しない', () => {
  const block = app.match(/async function leaveCurrentRoom\(\) \{[\s\S]*?\n\}(?=\s*\$\('#showJoin'\))/);
  const leaveBlock = block?.[0] || '';
  assert.match(leaveBlock, /await emitAck\('leaveRoom', \{\}\)/);
  const leaveAt = leaveBlock.indexOf("await emitAck('leaveRoom', {})");
  const tokenClearAt = leaveBlock.indexOf('clearSessionToken');
  const stateClearAt = leaveBlock.indexOf('state = null');
  assert.ok(leaveAt >= 0 && tokenClearAt > leaveAt && stateClearAt > leaveAt);
...
test('待機室修正版はJS/CSSを新しいリビジョンで読み込み、HTML/JSの旧キャッシュを残さない', () => {
  assert.match(html, /app\.js\?v=20260912oldleave1/);
