...
test('ゲーム終了画面ではWebSocketを解放し、トップ復帰で不要な再接続をしない', () => {
  ...
  const leaveBlock = app.match(/function leaveCurrentRoom\(\) \{[\s\S]*?\n\}/)?.[0] || '';
  assert.match(leaveBlock, /state\?\.status === 'lobby'/);
  assert.match(leaveBlock, /socket\.disconnect\(\)/);
});
...
test('待機をやめるは再接続競合を遮断し、ホーム表示後にサーバー退出を確定する', () => {
  const block = app.match(/function leaveCurrentRoom\(\) \{[\s\S]*?\n\}\n\n\$\('#showJoin'/)?.[0] || '';
  assert.match(block, /leavingLobby = true[\s\S]*sessionWritesBlocked = true[\s\S]*show\('#home'\)/);
  assert.match(block, /fetch\('\/api\/leave-lobby'/);
  assert.match(block, /socket\.timeout\(1800\)\.emit\('leaveRoom'/);
  assert.match(block, /Promise\.all\(\[socketLeave, httpLeave\]\)/);
  assert.match(block, /\.finally\(\(\) => \{[\s\S]*?socket\.disconnect\(\)/);
  assert.match(app, /if \(token && sessionWritesBlocked\) return/);
  assert.match(app, /if \(sessionWritesBlocked \|\| leavingLobby\) return null/);
  assert.match(app, /leaveLobbyButton\?\.addEventListener\('pointerup'/);
  assert.match(app, /ignoredLobbyRoomKey && nextRoomKey === ignoredLobbyRoomKey/);
});
...
test('待機室修正版はJS/CSSを新しいリビジョンで読み込み、HTML/JSの旧キャッシュを残さない', () => {
  assert.match(html, /app\.js\?v=20260912leavefix4/);
...
test('待機をやめるはサーバー退出完了を確認してからSocketを切断する', () => {
  const block = app.match(/function leaveCurrentRoom\(\) \{[\s\S]*?\n\}\n\n\$\('#showJoin'/)?.[0] || '';
  assert.match(block, /socket\.timeout\(1800\)\.emit\('leaveRoom'/);
  assert.match(block, /fetch\('\/api\/leave-lobby'/);
  assert.match(block, /Promise\.all\(\[socketLeave, httpLeave\]\)/);
  assert.match(block, /\.finally\(\(\) => \{[\s\S]*?socket\.disconnect\(\)/);
  assert.match(block, /browserId[\s\S]*clientInstanceId[\s\S]*pageInstanceId/);
});

test('HTTP待機退出はtokenが消えていても同一タブ識別子で待機プレイヤーを除去できる', () => {
  const block = server.match(/app\.post\('\/api\/leave-lobby'[\s\S]*?\n\}\);/)?.[0] || '';
  assert.match(block, /normalizeBrowserId\(req\.body\?\.browserId\)/);
  assert.match(block, /normalizeClientInstanceId\(req\.body\?\.clientInstanceId\)/);
  assert.match(block, /normalizePageInstanceId\(req\.body\?\.pageInstanceId\)/);
  assert.match(block, /candidateRoom\.status !== 'lobby'/);
  assert.match(block, /removePlayerRecord\(room, p\.playerId\)/);
});
