...
test('対戦フェーズ名は会話・選択と結果発表の2段階を明示する', () => {
  assert.match(app, /chat:'会話・選択'/);
  assert.doesNotMatch(app, /action:'バトル中'/);
  assert.match(app, /result:'結果発表'/);
  assert.match(html, /id="phaseLabel">会話・選択/);
});
...
test('会話中の行動選択は通常・特殊カードを同一画面に6列で揃える', () => {
...
test('結果は1分待機し接続中の人間全員が次へなら早送りできる', () => {
  ...
  assert.match(html, /id="resultTimer">60</);
});
