...
test('購入は本人だけ更新し、交換は通常関係者だけ・準備OK解除時だけ全員の進行数を同期する', () => {
  ...
  assert.match(exchangeBlock, /if \(targetReadyInterrupted\) emitState\(room\)/);
  assert.match(exchangeBlock, /else \{ emitPlayerState\(room, p\); emitPlayerState\(room, target\); \}/);
});
...
test('カード効果文は画像内テキストだけに依存せず正式ルール文をHTMLで重ねる', () => {
  ...
  assert.match(app, /cancel:[\s\S]*?相手1人の通常カードまたは特殊カードを選んで無効化/);
...
test('カード画像はリビジョン付きURLとimmutable長期キャッシュを使う', () => {
  assert.match(app, /const ASSET_REV = '20260914-complete-final33-game-card-art-current-rules'/);
...
test('新秘密目標10種と最終得点体系がルール表示に反映される', () => {
  ...
  assert.match(html, /告発失敗-15P/);
  assert.match(html, /攻撃成功\+10P/);
  assert.match(html, /防御成功\+10P/);
  assert.match(html, /告発成功\+30P/);
  assert.match(html, /秘密目標\+25P/);
  assert.match(html, /何ターン目でも/);
});


test('最終ターンの成功得点は2倍、告発失敗は固定と表示される', () => {
  assert.match(html, /生存\+50P/);
  assert.match(html, /告発失敗-15P/);
  assert.match(html, /告発失敗-15Pは固定/);
  assert.match(html, /2倍対象外/);
});


test('カード指定で達成した公開契約報酬は実行者本人へ入る', () => {
  assert.match(server, /specifiedActionsByPlayer/);
  assert.doesNotMatch(server, /specifiedCausedSuccess/);
  assert.match(server, /カード指定で達成した場合も、公開契約報酬は実際に行動した本人へ入る/);
  assert.match(html, /指定行動で公開契約を達成した場合、その契約報酬も実行者本人へ入る/);
});
...
test('通常防御は他人を対象にでき、完全防御も対象1人を選べる', () => {
  const engine = fs.readFileSync(path.join(root, 'src/engine.js'), 'utf8');
  assert.match(app, /defensePlayerOptions/);
  assert.match(app, /const targetLabel = draft\.special === 'fullDefense' \? '守る対象を選択' : '対象を選択'/);
  assert.doesNotMatch(app, /防御対象を選択|回復対象を選択|告発対象を選択|防御する対象|回復する対象|通常カードの対象/);
  assert.match(engine, /defenseClaims/);
  assert.match(engine, /a\.special === 'fullDefense'[\s\S]*fullDefense\.get\(protectedTarget\.playerId\)\.push\(a\.p\)/);
  assert.match(html, /完全防御[\s\S]*自分または他人1人を対象に選び/);
});
...
test('現行ルール文書の告発点数・契約参加人数に旧仕様が残らない', () => {
  const rulesDoc = fs.readFileSync(path.join(root, 'RULES.md'), 'utf8');
  const decisions = fs.readFileSync(path.join(root, 'RULE_DECISIONS.md'), 'utf8');
  assert.match(rulesDoc, /成功 \+30P/);
  assert.match(rulesDoc, /失敗 -15P/);
  assert.doesNotMatch(rulesDoc, /告発成功 \+25P/);
...
test('通常カード・特殊カード購入は所持ポイントの範囲で回数制限なし', () => {
  ...
  assert.match(inventoryBlock, /特殊カード購入：ランダム\$\{specialPrice\}P \/ 指定\$\{specifiedSpecialPrice\}P。所持ポイントの範囲で回数制限なし/);
  assert.match(inventoryBlock, /specialType: key/);
  assert.match(buyBlock, /SPECIAL_SPECIFIED_PURCHASE_PRICE/);
...
test('2倍回復は2回目の対象を同一/別対象から選べる', () => {
  const engine = fs.readFileSync(path.join(root, 'src/engine.js'), 'utf8');
  assert.match(app, /\['attack','defense','scout','accusation','heal'\]\.includes\(draft\.normal\)/);
  assert.match(app, /draft\.normal === 'heal' \? healPlayerOptions\(\)/);
  assert.match(engine, /d\.normal === 'heal' && d\.secondNormalTargetId/);
  assert.match(engine, /healUses\.push\(\{ targetId:a\.secondNormalTargetId \|\| a\.normalTargetId, cardMultiplier:2 \}\)/);
  assert.match(html, /回復2回（同一\/別対象可。1回につき\+2）/);
});

test('特殊カードはランダム40Pに加えて種類指定50Pで購入できる', () => {
  const rulesJs = fs.readFileSync(path.join(root, 'src/rules.js'), 'utf8');
  const rulesMd = fs.readFileSync(path.join(root, 'RULES.md'), 'utf8');
  assert.match(rulesJs, /const SPECIAL_PURCHASE_PRICE = 40;/);
  assert.match(rulesJs, /const SPECIAL_SPECIFIED_PURCHASE_PRICE = 50;/);
  assert.match(server, /specified \? SPECIAL_SPECIFIED_PURCHASE_PRICE : SPECIAL_PURCHASE_PRICE/);
  assert.match(app, /specialSpecifiedPurchasePrice \?\? 50/);
  assert.match(app, /type: 'special', specialType: key/);
  assert.match(rulesMd, /特殊カード（種類指定）50P/);
  assert.match(html, /ランダム40P \/ 種類指定50P/);
});



test('完全防御は成功時固定20Pで最終倍率なし', () => {
  const rulesJs = fs.readFileSync(path.join(root, 'src/rules.js'), 'utf8');
  const engineJs = fs.readFileSync(path.join(root, 'src/engine.js'), 'utf8');
  const rulesMd = fs.readFileSync(path.join(root, 'RULES.md'), 'utf8');
  const html = fs.readFileSync(path.join(root, 'public/index.html'), 'utf8');
  assert.match(rulesJs, /fullDefenseSuccess: 20/);
  assert.match(engineJs, /SCORING\.fullDefenseSuccess[\s\S]*multiplyFinal:false/);
  assert.match(rulesMd, /完全防御: 成功時固定\+20P/);
  assert.match(html, /完全防御[\s\S]*成功時\+20P/);
});
...
test('待機室修正版はJS/CSSを新しいリビジョンで読み込み、HTML/JSの旧キャッシュを残さない', () => {
  assert.match(html, /app\.js\?v=20260914-complete-final33-game-card-art-current-rules/);
  assert.match(html, /styles\.css\?v=20260914-complete-final33-game-card-art-current-rules/);
...
test('五戯チップ賞金は第1順位1位2.5倍・第2順位1位2.5倍で結果表示する', () => {
  ...
  assert.match(server, /room\.preBetRanking = buildRanking\(room\)/);
  assert.match(server, /room\.finishedRanking = buildRanking\(room\)/);
  assert.match(server, /room\.chipSettlement = settleChipWager\(room\)/);
  assert.match(html, /id="chipSettlement"/);
  assert.match(html, /第1順位発表（1位予想払戻し前）の1位に賭け額の2\.5倍/);
  assert.match(html, /第2順位発表（払戻し後の最終順位）の1位に賭け額の2\.5倍/);
  assert.match(app, /第1順位1位賞金枠/);
  assert.match(app, /第2順位1位賞金枠/);
});
...
test('CP最高戦略版は目標期限・終盤順位・期待値・再計画・二重告発推理まで使う', () => {
  ...
  assert.doesNotMatch(cpBlock, /target\.points/);
  assert.doesNotMatch(cpBlock, /target\.hand/);
  assert.match(server, /20260914-complete-final33-game-card-art-current-rules/);
});
...
test('準備OK中でも交換提案を受信でき、接続中の人間だけ準備OKを自動解除して回答を要求する', () => {
  const createBlock = server.match(/socket\.on\('createExchange'[\s\S]*?\n  \}\);/)?.[0] || '';
  assert.match(server, /function interruptReadyForIncomingExchange\(room, target\)/);
  assert.match(server, /target\.ready = false/);
  assert.match(server, /target\.connected/);
  assert.match(server, /target\.autoAdvance/);
  assert.doesNotMatch(createBlock, /target\.ready && !isCpu\(target\)/);
  assert.match(createBlock, /interruptReadyForIncomingExchange\(room, target\)/);
  assert.match(createBlock, /準備OKを解除しました。回答後、もう一度「次へ」を押してください。/);
  assert.match(createBlock, /readyInterrupted:targetReadyInterrupted/);
  assert.match(server, /interruptReadyForIncomingExchange\(room, plan\.target\)/);
  assert.match(server, /interruptReadyForIncomingExchange\(room, human\)/);
});

test('公開契約は準備OKを強制解除せず、公開契約・交換通知をトーストでも表示する', () => {
  const postBlock = server.match(/socket\.on\('postPublicContract'[\s\S]*?\n  \}\);/)?.[0] || '';
  assert.match(postBlock, /emitGameNotice\(room, room\.players\.map\(x => x\.playerId\)/);
  assert.doesNotMatch(postBlock, /\.ready = false/);
  const noticeBlock = app.match(/function showGameNotice\(event\) \{[\s\S]*?\n\}/)?.[0] || '';
  assert.match(noticeBlock, /event\.kind === 'contract' \|\| event\.kind === 'exchange'/);
  assert.match(noticeBlock, /toast\(event\.text\)/);
});


test('static: PWA and iOS home-screen name is exactly 五疑戦', () => {
  const manifest = fs.readFileSync(path.join(root, 'public/manifest.webmanifest'), 'utf8');
  assert.match(html, /<title>五疑戦<\/title>/);
  assert.match(html, /apple-mobile-web-app-title\" content=\"五疑戦\"/);
  assert.match(html, /application-name\" content=\"五疑戦\"/);
  assert.match(html, /manifest\.webmanifest\?v=20260914-complete-final33-game-card-art-current-rules/);
  assert.match(manifest, /\"name\": \"五疑戦\"/);
  assert.match(manifest, /\"short_name\": \"五疑戦\"/);
});

test('iOS/モバイルではピンチ・ダブルタップ・入力フォーカスを含むページ拡大を無効化する', () => {
  assert.match(html, /maximum-scale=1/);
  assert.match(html, /user-scalable=no/);
  assert.match(css, /html,body\{[\s\S]*touch-action:pan-x pan-y/);
  assert.match(app, /gesturestart[\s\S]*preventDefault/);
});
