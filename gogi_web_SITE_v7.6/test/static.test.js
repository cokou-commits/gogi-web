...
test('プライベートルームは5人参加済みなら一時切断者がいても開始できる', () => {
  ...
  assert.match(block, /room\.isPublic && !room\.players\.every\(p => p\.connected \|\| isTestCpu\(p\)\)/);
...
test('確認テスト版はCP補充・自動行動・チップ対応を持ち、外部AI接続は持たない', () => {
  const server = fs.readFileSync(path.join(root, 'server.js'), 'utf8');
  const app = fs.readFileSync(path.join(root, 'public/app.js'), 'utf8');
  const html = fs.readFileSync(path.join(root, 'public/index.html'), 'utf8');
  const engine = fs.readFileSync(path.join(root, 'src/engine.js'), 'utf8');
  assert.match(server, /fillRoomWithTestCpu/);
  assert.match(server, /buildTestCpuDraft/);
  assert.match(server, /socket\.on\('fillWithCpu'/);
  assert.match(server, /p\.connected \|\| isTestCpu\(p\)/);
  assert.match(server, /p\.chips = Math\.max/);
  assert.match(server, /条件上参加可能なCPは全員自動参加/);
  assert.match(app, /fillWithCpu/);
  assert.match(app, /p\.isCpu/);
  assert.match(html, /CPで空き枠を補充/);
...
test('待機室修正版はJS/CSSを新しいリビジョンで読み込み、HTML/JSの旧キャッシュを残さない', () => {
  assert.match(html, /app\.js\?v=20260913gogizerorecharge1/);
...
test('公開契約の公開表示は使用Pを見せず1人あたり報酬だけ表示する', () => {
  assert.doesNotMatch(app, /\/ 契約\$\{c\.reward\}P/);
  assert.match(app, /【\$\{rewardLabel\}】 \$\{c\.issuerColor\}：\$\{condition\}\$\{participants\}/);
  assert.doesNotMatch(server, /契約\$\{n\}P（報酬1人/);
  assert.doesNotMatch(server, /\$\{contract\.reward\}P返却/);
  assert.match(server, /公開契約を提示：\$\{condition\} \/ 報酬1人\$\{perPlayer\}P/);
  assert.match(server, /const perPlayer = contractRewardPerPlayer\(contract\.reward, contract\.dueTurn === MAX_TURNS \? 2 : 1\);[\s\S]*if \(acceptorIds\.length > 0\)/);
});

test('公開契約は第15ターンにも提示でき同ターン判定、報酬額を全員へ毎回表示する', () => {
  assert.match(server, /公開契約は第1〜15ターンの会話時間内に作成できます/);
  assert.match(server, /const dueTurn = room\.turn >= MAX_TURNS \? MAX_TURNS : room\.turn \+ 1/);
  assert.match(app, /state\.turn <= state\.maxTurns/);
  assert.match(app, /この契約の達成報酬：1人/);
  assert.match(app, /【\$\{rewardLabel\}】/);
  assert.match(server, /公開契約を提示：\$\{condition\} \/ 報酬1人\$\{perPlayer\}P/);
  assert.match(server, /公開契約に参加 \/ 報酬1人\$\{perPlayer\}P/);
  assert.match(server, /人参加 \/ 報酬1人\$\{perPlayer\}P \/ 第\$\{contract\.dueTurn\}ターンに判定/);
});
...
test('五戯チップUIは初期100000・所持0のみ100000チャージ・0/100刻み・同額公開マッチを持つ', () => {
  ...
  assert.match(app, /function rechargeChipWallet\(\)[\s\S]*Number\(chipWallet\.balance \|\| 0\) !== 0[\s\S]*chipWallet\.balance = CHIP_RECHARGE_AMOUNT/);
  ...
  assert.match(app, /emitAck\('publicMatch', \{[^}]*chipStake[^}]*chipBalance/);
...
test('プライベート五戯チップは部屋主設定額を強制し不足参加を拒否する', () => {
  assert.match(app, /emitAck\('createPrivate', \{[^}]*chipStake[^}]*chipBalance/);
  assert.match(app, /emitAck\('joinPrivate', \{[^}]*chipBalance/);
...
test('五戯チップ賞金は最終1位2.5倍・予想成績1位2.5倍で結果表示する', () => {
  ...
  assert.match(app, /予想1位 \+\$\{formatChips\(result\.predictionAward\)\}/);
});
