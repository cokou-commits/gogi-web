...
test('カード画像はリビジョン付きURLとimmutable長期キャッシュを使う', () => {
  assert.match(app, /const ASSET_REV = '20260913-complete-final17-pinch-cardfixed'/);
...
test('待機室修正版はJS/CSSを新しいリビジョンで読み込み、HTML/JSの旧キャッシュを残さない', () => {
  assert.match(html, /app\.js\?v=20260913-complete-final17-pinch-cardfixed/);
  assert.match(html, /styles\.css\?v=20260913-complete-final17-pinch-cardfixed/);
...
test('CP最高戦略版は目標期限・終盤順位・期待値・再計画・二重告発推理まで使う', () => {
  ...
  assert.doesNotMatch(cpBlock, /target\.points/);
  assert.doesNotMatch(cpBlock, /target\.hand/);
  assert.match(server, /20260913-complete-final17-pinch-cardfixed/);
});
...
test('static: PWA and iOS home-screen name is exactly 五疑戦', () => {
  ...
  assert.match(html, /manifest\.webmanifest\?v=20260913-complete-final17-pinch-cardfixed/);
  assert.match(manifest, /\"name\": \"五疑戦\"/);
  assert.match(manifest, /\"short_name\": \"五疑戦\"/);
});

test('iOS/モバイルは入力フォーカス自動拡大を防ぎつつピンチズームを許可する', () => {
  assert.doesNotMatch(html, /maximum-scale=1/);
  assert.doesNotMatch(html, /user-scalable=no/);
  assert.match(css, /touch-action:pan-x pan-y pinch-zoom !important/);
  assert.doesNotMatch(app, /gesturestart[\s\S]*preventDefault/);
  assert.match(css, /font-size:16px !important/);
});

test('縦画面のカード欄は内部スクロールせず固定表示に戻す', () => {
  assert.match(css, /#game\.portraitGame #draftComposer\{[\s\S]*?overflow:hidden !important/);
  assert.match(css, /#game\.portraitGame \.portraitCards\{[\s\S]*?overflow:hidden !important/);
