...
test('待機をやめるはACK待ちより先にホームへ戻り、遅延stateでロビー復帰しない', () => {
  const block = app.match(/async function leaveCurrentRoom\(\) \{[\s\S]*?\n\}/)?.[0] || '';
  assert.match(block, /leavingLobby = true[\s\S]*show\('#home'\)[\s\S]*socket\.timeout\(1500\)\.emit\('leaveRoom'/);
  assert.match(app, /if \(leavingLobby && next\.status === 'lobby'\) return/);
});
...
test('待機室は定期的にサーバーstateを再取得して人数表示の取りこぼしを自己修復する', () => {
  assert.match(app, /setInterval\(requestLobbySync, 2500\)/);
  assert.match(server, /socket\.on\('requestState'[\s\S]*emitPlayerState\(room, p\)/);
});
