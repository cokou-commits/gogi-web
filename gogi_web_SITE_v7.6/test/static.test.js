...
test('ゲーム終了画面ではWebSocketを解放し、待機退出は旧正常版のresume非依存方式を使う', () => {
  ...
  const leaveBlock = app.match(/async function leaveCurrentRoom\(\) \{[\s\S]*?\}(?=\s*\$\('#showJoin'\))/)?.[0] || '';
...
test('待機をやめるは旧210/210正常版の本線を現行機能を残したまま使う', () => {
  const leaveBlock = app.match(/async function leaveCurrentRoom\(\) \{[\s\S]*?\}(?=\s*\$\('#showJoin'\))/)?.[0] || '';
  assert.match(app, /!\['resume','leaveRoom'\]\.includes\(event\)/);
  assert.match(leaveBlock, /emitAck\('leaveRoom', \{ sessionToken: leavingToken \}, 3500\)/);
  assert.match(leaveBlock, /leavingLobby = true/);
  ...
  assert.match(leaveBlock, /ignoredLobbyRoomKey = leavingRoomKey/);
  assert.match(leaveBlock, /stopLobbySync\(\)/);
  assert.match(leaveBlock, /if \(!res\.ok && !res\.transient\)/);
  assert.match(leaveBlock, /if \(socket\.connected\) socket\.disconnect\(\)/);
  assert.doesNotMatch(leaveBlock, /fetch\('\/api\/leave-lobby'/);
  assert.doesNotMatch(leaveBlock, /Promise\.allSettled\(\[socketLeave, httpLeave\]\)/);
...
test('待機室修正版はJS/CSSを新しいリビジョンで読み込み、HTML/JSの旧キャッシュを残さない', () => {
  assert.match(html, /app\.js\?v=20260912waitleave-210restore1/);
