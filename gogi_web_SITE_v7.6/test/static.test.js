...
test('カード画像はリビジョン付きURLとimmutable長期キャッシュを使う', () => {
  assert.match(app, /const ASSET_REV = '20260913-complete-final10-icon'/);
...
test('待機室修正版はJS/CSSを新しいリビジョンで読み込み、HTML/JSの旧キャッシュを残さない', () => {
  assert.match(html, /app\.js\?v=20260913-complete-final10-icon/);
  assert.match(html, /styles\.css\?v=20260913-complete-final10-icon/);
...
test('CP最高戦略版は目標期限・終盤順位・期待値・再計画・二重告発推理まで使う', () => {
  ...
  assert.doesNotMatch(cpBlock, /target\.points/);
  assert.doesNotMatch(cpBlock, /target\.hand/);
  assert.match(server, /20260913-complete-final10-icon/);
});
...
test('static: PWA and iOS home-screen name is exactly 五疑戦', () => {
  ...
  assert.match(html, /manifest\.webmanifest\?v=20260913-complete-final10-icon/);
