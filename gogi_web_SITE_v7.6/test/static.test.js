...
test('カード画像はリビジョン付きURLとimmutable長期キャッシュを使う', () => {
  assert.match(app, /const ASSET_REV = '20260911economy1'/);
...
test('確認テスト版はCP補充と自動行動を持ち、本番用AI外部接続は持たない', () => {
  const server = fs.readFileSync(path.join(root, 'server.js'), 'utf8');
  const app = fs.readFileSync(path.join(root, 'public/app.js'), 'utf8');
  const html = fs.readFileSync(path.join(root, 'public/index.html'), 'utf8');
...
test('1位予想UIと二段階順位発表を持つ', () => {
  ...
  assert.match(html, /id="showBetSettlement"/);
...
test('カード指定中は指定カードしか使えないことを指定者非公開で明示する', () => {
  assert.match(app, /指定されているため、このカードしか使えません/);
  assert.match(app, /forcedKey !== key/);
  assert.doesNotMatch(app, /指定者[:：]/);
});

test('公開契約の報酬は所持P以内で5P刻みに自由設定できる', () => {
  assert.match(app, /reward\.min='5'; reward\.step='5'/);
  assert.match(server, /n < 5 \|\| n % 5 !== 0 \|\| n > p\.points/);
});


test('タイトル直下にBGM・効果音のON/OFFを持ち端末設定を保存する', () => {
  assert.match(html, /id="bgmToggle"/);
  assert.match(html, /id="sfxToggle"/);
  assert.match(css, /\.audioToggles/);
  assert.match(app, /AUDIO_BGM_KEY/);
  ...
  assert.match(app, /function startBgm\(/);
});
