...
test('再接続直後の通常操作はresume完了を待ってから送信する', () => {
  ...
  assert.match(app, /if \(event !== 'resume' && event !== 'leaveRoom'\)[\s\S]*await sessionReadyPromise/);
...
test('ゲーム終了画面ではWebSocketを解放し、待機退出もACK後にSocketを解放する', () => {
  ...
  assert.match(leaveBlock, /await emitAck\('leaveRoom'/);
...
test('ホーム閲覧だけではWebSocketを自動接続せず、待機退出後もSocketを残さない', () => {
...
test('待機をやめるは遅延ロビーstateを遮断して即ホームへ戻る', () => {
  const block = app.match(/async function leaveCurrentRoom\(\) \{[\s\S]*?\n\}(?=\s*\$\('#showJoin'\))/);
  const leaveBlock = block?.[0] || '';
  assert.match(leaveBlock, /leavingLobby = true/);
  assert.match(leaveBlock, /sessionWritesBlocked = true/);
  assert.match(leaveBlock, /ignoredLobbyRoomKey = leavingRoomKey/);
  assert.match(leaveBlock, /stopLobbySync\(\)/);
  assert.match(leaveBlock, /show\('#home'\)/);
  assert.match(leaveBlock, /await emitAck\('leaveRoom'/);
  assert.match(leaveBlock, /if \(socket\.connected\) socket\.disconnect\(\)/);
  assert.match(app, /if \(next\.status === 'lobby' && \(leavingLobby \|\| \(ignoredLobbyRoomKey/);
  assert.match(app, /event !== 'resume' && event !== 'leaveRoom'/);
});

test('待機退出APIはpageInstanceIdが変わっても同一browserIdとclientInstanceIdの古いロビー枠を掃除する', () => {
  const route = server.match(/app\.post\('\/api\/leave-lobby'[\s\S]*?\n\}\);/)?.[0] || '';
  assert.match(route, /player\.browserId === requestBrowserId[\s\S]*player\.clientInstanceId === requestClientInstanceId/);
});
...
test('待機室修正版はJS/CSSを新しいリビジョンで読み込み、HTML/JSの旧キャッシュを残さない', () => {
  assert.match(html, /app\.js\?v=20260912waitleaveracefix1/);
