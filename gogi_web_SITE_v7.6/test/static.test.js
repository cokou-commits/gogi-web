...
test('待機をやめるは以前の切断即退出方式でSocketを即切断する', () => {
  const block = app.match(/function leaveCurrentRoom\(\) \{[\s\S]*?\n\}\n\n\$\('#showJoin'/)?.[0] || '';
  assert.match(block, /clearSessionToken\(leavingToken\)/);
  assert.match(block, /socket\.disconnect\(\)/);
  assert.match(block, /show\('#home'\)/);
  assert.doesNotMatch(block, /fetch\('\/api\/leave-lobby'/);
  assert.doesNotMatch(block, /socket\.timeout\(1800\)\.emit\('leaveRoom'/);
  assert.doesNotMatch(block, /Promise\.all\(\[socketLeave, httpLeave\]\)/);
});
...
test('待機室修正版はJS/CSSを新しいリビジョンで読み込み、HTML/JSの旧キャッシュを残さない', () => {
  assert.match(html, /app\.js\?v=20260912lobbynoresume1/);
