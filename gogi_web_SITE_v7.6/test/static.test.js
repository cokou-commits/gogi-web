...
test('待機をやめるは通信応答なしで即ホームへ戻り、SocketとHTTPの二重経路で退出する', () => {
  const block = app.match(/function leaveCurrentRoom\(\) \{[\s\S]*?\n\}/)?.[0] || '';
  assert.match(block, /leavingLobby = true[\s\S]*ignoredLobbyRoomKey = roomKey[\s\S]*show\('#home'\)/);
  ...
  assert.match(app, /closest\?\.\('#leaveLobby'\)/);
});
...
test('待機室修正版はJS/CSSを新しいリビジョンで読み込み、HTML/JSの旧キャッシュを残さない', () => {
  assert.match(html, /app\.js\?v=20260912lobbyfix3/);
  assert.match(html, /styles\.css\?v=20260912lobbyfix3/);
...
test('友達招待はWeb Share失敗時にClipboardとlegacy copyへフォールバックする', () => {
  ...
  assert.match(app, /navigator\.share[\s\S]*copyTextRobust\(inviteText\)/);
