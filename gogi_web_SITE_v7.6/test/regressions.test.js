...
test('異なるカード指定が同じ対象に重なるとこのターンの通常行動なし', () => {
  const room = setup(); clear(room);
  ...
  const result = resolveTurn(room);
  assert.equal(victim.hp, 5, '元の通常行動も実行しない');
  assert.equal(target.hand.attack, attackBefore, '競合時は対象の通常カードを消費しない');
  assert.equal(target.forcedNormalType, null, '次ターンへ競合状態を持ち越さない');
  assert.ok(result.privateEvents.some(e => e.to === target.playerId && /このターンの通常行動はなし/.test(e.text)));
});

test('同じカード指定が重なっても競合せずこのターンに同一指定として発動する', () => {
  const room = setup(); clear(room);
  const [a,b,target,attacker] = room.players;
  a.specials.specify = 1;
  b.specials.specify = 1;
  ...
  assert.equal(a.points, 20, '各指定者はカード指定成功の固定+20P');
  assert.equal(b.points, 20, '各指定者はカード指定成功の固定+20P');
  assert.equal(target.points, 10, '指定された防御成功+10Pは実行者本人');
  assert.equal(target.forcedNormalType, null);
  assert.ok(result.scoreEvents.some(e => e.playerId === target.playerId && e.reason === '防御成功' && e.actual === 10));
});
