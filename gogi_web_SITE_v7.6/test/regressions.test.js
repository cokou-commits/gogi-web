...
test('特殊カードは所持中非公開だが使用時はカード名が公開イベントに出る', () => {
  const room = setup(); clear(room);
...
test('賭け師は偵察済み相手への告発成功では達成しない', () => {
  ...
  p.draft = draft({ normal:'scout', normalTargetId:target.playerId });
  resolveTurn(room);
  room.turn = 2; room.phase = 'action'; clear(room);
...
test('購入価格は15・15・15・25・35・特殊45P', () => {
  ...
  assert.deepEqual(Object.fromEntries(Object.entries(NORMAL_CARDS).map(([k,v]) => [k,v.price])), {
    attack:15, defense:15, scout:15, accusation:25, heal:35
  });
  assert.equal(SPECIAL_PURCHASE_PRICE, 45);
