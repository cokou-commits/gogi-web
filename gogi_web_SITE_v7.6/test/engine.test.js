...
test('カード指定された告発は成功+30Pだけ指定者へ入り、失敗-15Pは実行者本人が受ける', () => {
  const room = setup(); clearDrafts(room);
  const [specifier, forced, target] = room.players;
  ...
  assert.equal(specifier.points, 30);
  assert.equal(forced.points, 0);
  assert.ok(result.scoreEvents.some(e => e.playerId === specifier.playerId && e.sourcePlayerId === forced.playerId && e.reason === '告発成功' && e.actual === 30));
...
test('2倍告発は同じ相手に2回推理でき、2回目だけ正解でも成功', () => {
  ...
  assert.equal(a.points, 15, '1回失敗-15P + 1回成功+30P');
  assert.equal(b.secretState.invalid, true);
});
...
test('告発失敗は第15ターンに2倍で-30P', () => {
  ...
  const result = resolveTurn(room);
  assert.equal(a.points, -30);
  assert.ok(result.scoreEvents.some(e => e.playerId === a.playerId && e.reason === '告発失敗' && e.actual === -30));
});
