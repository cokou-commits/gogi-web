...
function randomValidDraft(room, p, rng) {
  ...
  if (['cancel','specify'].includes(d.special) && targets.length) d.specialTargetId = pick(rng, targets).playerId;
  if (d.special === 'specify') {
    d.specifiedType = pick(rng, Object.keys(NORMAL_CARDS));
    const forced = room.players.find(p => p.playerId === d.specialTargetId);
    const candidates = room.players.filter(p => {
      if (!forced) return false;
      if (['attack','scout','accusation'].includes(d.specifiedType) && p.playerId === forced.playerId) return false;
      if (['attack','defense','heal'].includes(d.specifiedType) && !p.alive) return false;
      return true;
    });
    if (candidates.length) d.specifiedTargetId = pick(rng, candidates).playerId;
  }
  if (d.special === 'steal') {
    d.stealTargets = {};
    let remaining = 50;
    for (const target of targets) {
      if (remaining <= 0 || rng() < 0.4) continue;
      const amount = Math.max(1, Math.min(20, remaining, 1 + Math.floor(rng() * 20)));
      d.stealTargets[target.playerId] = amount;
      remaining -= amount;
    }
    if (!Object.keys(d.stealTargets).length && targets.length) d.stealTargets[targets[0].playerId] = 1;
  }
