...
app.get('/health', (_req, res) => {
  ...
  res.json({ ok: true, game: '五疑', activeRooms, storedRooms: rooms.size, sockets: activeSocketConnections, uptime: Math.floor(process.uptime()) });
});
...
function log(room, text, targets = null) {
  ...
  if (room.logs.length > 180) room.logs.shift();
...
function visibleLogs(room, playerId) {
  return room.logs.filter(x => !x.targets || x.targets.includes(playerId)).slice(-80);
}
function visibleChat(room, playerId) {
  return room.chat.filter(m => !m.toId || m.fromId === playerId || m.toId === playerId).slice(-160);
}
...
function snapshot(room, playerId, { includeHistory = false } = {}) {
  ...
  const data = {
    ...
    normalCards: NORMAL_CARDS,
    specialCards: SPECIAL_CARDS,
    objectives: OBJECTIVES,
    rules: { scoring: SCORING, stealAmounts: STEAL_AMOUNTS, secretReward: SECRET_REWARD }
  };
  if (includeHistory) {
...
function emitState(room, { includeHistoryFor = null } = {}) {
  for (const p of room.players) {
    if (p.connected && p.socketId) {
      ...
      io.to(p.socketId).emit('state', snapshot(room, p.playerId, { includeHistory }));
    }
  }
}
function emitPlayerState(room, p, { includeHistory = false } = {}) {
  if (p?.connected && p.socketId) io.to(p.socketId).emit('state', snapshot(room, p.playerId, { includeHistory }));
}
...
function commitMutation(p, key) {
  ...
  while (p._recentMutations.size > 96) p._recentMutations.delete(p._recentMutations.keys().next().value);
}
...
server.listen(PORT, '0.0.0.0', () => {
  console.log(`五疑 / GOGI: http://localhost:${PORT}`);
});
