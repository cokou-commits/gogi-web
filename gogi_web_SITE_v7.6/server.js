...
function buildTestCpuDraft(room, p) {
  ...
  if (['attack','scout','accusation'].includes(normal) && targets.length) {
    ...
    if (normal === 'accusation' && contract?.conditionType === 'accuseTarget') target = targets.find(x => x.playerId === contract.subjectId) || null;
...
function tryAcceptPublicContractAsCpu(room, contract) {
  ...
  log(room, `公開契約参加：${cpu.color.label}（CP）が${issuer?.color?.label || '提示者'}の契約に参加しました。`);
...
function expireOpenPublicContracts(room) {
  ...
  for (const contract of room.publicContracts) {
    ...
    if (acceptorIds.length > 0) {
      ...
      log(room, `${issuer?.color?.label || '提示者'}の公開契約は${acceptorIds.length}人参加で受付終了。第${contract.dueTurn}ターンに判定します。`);
    } else {
