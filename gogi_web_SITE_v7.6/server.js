...
const BUILD_ID = '20260913-complete-final7';
...
function maybeCreateCpuExchanges(room) {
  ...
  for (const cpu of room.players) {
    ...
    emitGameNotice(room, [plan.target.playerId], { kind:'exchange', toolTarget:'transfer', text:`${cpu.color.label}（CP）から交換提案が届きました。` });
...
function cpuMaybeCounterOffer(room, cpu, human, rejectedReq) {
  ...
  emitGameNotice(room, [human.playerId], { kind:'exchange', toolTarget:'transfer', text:`${cpu.color.label}（CP）から条件変更の交換提案が届きました。` });
  emitPlayerState(room,cpu); emitPlayerState(room,human);
  return counter;
}
...
io.on('connection', socket => {
  ...
  socket.on('createExchange', (rawPayload = {}, cb) => {
    ...
    if (!target || (target.ready && !isCpu(target))) return safeCb(cb, { ok:false, message:'交換相手が不正、または準備OK中です。' });
