...
const BUILD_ID = '20260913-turn-history1-cptest';
...
// 確認テスト版専用CP。外部AI/APIは使わず、サーバー内の合法手だけを自動選択する。
...
function cpuAcceptedContract(room, p) {
  return (room.publicContracts || []).find(c => ['open','accepted'].includes(c.status) && Array.isArray(c.acceptorIds) && c.acceptorIds.includes(p.playerId) && c.dueTurn === room.turn) || null;
}
function buildTestCpuDraft(room, p) {
  const empty = { normal:null, special:null, normalTargetId:null, secondNormalTargetId:null, specialTargetId:null, accusationGuess:null, secondAccusationGuess:null, stealAmount:5, specifiedType:null, specifiedTargetId:null };
  if (!p?.alive) return empty;
  const aliveTargets = cpuAliveTargets(room, p);
  const anyTargets = cpuAnyTargets(room, p);
  const accusationTargets = anyTargets.filter(x => !x.secretState?.invalid);
  const contract = cpuAcceptedContract(room, p);
  let normal = null;
  const forced = p.forcedNormalType?.turn === room.turn && !p.forcedNormalType.conflict ? p.forcedNormalType.type : null;
  if (forced && (p.hand[forced] || 0) > 0) normal = forced;
  else {
    const available = Object.keys(NORMAL_CARDS).filter(k => (p.hand[k] || 0) > 0);
    if (contract?.conditionType === 'attackTarget' && (p.hand.attack || 0) > 0) normal = 'attack';
    else if (contract?.conditionType === 'defendIssuer' && (p.hand.defense || 0) > 0) normal = 'defense';
    else if (contract?.conditionType === 'accuseTarget' && (p.hand.accusation || 0) > 0) normal = 'accusation';
    else if (contract?.conditionType === 'dontAttackIssuer') {
      const safe = available.filter(k => k !== 'attack');
      normal = cpuRandom(safe.length ? safe : available);
    } else if (p.hp <= 2 && (p.hand.heal || 0) > 0) normal = 'heal';
    else normal = cpuRandom(available);
  }
  const d = { ...empty, normal };
  if (normal === 'heal') d.normalTargetId = p.playerId;
  if (normal === 'defense') {
    const protect = contract?.conditionType === 'defendIssuer' ? getPlayer(room, contract.issuerId) : p;
    d.normalTargetId = protect?.alive ? protect.playerId : p.playerId;
  }
  if (normal === 'attack') {
    let target = null;
    if (contract?.conditionType === 'attackTarget') target = aliveTargets.find(x => x.playerId === contract.subjectId) || null;
    if (!target && contract?.conditionType === 'dontAttackIssuer') {
      const safeTargets = aliveTargets.filter(x => x.playerId !== contract.issuerId);
      target = cpuRandom(safeTargets.length ? safeTargets : aliveTargets);
    }
    target ||= cpuRandom(aliveTargets);
    d.normalTargetId = target?.playerId || null;
  }
  if (normal === 'scout') d.normalTargetId = cpuRandom(anyTargets)?.playerId || null;
  if (normal === 'accusation') {
    let target = null;
    if (contract?.conditionType === 'accuseTarget') target = accusationTargets.find(x => x.playerId === contract.subjectId) || null;
    target ||= cpuRandom(accusationTargets);
    d.normalTargetId = target?.playerId || null;
    d.accusationGuess = cpuRandom(OBJECTIVES)?.key || null;
  }

  // 低確率で特殊カードも使い、特殊処理を実機確認しやすくする。
  const specialKeys = Object.keys(SPECIAL_CARDS).filter(k => (p.specials[k] || 0) > 0);
  if (specialKeys.length && crypto.randomInt(100) < 28) {
    const special = cpuRandom(specialKeys);
    if (special !== 'double' || d.normal) {
      d.special = special;
      if (['cancel','specify','steal'].includes(special)) d.specialTargetId = cpuRandom(aliveTargets)?.playerId || null;
      if (special === 'specify' && d.specialTargetId) {
        const forcedPlayer = getPlayer(room, d.specialTargetId);
        const ownedTypes = Object.keys(NORMAL_CARDS).filter(k => (forcedPlayer?.hand?.[k] || 0) > 0);
        d.specifiedType = cpuRandom(ownedTypes.length ? ownedTypes : Object.keys(NORMAL_CARDS));
        if (d.specifiedType) {
          let pool = room.players.filter(x => {
            if (['attack','scout','accusation'].includes(d.specifiedType) && x.playerId === d.specialTargetId) return false;
            if (['attack','defense','heal'].includes(d.specifiedType) && !x.alive) return false;
            if (d.specifiedType === 'accusation' && x.secretState?.invalid) return false;
            return true;
          });
          d.specifiedTargetId = cpuRandom(pool)?.playerId || null;
        }
      }
      if (special === 'steal') d.stealAmount = cpuRandom(STEAL_AMOUNTS) || 5;
      if (special === 'double' && ['attack','scout','accusation'].includes(d.normal)) {
        const basePool = d.normal === 'attack' ? aliveTargets : d.normal === 'accusation' ? accusationTargets : anyTargets;
        const secondPool = d.normal === 'scout' ? basePool.filter(x => x.playerId !== d.normalTargetId) : basePool;
        d.secondNormalTargetId = cpuRandom(secondPool)?.playerId || null;
        if (d.normal === 'accusation' && d.secondNormalTargetId) d.secondAccusationGuess = cpuRandom(OBJECTIVES)?.key || null;
      }
    }
  }
  const checked = validateDraft(room, p, d, { strict:true });
  return checked.ok ? checked.draft : empty;
}
function createTestCpuPlayer(room) {
  ...
  p.connected = false;
  p.autoAdvance = true;
  // テストCPはその部屋の賭け額を必ず払える残高を持つ。実ユーザーのウォレットには影響しない。
...
function maybePlaceCpuWinnerBets(room) {
  ...
  for (const p of room.players) {
    ...
    const target = cpuRandom(room.players.filter(x => x.playerId !== p.playerId));
    if (target) placeWinnerBet(room, p, target.playerId, WINNER_BET.step);
  }
}
function tryAcceptExchangeAsCpu(room, req) {
  ...
  if (cpu.points < wantPoints) return false;
  let targetCard = null;
  if (req.request?.card) {
...
function tryAcceptPublicContractAsCpu(room, contract) {
  if (!contract || contract.status !== 'open') return 0;
  if (!Array.isArray(contract.acceptorIds)) contract.acceptorIds = [];
  // 条件上参加可能なCPは全員自動参加する。
  const candidates = room.players.filter(p => isTestCpu(p) && p.alive && p.playerId !== contract.issuerId && !contract.acceptorIds.includes(p.playerId) && !(['attackTarget','accuseTarget'].includes(contract.conditionType) && contract.subjectId === p.playerId));
  ...
  for (const cpu of candidates) {
    contract.acceptorIds.push(cpu.playerId);
    cpu.turnHadManualInput = true;
    // 現在ターン契約なので、テストCPも参加直後にそのターンの行動を再選択する。
    cpu.draft = buildTestCpuDraft(room, cpu);
    cpu.ready = true;
    log(room, `公開契約参加：${cpu.color.label}（CP）が${issuer?.color?.label || '提示者'}の契約に参加しました（報酬1人${perPlayer}P）。`);
  }
...
function expireOpenPublicContracts(room) {
  ...
  for (const contract of room.publicContracts) {
    ...
    if (acceptorIds.length > 0) {
      ...
      emitGameNotice(room, room.players.map(x => x.playerId), { kind:'contract', toolTarget:'contractBox', text:`${issuerColor}の公開契約：${acceptorIds.length}人参加 / 報酬1人${perPlayer}P / 第${contract.dueTurn}ターンに判定` });
    } else {
      contract.status = 'expired';
      refundPublicContract(room, contract);
      if (issuer) {
        log(room, `${issuer.color.label}の公開契約は参加者なしで終了し、使用Pが返却されました。`);
        emitGameNotice(room, room.players.map(x => x.playerId), { kind:'contract', toolTarget:'contractBox', text:`${issuer.color.label}の公開契約は参加者なしで終了 / 報酬1人${perPlayer}P / 使用P返却` });
      }
    }
  }
}
function settleDuePublicContracts(room, result) {
  ...
  for (const contract of room.publicContracts) {
    ...
    if (relevantPlayer && !aliveAtTurnStart(relevantPlayer)) {
      ...
      const text = `公開契約は${color}が判定ターン開始前に脱落済みのため取消し、使用Pを${issuer?.color?.label || '提示色'}へ返却しました。`;
    ...
    const text = successCount > 0
      ? `公開契約結果：${successColors.join('・')}が達成。報酬は1人${perPlayer}P。未配布分は${issuer?.color?.label || '提示者'}へ返却しました。`
      : `公開契約結果：達成者なし。使用Pは${issuer?.color?.label || '提示者'}へ返却しました。`;
    result.publicEvents.push({ type:'contract', text });
    emitGameNotice(room, room.players.map(x => x.playerId), {
      kind:'contract',
      toolTarget:'contractBox',
      text: successCount > 0
        ? `公開契約結果：${successColors.join('・')}が達成 / 報酬1人${perPlayer}P / 未配布分は${issuer?.color?.label || '提示色'}へ返却`
        : `公開契約結果：達成者なし / 使用Pは${issuer?.color?.label || '提示色'}へ返却`
    });
  }
}
...
io.on('connection', socket => {
  ...
  socket.on('postPublicContract', (rawPayload = {}, cb) => {
    ...
    log(room, `公開契約：${p.color.label}が「${condition}」を提示しました（報酬1人${perPlayer}P）。`);
    emitGameNotice(room, room.players.map(x => x.playerId), { kind:'contract', toolTarget:'contractBox', text:`${p.color.label}が公開契約を提示：${condition} / 報酬1人${perPlayer}P` });
    tryAcceptPublicContractAsCpu(room, contract);
  ...
  socket.on('acceptPublicContract', (rawPayload = {}, cb) => {
    ...
    const issuer = getPlayer(room, contract.issuerId);
    const perPlayer = contractRewardPerPlayer(contract.reward, contract.dueTurn === MAX_TURNS ? 2 : 1);
    log(room, `公開契約参加：${p.color.label}が${issuer?.color?.label || '提示者'}の契約に参加しました（報酬1人${perPlayer}P）。`);
    emitGameNotice(room, room.players.map(x => x.playerId), { kind:'contract', toolTarget:'contractBox', text:`${p.color.label}が${issuer?.color?.label || '提示者'}の公開契約に参加 / 報酬1人${perPlayer}P` });
  ...
  socket.on('cancelPublicContract', (rawPayload = {}, cb) => {
    ...
    log(room, `${p.color.label}が公開契約を取り消しました。使用Pを返却しました。`);
    emitGameNotice(room, room.players.map(x => x.playerId), { kind:'contract', toolTarget:'contractBox', text:`${p.color.label}が公開契約を取消 / 報酬1人${contractRewardPerPlayer(contract.reward, contract.dueTurn === MAX_TURNS ? 2 : 1)}P / 使用P返却` });
    emitState(room); safeCb(cb, { ok:true });
  });
