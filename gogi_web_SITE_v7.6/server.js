...
const BUILD_ID = '20260913-gogi-zero-recharge1-cptest';
...
function buildTestCpuDraft(room, p) {
  ...
  if (normal === 'accusation') {
    let target = null;
    if (contract?.conditionType === 'accuseTarget') target = anyTargets.find(x => x.playerId === contract.subjectId) || null;
    target ||= cpuRandom(anyTargets);
    d.normalTargetId = target?.playerId || null;
  ...
  if (specialKeys.length && crypto.randomInt(100) < 28) {
    ...
    if (special !== 'double' || d.normal) {
      ...
      if (special === 'double' && ['attack','scout','accusation'].includes(d.normal)) {
        const basePool = d.normal === 'attack' ? aliveTargets : anyTargets;
...
io.on('connection', socket => {
  ...
  socket.on('postPublicContract', (rawPayload = {}, cb) => {
    ...
    p.points -= n;
    const dueTurn = room.turn >= MAX_TURNS ? MAX_TURNS : room.turn + 1;
