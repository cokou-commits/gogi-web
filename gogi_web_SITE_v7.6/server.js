...
function buildTestCpuDraft(room, p) {
  ...
  const accusationTargets = room.players.filter(x => x.playerId !== p.playerId);
  ...
  if (['attack','scout','accusation'].includes(normal)) {
    const targetPool = normal === 'accusation' ? accusationTargets : targets;
    if (targetPool.length) {
      ...
      if (normal === 'accusation' && contract?.conditionType === 'accuseTarget') target = accusationTargets.find(x => x.playerId === contract.subjectId) || null;
      target ||= cpuRandom(targetPool);
  ...
  if (specialKeys.length && crypto.randomInt(100) < 28) {
    ...
    if (special !== 'double' || d.normal) {
      ...
      if (special === 'double' && ['attack','scout','accusation'].includes(d.normal)) {
        const basePool = d.normal === 'accusation' ? accusationTargets : targets;
        const secondPool = d.normal === 'scout' ? basePool.filter(x => x.playerId !== d.normalTargetId) : basePool;
        d.secondNormalTargetId = secondPool.length ? cpuRandom(secondPool)?.playerId || null : null;
