...
const {
  ...
  awardSurvivalBonus, buildRanking, currentPointsStanding, recordStructuredStatement, canTransferPoints,
  randomId, randomRoomCode, invalidatePendingStructuredStatements, canTransferNormalCard
} = require('./src/engine');
const {
  chooseCpuDraft, cpuStructuredStatementPayload, chooseCpuPointTransfer, chooseCpuPurchasePlan,
  cpuOpeningChatPlan, cpuReplyToChat
} = require('./src/cpu');
const { generateCpuChatText, aiChatConfigured } = require('./src/cpu-ai');
...
function resultVoters(room) {
  // 結果の『次へ』は接続中の人間全員で判定。CPは待ち時間を作らない。
  return room.players.filter(p => !p.isCpu && p.connected);
}
...
function snapshot(room, playerId, { includeHistory = false, includeCatalog = true } = {}) {
  ...
  const data = {
    ...
    canStartWithCpu: !!(p && room.status === 'lobby' && room.ownerPlayerId === p.playerId && room.players.length >= 1 && room.players.length < MAX_PLAYERS)
  };
...
function applyAfkPolicy(room, result) {
  for (const p of room.players) {
    ...
    if (p.afkStreak >= 3) {
      ...
      // 第15ターン終了時の生存が条件の秘密目標は、AFK脱落者には成立させない。
      if (room.turn >= MAX_TURNS && p.secretState?.achievedTurn === room.turn && ['nearDeath', 'hermit'].includes(p.objective?.key)) {
        const awarded = Number(p.secretState.awardedPoints || 0);
        if (awarded > 0) p.points -= awarded;
        p.secretState.achieved = false;
        p.secretState.achievedTurn = null;
        p.secretState.awardedPoints = 0;
        const scoreIndex = result.scoreEvents.findIndex(e => e.playerId === p.playerId && e.reason === '秘密目標達成');
        if (scoreIndex >= 0) result.scoreEvents.splice(scoreIndex, 1);
      }
...
function beginChat(room) {
  ...
  for (const p of room.players) {
    p.ready = !p.alive || p.isCpu || p.autoAdvance;
    // 会話中に選択まで完結するため、actionLockedは互換状態としてreadyと同期する。
    p.actionLocked = p.ready;
    p.resultReady = false;
    p.autoReadySeq = null;
    p.autoLockSeq = null;
  ...
  // CPは人間の待ち時間を引き延ばさないよう、会話フェーズ開始時に行動を決めて準備済みにする。
  for (const p of room.players) if (p.isCpu && p.alive) prepareCpuForTurn(room, p);
  log(room, `ターン${room.turn}：会話・行動選択開始（最大${CHAT_SECONDS}秒）`);
  setPhase(room, 'chat', CHAT_SECONDS, () => resolveAndShowResult(room));
  scheduleCpuOpeningChats(room);
  for (const p of room.players) if (p.alive && !p.isCpu && p.autoAdvance && p.ready) p.autoReadySeq = room.phaseSeq;
  if (allAliveReady(room)) resolveAndShowResult(room);
}
function captureTurnState(room) {
  const fields = ['hp','maxHp','points','alive','hand','specials','specialPurchased','normalPurchasedTurn','secretState','draft','ready','actionLocked','forcedNormalType','scoutReports','stats','cpuBrain'];
...
function resolveAndShowResult(room) {
  ...
  for (const p of room.players) p.resultReady = !!p.isCpu || !p.connected;
  room.phase = 'result';
  room.phaseSeq++;
  const seq = room.phaseSeq;
...
function makeCpuPlayer(room) {
  const p = createPlayer({ socketId: null });
  p.isCpu = true;
  // Socketは持たないが、対戦中の公開表示では「切断」扱いにしない。
  p.connected = true;
  p.autoAdvance = false;
  room.players.push(p);
  return p;
...
function appendCpuStructuredMessage(room, p, draft = null) {
  const payload = cpuStructuredStatementPayload(room, p, draft);
  if (!payload) return;
...
function applyCpuGiverTransfer(room, p) {
  const plan = chooseCpuPointTransfer(room, p);
  if (!plan) return;
  const target = getPlayer(room, plan.targetId);
  const amount = Number(plan.amount);
  if (!target?.alive || target.playerId === p.playerId || !Number.isInteger(amount) || amount < 5 || amount % 5 !== 0 || p.points < amount) return;
  p.points -= amount;
  target.points += amount;
  p.stats.pointTransfers++;
  log(room, `${p.color.label}（CP）から${target.color.label}へ${amount}Pが譲渡されました。`);
}

function applyCpuPurchase(room, p) {
  const plan = chooseCpuPurchasePlan(room, p);
  if (!plan) return null;
  if (plan.type === 'special') {
    if (p.specialPurchased || p.points < 50) return null;
  ...
  const def = NORMAL_CARDS[plan.type];
  if (!def || p.normalPurchasedTurn === room.turn || p.points < def.price) return null;
  p.points -= def.price;
  p.hand[plan.type] = (p.hand[plan.type] || 0) + 1;
  p.normalPurchasedTurn = room.turn;
  p.stats.normalPurchases++;
  p.stats.lastNormalPurchaseType = plan.type;
  p.stats.lastNormalPurchaseTurn = room.turn;
  log(room, `${p.color.label}（CP）が通常カードを購入しました。`);
  return { type:plan.type };
}

function prepareCpuForTurn(room, p) {
  if (!p?.isCpu || !p.alive) return;
  applyCpuGiverTransfer(room, p);
  applyCpuPurchase(room, p);
  let draft = chooseCpuDraft(room, p);
  let validation = validateDraft(room, p, draft, { strict: true });
  if (!validation.ok) {
    // 特殊カードだけが不成立なら通常カードだけで再検証し、それでも駄目なら行動なし。
    draft = { ...draft, special: null, specialTargetId: null, specifiedType: null, secondNormalTargetId: null, secondAccusationGuess: null };
    validation = validateDraft(room, p, draft, { strict: true });
  }
  p.draft = validation.ok ? validation.draft : {
    normal: null, special: null,
    normalTargetId: null, secondNormalTargetId: null, specialTargetId: null,
    accusationGuess: null, secondAccusationGuess: null,
    stealAmount: 25, specifiedType: null
  };
  // 行動案を決めた後に情報発言を作ることで「次の行動」の真偽も自然に扱える。
  appendCpuStructuredMessage(room, p, p.draft);
  p.ready = true;
  p.actionLocked = true;
}


function emitCpuFreeChat(room, p, { text, toId = null } = {}) {
  if (!room || room.status !== 'playing' || room.phase !== 'chat' || !p?.isCpu || !p.alive) return false;
  const clean = cleanText(text, 300);
  if (!clean) return false;
  let target = null;
  if (toId) {
    target = getPlayer(room, toId);
    if (!target?.alive || target.playerId === p.playerId) return false;
  }
  const message = {
    id: randomId('msg'),
    fromId: p.playerId,
    fromColor: p.color?.label || 'CP',
    toId: target?.playerId || null,
    toColor: target?.color?.label || null,
    text: clean,
    at: now(),
    structured: false,
    fromCpu: true
  ...
  if (room.chat.length > CHAT_HISTORY_LIMIT) room.chat.shift();
  if (target) {
    if (target.connected && target.socketId) io.to(target.socketId).emit('chatMessage', message);
  } else {
    io.to(room.id).emit('chatMessage', message);
  }
  return true;
...
function scheduleCpuChat(room, p, plan, phaseSeq) {
  const delay = Math.max(200, Math.min(25_000, Number(plan?.delayMs) || 1000));
  setTimeout(async () => {
    if (rooms.get(room.id) !== room) return;
    if (room.status !== 'playing' || room.phase !== 'chat' || room.phaseSeq !== phaseSeq) return;
    if (!p.alive) return;

    // 戦略判断はローカルCPが済ませ、外部AIは自然な言い回しだけを担当する。
    // APIが未設定・無料枠上限・遅延・障害のときは即座にローカル文へフォールバックする。
    const text = await generateCpuChatText(room, p, plan);
    if (rooms.get(room.id) !== room) return;
    if (room.status !== 'playing' || room.phase !== 'chat' || room.phaseSeq !== phaseSeq || !p.alive) return;
    emitCpuFreeChat(room, p, { ...plan, text });
  }, delay);
}

function scheduleCpuOpeningChats(room) {
  const seq = room.phaseSeq;
  const openings = [];
  for (const p of room.players) {
    if (!p.isCpu || !p.alive) continue;
    const plans = cpuOpeningChatPlan(room, p, p.draft);
    for (const plan of plans) scheduleCpuChat(room, p, plan, seq);
    if (plans[0]) openings.push({ p, plan: plans[0] });
  }

  // CP同士も完全に無言にはせず、1ターン最大1往復だけ公開会話へ反応させる。
  // 無制限の応答連鎖は作らないので、チャットスパムや再帰ループにはならない。
  const cpAlive = room.players.filter(x => x.isCpu && x.alive);
  if (openings.length && cpAlive.length >= 2 && crypto.randomInt(100) < 72) {
    const seed = openings[crypto.randomInt(openings.length)];
    const responders = cpAlive.filter(x => x.playerId !== seed.p.playerId);
    const responder = responders[crypto.randomInt(responders.length)];
    const wait = Math.max(700, Number(seed.plan.delayMs || 800) + 250 + crypto.randomInt(900));
    setTimeout(() => {
      if (rooms.get(room.id) !== room || room.status !== 'playing' || room.phase !== 'chat' || room.phaseSeq !== seq) return;
      const reply = cpuReplyToChat(room, responder, seed.p, seed.plan.text, { direct:false });
      if (!reply) return;
      scheduleCpuChat(room, responder, { ...reply, delayMs: 350 + crypto.randomInt(900) }, seq);
    }, wait);
  }
}

function scheduleCpuReactionToHuman(room, sender, text, directTarget = null) {
  if (!room || room.status !== 'playing' || room.phase !== 'chat' || !sender?.alive) return;
  let cpCandidates = directTarget?.isCpu
    ? [directTarget]
    : room.players.filter(x => x.isCpu && x.alive && x.playerId !== sender.playerId);
  if (!cpCandidates.length) return;

  // 公開チャットで複数CPが一斉返信すると機械的なので、1人だけ反応させる。
  if (cpCandidates.length > 1) {
    const offset = crypto.randomInt(cpCandidates.length);
    cpCandidates = [...cpCandidates.slice(offset), ...cpCandidates.slice(0, offset)];
  }
  for (const cpu of cpCandidates) {
    const plan = cpuReplyToChat(room, cpu, sender, text, { direct: directTarget?.playerId === cpu.playerId });
    if (!plan) continue;
    scheduleCpuChat(room, cpu, plan, room.phaseSeq);
    break;
  }
}
...
function registerPlayer(room, socket, clientInstanceId, browserId, pageInstanceId) {
  ...
  p.browserId = normalizedBrowserId;
  room.players.push(p);
  if (!room.ownerPlayerId) room.ownerPlayerId = p.playerId;
  ...
  try {
    ...
    room.players.pop();
    if (room.ownerPlayerId === p.playerId) room.ownerPlayerId = room.players.find(x => !x.isCpu)?.playerId || null;
...
io.on('connection', socket => {
  ...
  socket.on('startWithCpu', (_rawPayload = {}, cb) => {
    if (!allow(socket, 'startWithCpu', 3, 10_000)) return safeCb(cb, { ok: false, message: '操作が多すぎます。' });
    const { room, p } = socketPlayer(socket);
    if (!room || !p) return safeCb(cb, { ok: false, message: '参加中の部屋がありません。' });
    if (room.status !== 'lobby') return safeCb(cb, { ok: false, message: 'すでに対戦が開始されています。' });
    if (room.ownerPlayerId !== p.playerId) return safeCb(cb, { ok: false, message: 'CP補充で開始できるのは部屋を作ったプレイヤーだけです。' });
    if (room.players.length >= MAX_PLAYERS) {
      maybeStart(room);
      return safeCb(cb, { ok: true, added: 0 });
    }
    const added = fillRoomWithCpu(room);
    if (added <= 0) return safeCb(cb, { ok: false, message: 'CPを追加できませんでした。' });
    log(room, `不足している${added}人をCPで補充しました。`);
    maybeStart(room);
  ...
  socket.on('chat', (rawPayload = {}, cb) => {
    ...
    if (!target || target.isCpu) scheduleCpuReactionToHuman(room, p, clean, target?.isCpu ? target : null);
    safeCb(cb, { ok: true });
  });
  ...
  socket.on('infoStatement', (rawPayload = {}, cb) => {
    ...
    if (!chatTarget || chatTarget.isCpu) scheduleCpuReactionToHuman(room, p, result.text, chatTarget?.isCpu ? chatTarget : null);
  ...
  socket.on('disconnect', () => {
    ...
    p.disconnectTimer = setTimeout(() => {
      ...
      if (room.status === 'playing') {
        p.autoAdvance = true;
        if (room.phase === 'chat') { p.ready = true; p.actionLocked = true; p.autoReadySeq = room.phaseSeq; }
...
server.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(`CP AI chat: ${aiChatConfigured() ? 'Gemini enabled' : 'local fallback'}`);
});
