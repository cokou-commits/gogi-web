...
const {
  ...
  SCORING, STEAL_MAX_TOTAL, STEAL_MAX_PER_TARGET, SECRET_REWARD, SPECIAL_PURCHASE_PRICE, SPECIAL_SPECIFIED_PURCHASE_PRICE, TURN_START_BONUSES, WINNER_BET, GOGI_CHIPS
} = require('./src/rules');
const {
...
const BUILD_ID = '20260914-complete-final33-game-card-art-current-rules';
...
function cpuEmptyDraft() {
  return { normal:null, special:null, normalTargetId:null, secondNormalTargetId:null, specialTargetId:null, accusationGuess:null, secondAccusationGuess:null, stealTargets:{}, cancelKind:null, specifiedType:null, specifiedTargetId:null };
}
function cpuBrain(p) {
...
function cpuNormalCandidateScore(room, p, normal, target) {
  ...
  if (normal === 'defense') {
    if (targetId === p.playerId) {
      ...
      // 他人防御は実際に攻撃が来れば+10P。複数人防御でも全員成功の現行ルールを反映する。
...
function cpuChooseSpecial(room, p, plan) {
  ...
  if (owned.includes('steal')) {
    // 他人の所持Pは非公開なので、CPも実値を見て対象選別しない。局面価値だけで使用判断する。
    options.push({ special:'steal', score:25 + ctx.comebackPressure * 0.7 + (room.turn >= 10 ? 10 : 0) - reserveBias });
  }
  if (owned.includes('specify')) {
    const specifyOptions = [];
    // カード指定そのものの成功点は第15ターンでも固定+20P。
    const specifyFixedValue = 20;
    ...
    for (const forcedPlayer of forcedPlayers) {
      ...
      if (canLikelyUse('attack')) {
        ...
        if (victim) specifyOptions.push({ special:'specify', target:forcedPlayer, forcedType:'attack', forcedTarget:victim,
          score:20 + specifyFixedValue + knowledge('attack') + cpuAttackOpportunity(room,p,victim) * 0.72 + threat * 0.16 + (room.turn >= 11 ? 8 : 0) - reserveBias });
      }

      // 自分への防御を強制。成功点は実行者本人へ入るが、生存率を上げられる。
      if (canLikelyUse('defense') && ownRisk >= 0.20) {
        specifyOptions.push({ special:'specify', target:forcedPlayer, forcedType:'defense', forcedTarget:p,
          score:18 + specifyFixedValue + knowledge('defense') + ownRisk * 48 + ctx.survivalPressure * 0.38 + (p.hp <= 2 ? 14 : 0) - reserveBias });
      }

      // 自分への回復を同ターンに強制。低HPからの立て直し＋成功点を狙う。
      if (canLikelyUse('heal') && p.hp < p.maxHp && p.hp >= 2) {
        ...
        specifyOptions.push({ special:'specify', target:forcedPlayer, forcedType:'heal', forcedTarget:p,
          score:17 + specifyFixedValue + knowledge('heal') + missing * 10 + ctx.survivalPressure * 0.30 - reserveBias });
      }

      // 強敵へ告発を強制。成功+30Pも失敗-15Pも実行者本人へ入るため、主目的は行動強制。
      if (canLikelyUse('accusation') && threat >= 15) {
        ...
        if (accTarget) specifyOptions.push({ special:'specify', target:forcedPlayer, forcedType:'accusation', forcedTarget:accTarget,
          score:22 + specifyFixedValue + knowledge('accusation') + threat * 0.34 + cpuObjectiveInference(room,p,accTarget).confidence * 24 + (room.turn >= 10 ? 7 : 0) - reserveBias });
      }
      ...
      if (canLikelyUse('scout') && threat >= 23) {
        ...
        if (scoutTarget && scoutTarget.playerId !== forcedPlayer.playerId) specifyOptions.push({ special:'specify', target:forcedPlayer, forcedType:'scout', forcedTarget:scoutTarget,
          score:15 + specifyFixedValue + knowledge('scout') + threat * 0.48 + (room.turn >= 12 ? 8 : 0) - reserveBias });
      }
    }
  ...
  if (['cancel','specify'].includes(best.special)) d.specialTargetId = best.target?.playerId || null;
  if (best.special === 'steal') {
    d.specialTargetId = null;
    d.stealTargets = {};
    const opponents = room.players.filter(x => x.playerId !== p.playerId);
    // 非公開の所持Pを参照せず、最大50Pを20/20/10で対象へ配分する。
    const shuffled = [...opponents].map(x => ({ x, r:crypto.randomBytes(4).readUInt32BE(0) })).sort((a,b) => a.r - b.r).map(v => v.x);
    let remaining = STEAL_MAX_TOTAL;
    for (const target of shuffled) {
      if (remaining <= 0) break;
      const amount = Math.min(STEAL_MAX_PER_TARGET, remaining);
      d.stealTargets[target.playerId] = amount;
      remaining -= amount;
    }
  }
  ...
  if (best.special === 'double' && ['attack','defense','scout','accusation','heal'].includes(d.normal)) {
    if (d.normal === 'attack') {
      const first = getPlayer(room, d.normalTargetId);
      ...
      const first = getPlayer(room, d.normalTargetId);
      // 自分が強く狙われ、2発以上受ける可能性が高い場合は同じ対象へ2回。
      // それ以外は攻撃リスクが高い別の生存者へ2回目を振り分ける。
      const ownIncoming = first?.playerId === p.playerId ? cpuIncomingAttackDistribution(room,p) : null;
      if (first?.playerId === p.playerId && ownIncoming?.pAtLeastTwo >= 0.28) {
        d.secondNormalTargetId = first.playerId;
      } else {
        const candidates = room.players.filter(x => x.alive && x.playerId !== d.normalTargetId);
        candidates.sort((a,b) => cpuEstimatedAttackRiskOnTarget(room,p,b) - cpuEstimatedAttackRiskOnTarget(room,p,a));
        d.secondNormalTargetId = candidates[0]?.playerId || d.normalTargetId;
      }
    } else if (d.normal === 'scout') {
      ...
      const first = getPlayer(room, d.normalTargetId);
      const firstMissing = first ? cpuKnownMissingHp(room,p,first) : null;
      if (first && firstMissing != null && firstMissing >= 3) {
        d.secondNormalTargetId = first.playerId;
      } else {
        const candidates = room.players.filter(x => x.alive && x.playerId !== d.normalTargetId)
          .map(x => ({ target:x, missing:cpuKnownMissingHp(room,p,x) }))
          .filter(x => x.missing != null && x.missing >= 1)
          .sort((a,b) => b.missing - a.missing);
        d.secondNormalTargetId = candidates[0]?.target?.playerId || d.normalTargetId;
      }
    } else if (d.normal === 'accusation') {
      const first = getPlayer(room, d.normalTargetId);
...
function buildCpuDraft(room, p) {
  ...
  if (!checked.ok) {
    ...
    const withoutSpecial = { ...d, special:null, secondNormalTargetId:null, specialTargetId:null, secondAccusationGuess:null, stealTargets:{}, specifiedType:null, specifiedTargetId:null };
...
function cpuSpecialStrategicValue(room,p,type) {
  ...
  else if (type === 'double') value += (room.turn >= 11 ? 13 : 5);
...
function settleDuePublicContracts(room, result) {
  ...
  const actions = room.lastEffectiveActions?.turn === room.turn
    ...
    : { attacksByPlayer:{}, defensesByPlayer:{}, healsByPlayer:{}, accusationsByPlayer:{}, specifiedActionsByPlayer:{} };
  for (const contract of room.publicContracts) {
    ...
    for (const playerId of acceptorIds) {
      ...
      if (success && acceptor) {
        // カード指定で達成した場合も、公開契約報酬は実際に行動した本人へ入る。
...
function buildPlayerTurnResult(room, playerId, turnSnapshot, result) {
  ...
  // カード指定で通常行動を強制されても、成功/失敗と得点は実際に行動した本人として正しく判定する。
  ...
  for (const p of room.players) {
    ...
    if (normal && NORMAL_CARDS[normal]) {
      ...
      if (normal === 'accusation') {
        ...
        if (d.special === 'double' && ['attack','defense','scout','heal'].includes(normal)) {
    ...
    if (special && SPECIAL_CARDS[special]) {
      if (special === 'fullDefense') actionParts.push('特殊：完全防御');
        ...
        const allocations = Object.entries(d.stealTargets || {}).filter(([,amount]) => Number(amount) > 0);
        const total = allocations.reduce((sum,[,amount]) => sum + Number(amount), 0);
        const detail = allocations.map(([id,amount]) => `${labelTarget(id)}から${amount}P`).join('・');
        actionParts.push(`特殊：ポイント泥棒（合計${total}P${detail ? ` / ${detail}` : ''}）`);
      }
...
function snapshot(room, playerId, { includeHistory = false, includeCatalog = true } = {}) {
  ...
  if (includeCatalog) {
    ...
    data.rules = { scoring: SCORING, stealRules:{ maxTotal:STEAL_MAX_TOTAL, maxPerTarget:STEAL_MAX_PER_TARGET }, secretReward: SECRET_REWARD, specialPurchasePrice: SPECIAL_PURCHASE_PRICE, specialSpecifiedPurchasePrice: SPECIAL_SPECIFIED_PURCHASE_PRICE, turnStartBonuses: TURN_START_BONUSES, winnerBet: WINNER_BET, gogiChips: GOGI_CHIPS };
  }
  if (includeHistory) {
...
function beginChat(room) {
  ...
  for (const p of room.players) {
    ...
    p.draft = {
      ...
      stealTargets: {}, specifiedType: null, specifiedTargetId: null
    };
  }
...
io.on('connection', socket => {
  ...
  socket.on('buy', (rawPayload = {}, cb) => {
    const { type, specialType, phaseSeq, opId } = objectPayload(rawPayload);
    ...
    const key = cleanText(type, 32);
    if (key === 'special') {
      const requestedSpecial = cleanText(specialType, 32);
      const specified = !!requestedSpecial;
      if (specified && !SPECIAL_CARDS[requestedSpecial]) return safeCb(cb, { ok: false, message: '指定した特殊カードが不正です。' });
      const price = specified ? SPECIAL_SPECIFIED_PURCHASE_PRICE : SPECIAL_PURCHASE_PRICE;
      if (p.points < price) return safeCb(cb, { ok: false, message: 'ポイント不足です。' });
      const keys = Object.keys(SPECIAL_CARDS);
      const special = specified ? requestedSpecial : keys[crypto.randomInt(keys.length)];
      markTurnActivity(room, p);
      commitMutation(p, mutation.key);
      p.points -= price;
      p.specials[special] = (p.specials[special] || 0) + 1;
      ...
      emitPrivateEvent(room, p.playerId, { type: 'notice', text: `「${SPECIAL_CARDS[special].label}」を${specified ? '指定購入' : 'ランダム購入'}しました（${price}P）。` });
    } else {
