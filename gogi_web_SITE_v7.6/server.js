...
function tryAcceptPublicContractAsCpu(room, contract) {
  ...
  // 確認テスト版では、1人プレイでも複数参加契約の精算を確認できるよう、
  // 条件上参加可能なCPは全員自動参加する。
...
function settleDuePublicContracts(room, result) {
  ...
  for (const contract of room.publicContracts) {
    ...
    const relevantPlayer = ['attackTarget', 'accuseTarget'].includes(contract.conditionType) ? subject : issuer;
    if (!aliveAtTurnStart(relevantPlayer)) {
