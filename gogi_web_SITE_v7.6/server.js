...
const {
  ...
  randomId, randomRoomCode, canTransferNormalCard
} = require('./src/engine');
...
function cpuAcceptedContract(room, p) {
  return (room.publicContracts || []).find(c => c.status === 'accepted' && c.acceptorId === p.playerId && c.dueTurn === room.turn) || null;
}
function buildTestCpuDraft(room, p) {
  const empty = { normal:null, special:null, normalTargetId:null, secondNormalTargetId:null, specialTargetId:null, accusationGuess:null, secondAccusationGuess:null, stealAmount:5, specifiedType:null };
  if (!p?.alive) return empty;
  const targets = cpuAliveTargets(room, p);
  ...
  if (['attack','scout','accusation'].includes(normal) && targets.length) {
    let target = null;
    if (normal === 'attack' && contract?.conditionType === 'attackTarget') target = targets.find(x => x.playerId === contract.subjectId) || null;
    if (normal === 'attack' && contract?.conditionType === 'dontAttackIssuer') {
      const safeTargets = targets.filter(x => x.playerId !== contract.issuerId);
      target = cpuRandom(safeTargets.length ? safeTargets : targets);
    }
    target ||= cpuRandom(targets);
    d.normalTargetId = target?.playerId || null;
    if (normal === 'accusation') d.accusationGuess = cpuRandom(OBJECTIVES)?.key || null;
  }
  ...
  if (specialKeys.length && crypto.randomInt(100) < 28) {
    ...
    if (special !== 'double' || d.normal) {
      d.special = special;
      if (['cancel','specify','steal'].includes(special)) d.specialTargetId = cpuRandom(targets)?.playerId || null;
      if (special === 'specify') d.specifiedType = cpuRandom(Object.keys(NORMAL_CARDS));
      if (special === 'steal') d.stealAmount = cpuRandom(STEAL_AMOUNTS) || 5;
      if (special === 'double' && ['attack','scout','accusation'].includes(d.normal) && targets.length > 1) {
        const secondPool = d.normal === 'scout' ? targets.filter(x => x.playerId !== d.normalTargetId) : targets;
...
function maybePlaceCpuWinnerBets(room) {
  if (![3,6,9].includes(room.turn)) return;
  for (const p of room.players) {
    if (!isTestCpu(p) || !p.alive || p.winnerBet || p.points < 5 || !canPlaceWinnerBet(room, p)) continue;
    const target = cpuRandom(room.players.filter(x => x.playerId !== p.playerId));
    if (target) placeWinnerBet(room, p, target.playerId, 5);
  }
}
...
function tryAcceptPublicContractAsCpu(room, contract) {
  ...
  const candidates = room.players.filter(p => isTestCpu(p) && p.alive && p.playerId !== contract.issuerId && !(contract.conditionType === 'attackTarget' && contract.subjectId === p.playerId));
  const cpu = cpuRandom(candidates);
  if (!cpu) return false;
  contract.status = 'accepted';
  contract.acceptorId = cpu.playerId;
  contract.acceptedAt = now();
  cpu.turnHadManualInput = true;
  const issuer = getPlayer(room, contract.issuerId);
  log(room, `公開契約成立：${cpu.color.label}（CP）が${issuer?.color?.label || '契約主'}の契約を受諾しました。次ターンに自動判定します。`);
  return true;
}
...
function expirePendingExchanges(room) {
  ...
  for (const req of expired) {
    ...
    const from = getPlayer(room, req.fromId);
    const to = getPlayer(room, req.toId);
    if (from && to) log(room, `${to.color.label}が未回答のため、${from.color.label}の交換提案はターン終了で自動キャンセルされました。`, [from.playerId, to.playerId]);
  }
}
...
function publicContractView(room, contract) {
  const issuer = getPlayer(room, contract.issuerId);
  const acceptor = contract.acceptorId ? getPlayer(room, contract.acceptorId) : null;
  ...
  return {
    ...
    acceptorId:contract.acceptorId || null,
    acceptorColor:acceptor?.color?.label || '',
...
function refundPublicContract(room, contract) {
  const issuer = contract && getPlayer(room, contract.issuerId);
  if (issuer) issuer.points += Number(contract.reward || 0);
}
function expireOpenPublicContracts(room) {
  ...
  for (const contract of room.publicContracts) {
    ...
    const issuer = getPlayer(room, contract.issuerId);
    if (issuer) log(room, `${issuer.color.label}の公開契約は未成立のため終了し、${contract.reward}Pが返却されました。`);
  }
}
function settleDuePublicContracts(room, result) {
  if (!Array.isArray(room.publicContracts)) return;
  const actions = room.lastEffectiveActions?.turn === room.turn ? room.lastEffectiveActions : { attacksByPlayer:{} };
  for (const contract of room.publicContracts) {
    if (contract.status !== 'accepted' || contract.dueTurn !== room.turn) continue;
    const issuer = getPlayer(room, contract.issuerId);
    const acceptor = getPlayer(room, contract.acceptorId);
    const attacks = new Set(actions.attacksByPlayer?.[contract.acceptorId] || []);
    ...
    if (success && acceptor) acceptor.points += Number(contract.reward || 0);
    else if (issuer) issuer.points += Number(contract.reward || 0);
    const text = success
      ? `公開契約成立：${acceptor?.color?.label || '契約者'}が条件を達成し、${contract.reward}Pを受け取りました。`
      : `公開契約不成立：条件未達のため${contract.reward}Pは${issuer?.color?.label || '契約主'}へ返却されました。`;
  ...
  if (normal && NORMAL_CARDS[normal]) {
    ...
    if (normal === 'attack') success = hasScoreReason('攻撃成功');
      ...
      success = ownPrivate.some(e => {
        const m = String(e.text || '').match(/^HPを(\d+)回復しました。$/);
...
function resolveAndShowResult(room) {
  ...
  // 第15ターンの結果画面時点で生存ボーナスも2倍（+100P）で反映し、結果画面と最終順位のポイント差をなくす。
...
function finishGame(room) {
  ...
  room.phaseSeq++;
  room.finishedAt = now();
  log(room, 'ゲーム終了。賭け判定前順位と払戻し後の最終順位を公開します。');
  emitState(room);
}
function maybeStart(room) {
  if (room.status !== 'lobby') return;
  if (room.players.length !== MAX_PLAYERS) return;
  if (!room.players.every(p => p.connected || isTestCpu(p))) return;
  try {
    startGame(room);
...
io.on('connection', socket => {
  ...
  socket.on('createExchange', (rawPayload = {}, cb) => {
    ...
    if (Array.isArray(room.exchangeRequests) && room.exchangeRequests.some(x => x.turn === room.turn && x.fromId === p.playerId)) return safeCb(cb, { ok:false, message:'同時に出せる交換提案は1件までです。' });
  ...
  socket.on('postPublicContract', (rawPayload = {}, cb) => {
    ...
    if (!Array.isArray(room.publicContracts)) room.publicContracts = [];
    if (room.publicContracts.some(c => ['open','accepted'].includes(c.status) && c.issuerId === p.playerId)) return safeCb(cb, { ok:false, message:'同時に公開できる契約は1件までです。' });
    const n = Number(reward);
    if (!Number.isInteger(n) || n < 5 || n % 5 !== 0 || n > p.points) return safeCb(cb, { ok:false, message:'報酬は所持P以内の5P刻みで指定してください。' });
    const kind = cleanText(conditionType, 32);
    let subject = null;
    if (kind === 'attackTarget') {
      subject = validTarget(room, p, subjectId);
      if (!subject) return safeCb(cb, { ok:false, message:'攻撃対象が不正です。' });
    } else if (kind !== 'dontAttackIssuer') return safeCb(cb, { ok:false, message:'契約条件が不正です。' });
    p.points -= n;
    const contract = { contractId:randomId('ct'), issuerId:p.playerId, acceptorId:null, conditionType:kind, subjectId:subject?.playerId || null, reward:n, createdTurn:room.turn, dueTurn:room.turn + 1, status:'open', createdAt:now() };
    room.publicContracts.push(contract);
    markTurnActivity(room, p); commitMutation(p, mutation.key);
    const condition = kind === 'attackTarget' ? `次ターンに${subject.color.label}を有効な攻撃で狙う` : `次ターンに契約主${p.color.label}を攻撃しない`;
    log(room, `公開契約：${p.color.label}が「${condition}」報酬${n}Pを提示しました。`);
  ...
  socket.on('acceptPublicContract', (rawPayload = {}, cb) => {
    ...
    if (contract.conditionType === 'attackTarget' && contract.subjectId === p.playerId) return safeCb(cb, { ok:false, message:'自分自身を攻撃する契約は受けられません。' });
    contract.status = 'accepted'; contract.acceptorId = p.playerId; contract.acceptedAt = now();
    markTurnActivity(room, p); commitMutation(p, mutation.key);
    const issuer = getPlayer(room, contract.issuerId);
    log(room, `公開契約成立：${p.color.label}が${issuer?.color?.label || '契約主'}の契約を受諾しました。次ターンに自動判定します。`);
  ...
  socket.on('cancelPublicContract', (rawPayload = {}, cb) => {
    ...
    const contract = Array.isArray(room.publicContracts) ? room.publicContracts.find(c => c.contractId === cleanText(contractId, 96)) : null;
    if (!contract || contract.status !== 'open' || contract.issuerId !== p.playerId || contract.createdTurn !== room.turn) return safeCb(cb, { ok:false, message:'取り消せる契約がありません。' });
